...
const AuthPage: React.FC = () => {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      const msg = err.response?.data?.error || 'Google sign-in is not available at the moment.';
      if (activeTab === 'login') setLoginError(msg);
            ...
            {activeTab === 'login' && !twoFaRequired && !pinMode && (
              <form onSubmit={handleLogin} className="space-y-5">
                ...
                <button
                  ...
                  className="w-full py-2.5 px-4 bg-white border border-gray-300 text-gray-700 font-medium rounded-lg hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-gray-200 focus:ring-offset-1 transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-sm flex items-center justify-center gap-3"
                >
                  {googleLoading ? (
