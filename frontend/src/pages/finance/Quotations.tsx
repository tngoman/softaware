...
const Quotations: React.FC = () => {
  ...
  // Force reload when navigating back to list (no ID)
  useEffect(() => {
    if (!id) {
      loadQuotations();
    }
  ...
  const handleSort = (column: string, order: 'asc' | 'desc') => {
    setSortBy(column);
    setSortOrder(order);
    setPagination(prev => ({ ...prev, page: 0 })); // Reset to first page on sort
  };
  ...
  const deleteQuotation = async (quotation: Quotation) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        await QuotationModel.delete(quotation.quotation_id);
  ...
  const generatePDF = async (quotation: Quotation) => {
    try {
      ...
      notify.success('PDF opened in new tab');
    } catch (error: any) {
            ...
            <div className="grid grid-cols-2 gap-6">
              ...
              <div className="text-right">
                <div className="flex items-center justify-end gap-3">
                  ...
                  <Can permission="quotations.delete">
                    <button
                      onClick={() => deleteQuotation(selectedQuote)}
                      className="inline-flex items-center justify-center w-9 h-9 rounded-lg bg-red-50 text-scarlet hover:bg-red-100 transition-colors"
                      title="Delete Quotation"
                    >
