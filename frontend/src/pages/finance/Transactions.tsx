...
const Transactions: React.FC = () => {
  ...
  const handleProcessPendingPayments = async () => {
    ...
    try {
      ...
      if (errors.length > 0) {
        ...
        notify.info('No payments were processed. Clear income transactions to reset processed payments and try again.');
      } else {
      ...
      notify.error(error?.response?.data?.error || error?.message || 'Unable to process payments.');
    } finally {
  ...
  const columns: ColumnDef<Transaction>[] = [
    ...
    },
    {
      ...
      header: 'Type',
      cell: ({ getValue }) => getTypeBadge(getValue() as 'income' | 'expense'),
    },
    {
    ...
    },
    {
      ...
      header: 'Category',
      cell: ({ row }) => {
        if (row.original.transaction_type === 'income') {
          return (row.original as any).income_type || '-';
        }
        return '-';
      },
    },
    ...
    },
    {
      id: 'vat',
      header: 'VAT',
      cell: ({ row }) => getVatTypeBadge(row.original.vat_type || 'non-vat'),
    },
    {
      id: 'vat_amount',
      header: 'VAT Amount',
      cell: ({ row }) => formatCurrency(Number(row.original.vat_amount || 0)),
    },
    {
      id: 'actions',
      header: 'Actions',
      cell: ({ row }) => (
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={() => handleDelete(row.original.id || row.original.transaction_id!)}
