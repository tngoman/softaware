  ...
  const authenticate = async (useOtp = false) => {
    ...
    try {
      ...
      if (data.success && data.token) {
        localStorage.setItem('software_token', data.token);
        setStatus('success');
...
const SoftwareDialog: React.FC<{
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (software) {
        await api.put(`/softaware/software?id=${software.id}`, form);
      } else {
...
const SoftwareManagement: React.FC = () => {
  ...
  const isAuthenticated = !!localStorage.getItem('software_token');
    ...
    <div className="space-y-4">
      ...
      {authSoftware && (
        <AuthenticateDialog
          ...
          onSuccess={() => { toast.success('Authenticated!'); setAuthSoftware(null); }}
        />
      )}
    </div>
  );
