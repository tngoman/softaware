...
const UpdateDialog: React.FC<{
  ...
  const handleUploadFile = async (): Promise<string | null> => {
    ...
    try {
      ...
      notify.error(err.response?.data?.error || 'File upload failed');
      return null;
    } finally {
      setUploading(false);
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.version.trim()) { notify.error('Version is required'); return; }
    if (!form.software_id) { notify.error('Select a software product'); return; }
    ...
    try {
      ...
      if (update) {
        ...
        notify.success('Update modified');
      } else {
        await api.post('/updates/updates', payload);
        notify.success('Update created');
      }
      onSaved();
      onClose();
    } catch (err: any) {
      notify.error(err.response?.data?.error || 'Failed to save update');
    } finally {
      setLoading(false);
    }
  };
              ...
              <>
                <div>
                  ...
                  <RichTextEditor
                    ...
                    onChange={(val) => setForm({ ...form, migration_notes: val })}
...
const UpdatesAdmin: React.FC = () => {
  ...
  const loadUpdates = useCallback(async () => {
    ...
    try {
      ...
      notify.error('Failed to load updates');
    } finally {
      setLoading(false);
    }
  }, []);
      ...
      {loading && updates.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {filteredUpdates.map(update => (
            <div key={update.id}
              className="bg-white rounded-xl border shadow-sm hover:shadow-lg transition-all flex flex-col overflow-hidden group">
              {/* Card header — version + software */}
              <div className="px-5 pt-5 pb-3">
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg bg-gradient-to-r from-indigo-500 to-purple-600 text-white text-sm font-bold shadow-sm">
                      v{update.version}
                    </span>
                    {update.has_migrations === 1 && (
                      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-amber-50 text-amber-700 text-xs font-medium border border-amber-200">
                        <CircleStackIcon className="h-3 w-3" /> Migrations
                      </span>
                    )}
                  </div>
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => { setEditingUpdate(update); setDialogOpen(true); }}
                      title="Edit" className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-indigo-600 transition">
                      <PencilIcon className="h-4 w-4" />
                    </button>
                    <button onClick={() => handleDelete(update)}
                      title="Delete" className="p-1.5 rounded-lg hover:bg-red-50 text-gray-400 hover:text-red-500 transition">
                ...
                {update.software_name && (
                  <div className="mt-2 flex items-center gap-1.5 text-xs text-gray-500">
                    <CubeIcon className="h-3.5 w-3.5 text-blue-500" />
                    <span className="font-medium text-gray-700">{update.software_name}</span>
                  </div>
                )}
              </div>

              {/* Card body — description rendered as HTML */}
              {update.description && (
                <div className="px-5 pb-3 flex-1">
                  <div
                    className="prose prose-sm max-w-none text-gray-600 line-clamp-4 [&_p]:my-1 [&_ul]:my-1 [&_ol]:my-1 [&_li]:my-0 [&_h1]:text-base [&_h2]:text-sm [&_h3]:text-sm [&_img]:max-w-full [&_img]:rounded"
                    dangerouslySetInnerHTML={{ __html: update.description }}
                  />
                </div>
              )}

              {/* Migration notes rendered as HTML */}
              {update.migration_notes && (
                <div className="mx-5 mb-3 p-2.5 bg-amber-50 rounded-lg border border-amber-100">
                  <p className="text-xs font-semibold text-amber-700 mb-1 flex items-center gap-1">
                    <CircleStackIcon className="h-3 w-3" /> Migration Notes
                  </p>
                  <div
                    className="prose prose-xs max-w-none text-amber-800 [&_p]:my-0.5 [&_ul]:my-0.5 [&_li]:my-0 text-xs"
                    dangerouslySetInnerHTML={{ __html: update.migration_notes }}
                  />
                </div>
              )}

              {/* Card footer — metadata */}
              <div className="mt-auto border-t bg-gray-50/80 px-5 py-3">
                ...
                {update.uploaded_by_name && (
                  <p className="text-xs text-gray-400 mt-1">Uploaded by {update.uploaded_by_name}</p>
                )}
              </div>
            </div>
          ))}
