...
const Updates: React.FC = () => {
  ...
  const checkForUpdates = async () => {
    ...
    try {
      ...
      notify.error(error.response?.data?.message || 'Failed to check for updates');
    } finally {
      setChecking(false);
    }
  };
  ...
  const handleInstallUpdate = async () => {
    ...
    if (result.isConfirmed) {
      ...
      try {
        ...
        notify.error(error.response?.data?.message || 'Failed to install update');
      } finally {
        setInstalling(false);
      }
    }
  ...
  const handleRollback = async (update: InstalledUpdate) => {
    ...
    if (result.isConfirmed) {
      ...
      try {
        ...
        notify.error(error.response?.data?.message || 'Failed to rollback');
      } finally {
        setInstalling(false);
      }
    }
