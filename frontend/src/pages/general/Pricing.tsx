...
import { PlusIcon, PencilIcon, TrashIcon, TagIcon, CurrencyDollarIcon, CubeIcon, ArrowUpTrayIcon } from '@heroicons/react/24/outline';
...
const Pricing: React.FC = () => {
  ...
  const columns: any[] = [
    ...
    },
    {
      ...
      cell: ({ row }: any) => (
        <div>
          <div className="font-medium text-gray-900">{row.original.pricing_item}</div>
          {row.original.pricing_note && (
            <div className="text-xs text-gray-500 mt-0.5">{row.original.pricing_note}</div>
          )}
        </div>
      )
    },
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file.name.match(/\.(xlsx|xls)$/i)) {
      notify.error('Please select an Excel file (.xlsx or .xls)');
      return;
    }
    ...
    try {
      ...
      const result = await PricingModel.import(formData);
      
      notify.success(`Import complete: ${result.created || 0} created, ${result.updated || 0} updated, ${result.categories_created || 0} categories created`);
      
      // Reload data
      loadData();
    } catch (error: any) {
      console.error('Error importing pricing:', error);
      notify.error(error.response?.data?.error || 'Failed to import pricing data');
    } finally {
      setImporting(false);
      // Reset file input
          ...
          <div className="flex items-center gap-3">
            ...
            <Can permission="pricing.create">
              ...
              <button
                ...
                disabled={importing}
                className="inline-flex items-center px-5 py-2.5 rounded-lg text-sm font-semibold text-white bg-white/20 hover:bg-white/30 border border-white/30 shadow-md transition-all hover:shadow-lg disabled:opacity-50"
              >
                {importing ? (
                  <>
