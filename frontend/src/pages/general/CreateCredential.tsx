...
export const CreateCredential: React.FC = () => {
  ...
  const loadCredential = async (credentialId: number) => {
    try {
      ...
      notify.error(error.message || 'Failed to load credential');
    ...
    try {
      ...
      if (additionalDataJson.trim()) {
        try {
          ...
          notify.error('Invalid JSON in Additional Data field');
          return;
        }
      }
      ...
      notify.error(error.message || `Failed to ${isEditMode ? 'update' : 'create'} credential`);
    }
  };
