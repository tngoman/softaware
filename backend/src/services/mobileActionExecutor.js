...
import { createEndpoint, } from './enterpriseEndpoints.js';
 ...
 */
export async function executeMobileAction(toolCall, ctx) {
...
async function requireStaff(ctx, fn) {
    if (ctx.role !== 'staff') {
 ...
 */
async function verifyAssistantOwnership(assistantId, ctx) {
    const row = await db.queryOne('SELECT id, userId FROM assistants WHERE id = ?', [assistantId]);
    if (!row)
 ...
 * Staff bypass ownership checks.
 */
async function verifyJobOwnership(jobId, ctx) {
    const row = await db.queryOne('SELECT ij.id, ij.assistant_id FROM ingestion_jobs ij WHERE ij.id = ?', [jobId]);
    if (!row)
        return { owned: false, exists: false };
    if (ctx.role === 'staff')
        return { owned: true, exists: true, assistantId: row.assistant_id };
...
// ============================================================================
async function execListMyAssistants(ctx) {
    const assistants = await db.query(`SELECT id, name, status, tier, pages_indexed
     FROM assistants
     WHERE userId = ?
    ...
    const lines = assistants.map((a, i) => `${i + 1}. **${a.name}** — ${a.status} (${a.tier} tier, ${a.pages_indexed} pages indexed)\n   ID: ${a.id}`);
    return {
        success: true,
...
async function execToggleAssistantStatus(args, ctx) {
    const assistantId = String(args.assistantId || '');
    const status = String(args.status || '');
    if (!assistantId)
        return { success: false, message: 'Please provide the assistant ID.' };
    ...
    if (!check.owned)
        return { success: false, message: 'You do not have permission to modify this assistant.' };
    await db.execute('UPDATE assistants SET status = ?, updated_at = ? WHERE id = ?', [status, toMySQLDate(new Date()), assistantId]);
    const action = status === 'active' ? 'activated' : 'suspended';
    return { success: true, message: `The assistant has been successfully ${action}.` };
}
async function execGetUsageStats(ctx) {
    // Find the user's team subscription
    const membership = await db.queryOne('SELECT teamId FROM team_members WHERE userId = ? LIMIT 1', [ctx.userId]);
    let tier = 'FREE';
    let messageLimitMonthly = 500;
    let pageLimit = 50;
    let assistantLimit = 5;
    if (membership) {
        const sub = await db.queryOne(`SELECT sp.tier, sp.maxAgents
       FROM subscriptions s
       JOIN subscription_plans sp ON s.planId = sp.id
       WHERE s.teamId = ? AND s.status IN ('TRIAL','ACTIVE')
       ORDER BY s.createdAt DESC LIMIT 1`, [membership.teamId]);
        if (sub) {
            tier = sub.tier;
    ...
    const assistantCount = await db.queryOne('SELECT COUNT(*) as cnt FROM assistants WHERE userId = ?', [ctx.userId]);
    const pagesIndexed = await db.queryOne(`SELECT COALESCE(SUM(a.pages_indexed), 0) as cnt
     FROM assistants a WHERE a.userId = ?`, [ctx.userId]);
    const summary = [
        `📊 **Your Usage Summary**`,
        `Plan: ${tier}`,
...
async function execListFailedJobs(ctx) {
    const jobs = await db.query(`SELECT ij.id, ij.source, ij.job_type, ij.error_message, ij.assistant_id,
    ...
    const lines = jobs.map((j, i) => `${i + 1}. **${j.source}** (${j.job_type}) — ${j.error_message || 'Unknown error'}\n   Job ID: ${j.id} | Retries: ${j.retry_count}`);
    return {
        success: true,
...
async function execRetryFailedIngestion(args, ctx) {
    const jobId = String(args.jobId || '');
    if (!jobId)
        return { success: false, message: 'Please provide the ingestion job ID.' };
    // SECURITY CHECK: ownership
    const check = await verifyJobOwnership(jobId, ctx);
    if (!check.exists)
        return { success: false, message: 'Ingestion job not found.' };
    if (!check.owned)
        return { success: false, message: 'You do not have permission to retry this job.' };
    // Verify the job is actually failed
    const job = await db.queryOne('SELECT status, retry_count FROM ingestion_jobs WHERE id = ?', [jobId]);
    if (!job)
    ...
    await db.execute(`UPDATE ingestion_jobs SET status = 'pending', retry_count = 0, error_message = NULL, updated_at = ? WHERE id = ?`, [toMySQLDate(new Date()), jobId]);
    return {
        success: true,
 ...
 */
async function resolveTaskSourceForTools(softwareId) {
    if (softwareId) {
        const source = await db.queryOne(`SELECT id, base_url, api_key, software_id, name
       FROM task_sources
       WHERE software_id = ? AND sync_enabled = 1 AND api_key IS NOT NULL AND api_key != ''
       ORDER BY id ASC LIMIT 1`, [softwareId]);
        if (source)
            return source;
    }
    // Fall back to first enabled source with an API key
    return db.queryOne(`SELECT id, base_url, api_key, software_id, name
     FROM task_sources
 ...
 */
async function resolveLocalTask(taskId) {
    // Try by local id first
    let task = await db.queryOne(`SELECT t.*, s.name as source_name, s.source_type
     FROM local_tasks t
     LEFT JOIN task_sources s ON s.id = t.source_id
     WHERE t.id = ? AND t.task_deleted = 0`, [taskId]);
    if (task)
        return task;
    // Try by external_id
    task = await db.queryOne(`SELECT t.*, s.name as source_name, s.source_type
     FROM local_tasks t
     LEFT JOIN task_sources s ON s.id = t.source_id
     WHERE t.external_id = ? AND t.task_deleted = 0`, [taskId]);
    return task;
}
// ============================================================================
// Staff Task Tool Executors (v2.0 — Local DB reads, proxy writes)
// ============================================================================
async function execListTasks(args, ctx) {
    ...
    if (String(args.assignedToMe) === 'true') {
        const user = await db.queryOne('SELECT name, email FROM users WHERE id = ?', [ctx.userId]);
        if (user) {
    ...
    // Fetch tasks
    const tasks = await db.query(`SELECT t.*, s.name as source_name
     FROM local_tasks t
     LEFT JOIN task_sources s ON s.id = t.source_id
     ${where}
    ...
    const display = tasks.map((t, i) => {
        ...
        const tags = t.local_tags ? (() => { try {
            const arr = typeof t.local_tags === 'string' ? JSON.parse(t.local_tags) : t.local_tags;
...
async function execGetTask(args) {
    ...
    const tags = task.local_tags ? (() => { try {
        const arr = typeof task.local_tags === 'string' ? JSON.parse(task.local_tags) : task.local_tags;
        return arr.length ? arr.join(', ') : '';
    }
    catch {
        return '';
...
async function execCreateTask(args, ctx) {
    ...
    if (args.workflow_phase)
        taskPayload.workflow_phase = String(args.workflow_phase);
    if (args.assigned_to)
        taskPayload.assigned_to = parseInt(String(args.assigned_to), 10);
...
async function execUpdateTask(args, ctx) {
    const taskId = String(args.taskId || '').trim();
    if (!taskId)
        return { success: false, message: 'Task ID is required.' };
    ...
    if (localUpdates.length > 0) {
        ...
        await db.execute(`UPDATE local_tasks SET ${localUpdates.join(', ')} WHERE id = ?`, localValues);
    }
        ...
        if (source) {
            ...
            if (externalChanges.assigned_to) {
                createNotificationWithPush(String(externalChanges.assigned_to), {
                    ...
                    message: `${(await db.queryOne('SELECT name FROM users WHERE id = ?', [ctx.userId]))?.name || 'Someone'} assigned you: ${localTask.title}`,
                    type: 'info',
...
async function execDeleteTask(args) {
    const taskId = String(args.taskId || '').trim();
    if (!taskId)
        return { success: false, message: 'Task ID is required.' };
    const task = await resolveLocalTask(taskId);
    if (!task)
        return { success: false, message: `Task ${taskId} not found.` };
    await db.execute('UPDATE local_tasks SET task_deleted = 1, local_dirty = 1, updated_at = NOW() WHERE id = ?', [task.id]);
    return {
        success: true,
...
async function execGetTaskComments(args) {
    const taskId = String(args.taskId || '').trim();
    if (!taskId)
        return { success: false, message: 'Task ID is required.' };
    ...
    const display = comments.slice(0, 20).map((c, i) => {
...
async function execAddTaskComment(args, ctx) {
    const taskId = String(args.taskId || '').trim();
    ...
    const result = await taskProxyV2(source.base_url, `/api/tasks-api/${externalId}/comments`, 'POST', source.api_key, commentPayload);
    if (result.status >= 400) {
...
async function execBookmarkTask(args) {
    const taskId = String(args.taskId || '').trim();
    if (!taskId)
        return { success: false, message: 'Task ID is required.' };
    const task = await resolveLocalTask(taskId);
...
async function execSetTaskPriority(args) {
    const taskId = String(args.taskId || '').trim();
...
async function execSetTaskColor(args) {
    const taskId = String(args.taskId || '').trim();
    if (!taskId)
        return { success: false, message: 'Task ID is required.' };
    const task = await resolveLocalTask(taskId);
...
async function execSetTaskTags(args) {
    const taskId = String(args.taskId || '').trim();
    if (!taskId)
        return { success: false, message: 'Task ID is required.' };
    const task = await resolveLocalTask(taskId);
...
async function execStartTask(args) {
    const taskId = String(args.taskId || '').trim();
    if (!taskId)
        return { success: false, message: 'Task ID is required.' };
    const localTask = await resolveLocalTask(taskId);
...
async function execCompleteTask(args) {
    const taskId = String(args.taskId || '').trim();
    if (!taskId)
        return { success: false, message: 'Task ID is required.' };
    const localTask = await resolveLocalTask(taskId);
...
async function execApproveTask(args) {
    const taskId = String(args.taskId || '').trim();
    if (!taskId)
        return { success: false, message: 'Task ID is required.' };
    const localTask = await resolveLocalTask(taskId);
...
async function execGetTaskStats() {
    // Get stats from local database
    const statusCounts = await db.query(`SELECT status, COUNT(*) as count
     FROM local_tasks
     WHERE task_deleted = 0
     GROUP BY status`);
    const typeCounts = await db.query(`SELECT type, COUNT(*) as count
     FROM local_tasks
     WHERE task_deleted = 0
     GROUP BY type`);
    const phaseCounts = await db.query(`SELECT workflow_phase, COUNT(*) as count
     FROM local_tasks
     WHERE task_deleted = 0 AND workflow_phase IS NOT NULL AND workflow_phase != ''
     GROUP BY workflow_phase`);
    const [totalRow] = await db.query('SELECT COUNT(*) as total FROM local_tasks WHERE task_deleted = 0');
    const [bookmarkedRow] = await db.query('SELECT COUNT(*) as count FROM local_tasks WHERE task_deleted = 0 AND is_bookmarked = 1');
    const total = totalRow?.total || 0;
    const bookmarked = bookmarkedRow?.count || 0;
    const statusLines = statusCounts.map((s) => `  • ${s.status === 'progress' ? 'in-progress' : s.status}: ${s.count}`).join('\n');
    const typeLines = typeCounts.map((t) => `  • ${t.type}: ${t.count}`).join('\n');
    const phaseLines = phaseCounts.length > 0
        ? phaseCounts.map((p) => `  • ${p.workflow_phase}: ${p.count}`).join('\n')
        : '  (none)';
    return {
        success: true,
...
async function execGetPendingApprovals() {
    ...
    const display = tasks.slice(0, 15).map((t, i) => {
        const assignee = t.assigned_to_name ? ` → ${t.assigned_to_name}` : '';
...
async function execGetTaskTags() {
...
async function execGetSyncStatus() {
...
async function execStageTasksForInvoice(args) {
    const rawIds = String(args.task_ids || '').trim();
    if (!rawIds)
        return { success: false, message: 'task_ids is required (comma-separated external IDs, e.g. "42,43,44").' };
    ...
    const affected = await db.execute(`UPDATE local_tasks SET task_billed = 2, task_bill_date = ? WHERE external_id IN (${placeholders}) AND task_billed = 0`, [billDate, ...taskIds]);
    return {
        success: true,
...
async function execGetStagedInvoices() {
    const tasks = await db.query(`SELECT id, external_id, title, hours, estimated_hours, task_bill_date, assigned_to_name, type
...
async function execProcessStagedInvoices() {
    ...
    const result = await taskProxyV2(source.base_url, '/api/tasks-api/invoice-tasks', 'POST', source.api_key, { task_ids: externalIds, bill_date: billDate });
    if (result.status >= 400) {
...
// ============================================================================
async function execSearchClients(args) {
    const query = String(args.query || '').trim();
    if (!query || query.length < 2) {
        return { success: false, message: 'Please provide at least 2 characters to search.' };
    }
    const clients = await db.query(`SELECT id, email, name, account_status FROM users
    ...
    const lines = clients.map((c, i) => `${i + 1}. **${c.name || '(no name)'}** — ${c.email} [${c.account_status}]\n   ID: ${c.id}`);
    return {
        success: true,
...
async function execSuspendClientAccount(args, ctx) {
    const clientId = String(args.clientId || '');
    const status = String(args.status || '');
    const reason = String(args.reason || 'No reason provided');
    if (!clientId)
        return { success: false, message: 'Please provide the client ID.' };
    ...
    const target = await db.queryOne('SELECT id, email, name, account_status FROM users WHERE id = ?', [clientId]);
    if (!target)
        return { success: false, message: 'User not found.' };
    // Prevent suspending other staff members
    const staffCheck = await db.queryOne('SELECT is_admin, is_staff FROM users WHERE id = ?', [clientId]);
    ...
    await db.execute('UPDATE users SET account_status = ? WHERE id = ?', [status, clientId]);
    // Log the action
    console.log(`[MobileAction] Account status change: ${clientId} → ${status} by staff ${ctx.userId}. Reason: ${reason}`);
...
async function execCheckClientHealth(args) {
    const clientId = String(args.clientId || '');
    if (!clientId)
        return { success: false, message: 'Please provide the client ID.' };
    const user = await db.queryOne('SELECT id, email, name, account_status FROM users WHERE id = ?', [clientId]);
    if (!user)
        return { success: false, message: 'User not found.' };
    // Assistants
    const assistants = await db.query('SELECT id, name, status, tier, pages_indexed FROM assistants WHERE userId = ? ORDER BY created_at DESC', [clientId]);
    // Pending/failed jobs
    const queueStats = await db.queryOne(`SELECT
    ...
    for (const a of assistants.slice(0, 5)) {
        try {
            ...
            healthLines.push(`  • **${a.name}** [${a.status}] — Health: ${health.score}%, ${a.pages_indexed} pages (${a.tier})`);
        }
        catch {
...
async function execGenerateEnterpriseEndpoint(args) {
    const clientId = String(args.clientId || '');
    ...
    const user = await db.queryOne('SELECT id, email, name FROM users WHERE id = ?', [clientId]);
    if (!user)
 ...
 */
async function verifySiteOwnership(siteId, ctx) {
    const site = await db.queryOne('SELECT id, user_id FROM generated_sites WHERE id = ?', [siteId]);
    if (!site)
        return { owned: false, exists: false };
    if (ctx.role === 'staff')
        return { owned: true, exists: true };
 ...
 */
async function getUserSiteIds(userId) {
    const sites = await db.query('SELECT id FROM generated_sites WHERE user_id = ?', [userId]);
    return sites.map(s => s.id);
}
async function execListLeads(args, ctx) {
    ...
    const lines = leads.map((l, i) => `${i + 1}. **${l.sender_name}** (${l.sender_email})${l.sender_phone ? ` — ${l.sender_phone}` : ''}\n   Status: ${l.status} | ${new Date(l.created_at).toLocaleDateString()}\n   ID: ${l.id}`);
    return {
        success: true,
...
async function execGetLeadDetails(args, ctx) {
    const leadId = String(args.leadId || '').trim();
    if (!leadId)
        return { success: false, message: 'Please provide the lead ID.' };
    const lead = await db.queryOne('SELECT * FROM form_submissions WHERE id = ?', [leadId]);
    if (!lead)
        return { success: false, message: 'Lead not found.' };
    // Verify ownership
    const check = await verifySiteOwnership(lead.site_id, ctx);
    if (!check.owned)
        return { success: false, message: 'You do not have permission to view this lead.' };
    const details = [
        `📋 **Lead Details**`,
...
async function execUpdateLeadStatus(args, ctx) {
    const leadId = String(args.leadId || '').trim();
    ...
    const lead = await db.queryOne('SELECT id, site_id FROM form_submissions WHERE id = ?', [leadId]);
    if (!lead)
        return { success: false, message: 'Lead not found.' };
    const check = await verifySiteOwnership(lead.site_id, ctx);
    if (!check.owned)
        return { success: false, message: 'You do not have permission to update this lead.' };
...
async function execGetLeadStats(ctx) {
    ...
    const recentCount = await db.queryOne(`SELECT COUNT(*) as cnt FROM form_submissions
     WHERE site_id IN (${placeholders}) AND created_at >= DATE_SUB(NOW(), INTERVAL 7 DAY)`, [...siteIds]);
...
// ============================================================================
async function execSendFollowupEmail(args, ctx) {
    const leadId = String(args.leadId || '').trim();
    ...
    if (!leadId)
        return { success: false, message: 'Please provide the lead ID.' };
    if (!subject)
        return { success: false, message: 'Please provide an email subject.' };
    if (!body)
        return { success: false, message: 'Please provide the email body.' };
    // Get the lead
    const lead = await db.queryOne('SELECT id, site_id, sender_name, sender_email FROM form_submissions WHERE id = ?', [leadId]);
    if (!lead)
        return { success: false, message: 'Lead not found.' };
    // Verify ownership
    const check = await verifySiteOwnership(lead.site_id, ctx);
    if (!check.owned)
        return { success: false, message: 'You do not have permission to email this lead.' };
    // Get the site owner's info for the "from" address context
    const user = await db.queryOne('SELECT email, name FROM users WHERE id = ?', [ctx.userId]);
    ...
    await db.execute(`UPDATE form_submissions SET status = 'contacted', updated_at = ? WHERE id = ? AND status = 'new'`, [toMySQLDate(new Date()), leadId]);
    return {
        success: true,
        message: `✅ Follow-up email sent to **${lead.sender_name}** (${lead.sender_email}).\nSubject: ${subject}`,
    };
}
async function execSendInfoEmail(args, ctx) {
    const toEmail = String(args.toEmail || '').trim();
    const subject = String(args.subject || '').trim();
    const body = String(args.body || '').trim();
    if (!toEmail)
        return { success: false, message: 'Please provide a recipient email address.' };
    if (!subject)
        return { success: false, message: 'Please provide an email subject.' };
    if (!body)
        return { success: false, message: 'Please provide the email body.' };
...
// ============================================================================
async function execListMySites(ctx) {
    ...
    const lines = sites.map((s, i) => `${i + 1}. **${s.business_name}** — ${s.status}${s.last_deployed_at ? ` (deployed ${new Date(s.last_deployed_at).toLocaleDateString()})` : ''}\n   ID: ${s.id}`);
    return {
        success: true,
...
async function execGetSiteDetails(args, ctx) {
    const siteId = String(args.siteId || '').trim();
    if (!siteId)
        return { success: false, message: 'Please provide the site ID.' };
    const check = await verifySiteOwnership(siteId, ctx);
    if (!check.exists)
        return { success: false, message: 'Site not found.' };
    if (!check.owned)
        return { success: false, message: 'You do not have permission to view this site.' };
...
async function execUpdateSiteField(args, ctx) {
    const siteId = String(args.siteId || '').trim();
    ...
    if (!siteId)
        return { success: false, message: 'Please provide the site ID.' };
    if (!field)
        return { success: false, message: 'Please specify which field to update.' };
    const allowedFields = {
...
async function execRegenerateSite(args, ctx) {
    const siteId = String(args.siteId || '').trim();
    if (!siteId)
        return { success: false, message: 'Please provide the site ID.' };
    const check = await verifySiteOwnership(siteId, ctx);
    if (!check.exists)
        return { success: false, message: 'Site not found.' };
    if (!check.owned)
        return { success: false, message: 'You do not have permission to regenerate this site.' };
    try {
...
async function execDeploySite(args, ctx) {
    const siteId = String(args.siteId || '').trim();
    if (!siteId)
        return { success: false, message: 'Please provide the site ID.' };
    const check = await verifySiteOwnership(siteId, ctx);
    if (!check.exists)
        return { success: false, message: 'Site not found.' };
    if (!check.owned)
        return { success: false, message: 'You do not have permission to deploy this site.' };
    const site = await siteBuilderService.getSiteById(siteId);
    if (!site)
        return { success: false, message: 'Site not found.' };
...
async function execGetSiteDeployments(args, ctx) {
    const siteId = String(args.siteId || '').trim();
    if (!siteId)
        return { success: false, message: 'Please provide the site ID.' };
    const check = await verifySiteOwnership(siteId, ctx);
    ...
    if (!check.owned)
        return { success: false, message: 'You do not have permission to view this site.' };
    const deployments = await db.query(`SELECT id, status, deployed_at, error_message, files_deployed
...
// ============================================================================
async function execListCases(args) {
    const limit = Math.min(parseInt(String(args.limit || '20'), 10) || 20, 50);
...
async function execGetCaseDetails(args) {
    const caseId = String(args.caseId || '').trim();
    if (!caseId)
        return { success: false, message: 'Please provide the case ID.' };
    const c = await db.queryOne('SELECT * FROM cases WHERE id = ?', [caseId]);
    if (!c)
        return { success: false, message: 'Case not found.' };
    // Get recent comments
    const comments = await db.query(`SELECT cc.id, cc.comment, cc.is_internal, cc.created_at, cc.user_id
     FROM case_comments cc WHERE cc.case_id = ?
     ORDER BY cc.created_at DESC LIMIT 5`, [caseId]);
    const commentLines = comments.map((cm) => `  • ${cm.is_internal ? '[Internal]' : '[Public]'} ${cm.comment.substring(0, 100)}${cm.comment.length > 100 ? '...' : ''} (${new Date(cm.created_at).toLocaleDateString()})`);
    const details = [
...
async function execUpdateCase(args, ctx) {
    const caseId = String(args.caseId || '').trim();
    if (!caseId)
        return { success: false, message: 'Please provide the case ID.' };
    const existing = await db.queryOne('SELECT id FROM cases WHERE id = ?', [caseId]);
    if (!existing)
        return { success: false, message: 'Case not found.' };
    const updates = [];
    const values = [];
...
async function execAddCaseComment(args, ctx) {
    const caseId = String(args.caseId || '').trim();
    const comment = String(args.comment || '').trim();
    if (!caseId)
        return { success: false, message: 'Please provide the case ID.' };
    if (!comment)
        return { success: false, message: 'Please provide the comment text.' };
    const existing = await db.queryOne('SELECT id FROM cases WHERE id = ?', [caseId]);
    if (!existing)
        return { success: false, message: 'Case not found.' };
    const isInternal = String(args.isInternal || 'true') === 'true' ? 1 : 0;
    await db.execute(`INSERT INTO case_comments (id, case_id, user_id, comment, is_internal, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?)`, [randomUUID(), caseId, ctx.userId, comment, isInternal, toMySQLDate(new Date()), toMySQLDate(new Date())]);
    return {
        success: true,
...
// ============================================================================
async function execListContacts(args) {
    const limit = Math.min(parseInt(String(args.limit || '20'), 10) || 20, 50);
...
async function execGetContactDetails(args) {
    const contactId = String(args.contactId || '').trim();
    if (!contactId)
        return { success: false, message: 'Please provide the contact ID.' };
    const c = await db.queryOne('SELECT * FROM contacts WHERE id = ?', [contactId]);
    if (!c)
...
async function execCreateContact(args) {
    ...
    await db.execute(`INSERT INTO contacts (company_name, contact_person, email, phone, location, vat_number, contact_type, active, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, 1, NOW(), NOW())`, [
        companyName,
...
// ============================================================================
async function execListQuotations(args) {
    const limit = Math.min(parseInt(String(args.limit || '20'), 10) || 20, 50);
    ...
    const lines = quotations.map((q, i) => `${i + 1}. **${q.quotation_number}** — R${Number(q.quotation_amount).toFixed(2)}\n   ${q.company_name || 'No contact'} | ${new Date(q.quotation_date).toLocaleDateString()}\n   ID: ${q.id}`);
    return {
        success: true,
...
async function execGetQuotationDetails(args) {
    const quotationId = String(args.quotationId || '').trim();
    if (!quotationId)
        return { success: false, message: 'Please provide the quotation ID.' };
    const q = await db.queryOne('SELECT * FROM quotations WHERE id = ? AND active = 1', [quotationId]);
    if (!q)
        return { success: false, message: 'Quotation not found.' };
    // Get contact info
    let contactName = 'Unknown';
    if (q.contact_id) {
        const c = await db.queryOne('SELECT company_name FROM contacts WHERE id = ?', [q.contact_id]);
        if (c)
            contactName = c.company_name;
    }
    // Get line items
    const items = await db.query('SELECT * FROM quote_items WHERE quotation_id = ?', [quotationId]);
    const itemLines = items.map((it, i) => `  ${i + 1}. ${it.item_description} — R${Number(it.item_price).toFixed(2)} × ${it.item_quantity}${it.item_discount ? ` (-${it.item_discount}%)` : ''} = R${Number(it.line_total).toFixed(2)}`);
    const details = [
...
async function execListInvoices(args) {
    const limit = Math.min(parseInt(String(args.limit || '20'), 10) || 20, 50);
...
async function execGetInvoiceDetails(args) {
    const invoiceId = String(args.invoiceId || '').trim();
    if (!invoiceId)
        return { success: false, message: 'Please provide the invoice ID.' };
    const inv = await db.queryOne('SELECT * FROM invoices WHERE id = ? AND active = 1', [invoiceId]);
    if (!inv)
        return { success: false, message: 'Invoice not found.' };
    // Get contact info
    let contactName = 'Unknown';
    if (inv.contact_id) {
        const c = await db.queryOne('SELECT company_name FROM contacts WHERE id = ?', [inv.contact_id]);
        if (c)
            contactName = c.company_name;
    }
    // Get line items
...
async function execSearchPricing(args) {
    ...
    const lines = items.map((p, i) => `${i + 1}. **${p.item_name}** — R${Number(p.unit_price).toFixed(2)}${p.description ? `\n   ${p.description.substring(0, 100)}` : ''}`);
    return {
        success: true,
...
// ============================================================================
async function execListScheduledCalls(args, ctx) {
...
async function execCreateScheduledCall(args, ctx) {
    ...
    if (!title)
        return { success: false, message: 'Please provide a title for the call.' };
    if (!scheduledAt)
        return { success: false, message: 'Please provide the date and time for the call.' };
    // Validate the conversation exists
    const conv = await db.queryOne('SELECT id FROM conversations WHERE id = ?', [conversationId]);
    if (!conv)
    ...
    await db.execute(`INSERT INTO scheduled_calls
      (conversation_id, created_by, title, call_type, screen_share, scheduled_at, duration_minutes, status, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, 'scheduled', NOW(), NOW())`, [conversationId, ctx.userId, title, callType, screenShare, scheduledAt, duration]);
    return {
        success: true,
...
// ============================================================================
async function execListConversations(args, ctx) {
    const limit = Math.min(parseInt(String(args.limit || '20'), 10) || 20, 50);
...
async function execSendChatMessage(args, ctx) {
    const conversationId = String(args.conversationId || '').trim();
    const content = String(args.content || '').trim();
    if (!conversationId)
        return { success: false, message: 'Please provide a conversation ID.' };
    if (!content)
        return { success: false, message: 'Please provide message content.' };
    // Verify conversation exists and user is a member
    const membership = await db.queryOne('SELECT conversation_id FROM conversation_members WHERE conversation_id = ? AND user_id = ?', [conversationId, ctx.userId]);
    if (!membership) {
        return { success: false, message: 'You are not a member of this conversation.' };
    }
    await db.execute(`INSERT INTO messages (conversation_id, sender_id, content, message_type, created_at, updated_at)
     VALUES (?, ?, ?, 'text', NOW(), NOW())`, [conversationId, ctx.userId, content]);
    return {
        success: true,
...
// ============================================================================
async function execListBugs(args) {
    const limit = Math.min(parseInt(String(args.limit || '20'), 10) || 20, 50);
    ...
    if (phase) {
        sql += ' AND b.workflow_phase = ?';
        params.push(phase);
    }
    const softwareId = String(args.software_id || '').trim();
    if (softwareId) {
        sql += ' AND b.software_id = ?';
    ...
    params.push(limit);
    const bugs = await db.query(sql, params);
    if (bugs.length === 0) {
...
async function execGetBugDetails(args) {
    const bugId = String(args.bugId || '').trim();
    if (!bugId)
        return { success: false, message: 'Please provide the bug ID.' };
    const bug = await db.queryOne('SELECT * FROM bugs WHERE id = ?', [bugId]);
    if (!bug)
        return { success: false, message: `Bug #${bugId} not found.` };
    const comments = await db.query('SELECT id, author_name, content, comment_type, is_internal, created_at FROM bug_comments WHERE bug_id = ? ORDER BY created_at ASC LIMIT 20', [bugId]);
    const attachmentCount = await db.queryOne('SELECT COUNT(*) AS cnt FROM bug_attachments WHERE bug_id = ?', [bugId]);
    const severityIcon = { critical: '🔴', high: '🟠', medium: '🟡', low: '🟢' };
    ...
    if (bug.assigned_to_name)
        msg += `**Assigned to:** ${bug.assigned_to_name}\n`;
    if (bug.software_name)
        msg += `**Software:** ${bug.software_name}\n`;
    msg += `**Created:** ${new Date(bug.created_at).toLocaleString()}\n`;
    if (bug.description)
        msg += `\n**Description:**\n${bug.description.replace(/<[^>]+>/g, '')}\n`;
    if (bug.current_behaviour)
        msg += `\n**Current Behaviour:**\n${bug.current_behaviour.replace(/<[^>]+>/g, '')}\n`;
    if (bug.expected_behaviour)
        msg += `\n**Expected Behaviour:**\n${bug.expected_behaviour.replace(/<[^>]+>/g, '')}\n`;
    if (bug.resolution_notes)
        msg += `\n**Resolution:** ${bug.resolution_notes}\n`;
    if (bug.resolved_by)
        msg += `**Resolved by:** ${bug.resolved_by} at ${bug.resolved_at ? new Date(bug.resolved_at).toLocaleString() : 'N/A'}\n`;
...
async function execCreateBug(args, ctx) {
    const title = String(args.title || '').trim();
    const reporter_name = String(args.reporter_name || '').trim();
    if (!title)
        return { success: false, message: 'Please provide a title for the bug.' };
    if (!reporter_name)
        return { success: false, message: 'Please provide the reporter name.' };
    ...
    const userRow = await db.queryOne('SELECT name FROM users WHERE id = ?', [ctx.userId]);
    if (userRow)
        created_by_name = userRow.name;
    const result = await db.execute(`INSERT INTO bugs
     ...
     VALUES (?, ?, ?, ?, ?, ?, ?, 'open', ?, 'intake', ?, ?, ?, ?, NOW(), NOW())`, [title, description, current_behaviour, expected_behaviour, reporter_name,
    ...
    await db.execute(`INSERT INTO bug_comments (bug_id, author_name, author_id, content, is_internal, comment_type, created_at, updated_at)
     VALUES (?, ?, ?, ?, 0, 'status_change', NOW(), NOW())`, [insertId, 'System', ctx.userId, `Bug reported and entered Intake phase.`]);
    return {
        success: true,
...
async function execUpdateBug(args, ctx) {
    const bugId = String(args.bugId || '').trim();
    if (!bugId)
        return { success: false, message: 'Please provide the bug ID.' };
    // Verify bug exists
    const existing = await db.queryOne('SELECT id, status, assigned_to FROM bugs WHERE id = ?', [bugId]);
    if (!existing)
        return { success: false, message: `Bug #${bugId} not found.` };
    const allowed = [
...
async function execAddBugComment(args, ctx) {
    const bugId = String(args.bugId || '').trim();
    const content = String(args.content || '').trim();
    if (!bugId)
        return { success: false, message: 'Please provide the bug ID.' };
    if (!content)
        return { success: false, message: 'Please provide comment content.' };
    // Verify bug exists
    const bug = await db.queryOne('SELECT id FROM bugs WHERE id = ?', [bugId]);
    if (!bug)
        return { success: false, message: `Bug #${bugId} not found.` };
    const isInternal = String(args.is_internal || 'true') === 'true' ? 1 : 0;
    ...
    await db.execute(`INSERT INTO bug_comments (bug_id, author_name, author_id, content, is_internal, comment_type, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, 'comment', NOW(), NOW())`, [bugId, authorName, ctx.userId, content, isInternal]);
...
async function execUpdateBugWorkflow(args, ctx) {
    const bugId = String(args.bugId || '').trim();
    ...
    const bug = await db.queryOne('SELECT id, workflow_phase FROM bugs WHERE id = ?', [bugId]);
    if (!bug)
        return { success: false, message: `Bug #${bugId} not found.` };
    ...
    if (userRow)
        authorName = userRow.name;
    await db.execute(`INSERT INTO bug_comments (bug_id, author_name, author_id, content, is_internal, comment_type, created_at, updated_at)
     VALUES (?, ?, ?, ?, 0, 'workflow_change', NOW(), NOW())`, [bugId, authorName, ctx.userId, `Workflow phase changed: ${oldPhase} → ${newPhase}`]);
    const phaseLabels = { intake: 'Intake', qa: 'QA', development: 'Development' };
    return {
        success: true,
        message: `✅ Bug #${bugId} moved from **${phaseLabels[oldPhase] || oldPhase}** to **${phaseLabels[newPhase]}**.`,
    };
}
async function execGetBugStats() {
    const total = await db.queryOne('SELECT COUNT(*) AS cnt FROM bugs');
    const byStatus = await db.query('SELECT status, COUNT(*) AS cnt FROM bugs GROUP BY status');
    const bySeverity = await db.query('SELECT severity, COUNT(*) AS cnt FROM bugs GROUP BY severity');
    const byPhase = await db.query('SELECT workflow_phase, COUNT(*) AS cnt FROM bugs GROUP BY workflow_phase');
    const bySoftware = await db.query(`SELECT COALESCE(software_name, 'Unspecified') AS software_name, COUNT(*) AS cnt FROM bugs GROUP BY software_name`);
    ...
    for (const s of ['critical', 'high', 'medium', 'low']) {
        if (severityMap[s])
            msg += `  ${severityIcon[s]} ${s}: ${severityMap[s]}\n`;
    }
    msg += `\n**By Phase:**\n`;
