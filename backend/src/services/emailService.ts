...
async function getSmtpCredentials(): Promise<SmtpCredentials> {
  ...
  if (row?.additional_data) {
    ...
    let password: string = row.credential_value || '';
    const parts = password.split(':');
    if (parts.length === 3 && parts.every((p: string) => /^[0-9a-f]{16,}$/i.test(p))) {
      password = decryptPassword(password) ?? '';
      if (!password) throw new Error('[Email] Failed to decrypt SMTP credential_value');
    }
...
async function getTransporter(): Promise<{ transporter: nodemailer.Transporter; credentials: SmtpCredentials }> {
...
export async function sendTwoFactorOtp(to: string, code: string, userName?: string): Promise<EmailSendResult> {
  return sendEmail({
    to,
...
async function logEmailSend(
  ...
  try {
    ...
    await db.execute(
      `INSERT INTO email_log (to_address, subject, status, message_id, error)
       VALUES (?, ?, ?, ?, ?)`,
      [
