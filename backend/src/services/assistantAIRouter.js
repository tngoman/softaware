 ...
 */
export async function shouldUseOpenRouter(tier) {
    if (tier !== 'paid')
        return false;
 ...
 */
function buildGLMBody(messages, opts, stream = false) {
    let system;
    const filtered = [];
...
export async function glmChat(messages, opts) {
    const apiKey = await getGLMKey();
    if (!apiKey)
        throw new Error('GLM key not available');
...
async function glmStream(messages, opts) {
    const apiKey = await getGLMKey();
    if (!apiKey)
        throw new Error('GLM key not available');
    const res = await fetch(`${GLM_BASE_URL}/v1/messages`, {
...
async function openRouterChat(messages, opts) {
    ...
    if (!apiKey)
        throw new Error('OpenRouter key not available');
...
async function openRouterStream(messages, opts) {
    const apiKey = await getOpenRouterKey();
    if (!apiKey)
        throw new Error('OpenRouter key not available');
    const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
...
async function ollamaChat(messages, opts, modelOverride) {
...
async function ollamaStream(messages, opts, modelOverride) {
    const model = modelOverride || OLLAMA_MODEL;
    const res = await fetch(`${OLLAMA_BASE}/api/chat`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
 ...
 */
export async function chatCompletion(tier, messages, opts = {}, modelOverride) {
    ...
    if (glmKey) {
        try {
            ...
            console.warn(`[AssistantRouter] GLM failed: ${err.message} — trying ${tier === 'paid' ? 'OpenRouter' : 'Ollama'}`);
        }
    }
    ...
    if (orKey) {
        try {
            ...
            console.warn(`[AssistantRouter] OpenRouter failed: ${err.message} — falling back to Ollama`);
        }
    }
    // ── 3. Ollama (last resort) ──
 ...
 */
export async function chatCompletionStream(tier, messages, opts = {}, modelOverride) {
    // ── 1. GLM (tried first for ALL tiers) ──
    const glmKey = await getGLMKey();
    if (glmKey) {
        try {
            ...
            console.warn(`[AssistantRouter] GLM stream failed: ${err.message} — trying ${tier === 'paid' ? 'OpenRouter' : 'Ollama'}`);
        }
    }
    // ── 2. OpenRouter (paid tier only) ──
    if (tier !== 'paid') {
    ...
    if (orKey) {
        try {
            ...
            console.warn(`[AssistantRouter] OpenRouter stream failed: ${err.message} — falling back to Ollama`);
        }
    }
    // ── 3. Ollama (last resort) ──
...
async function openRouterVisionChat(messages, opts, modelOverride) {
    const apiKey = await getOpenRouterKey();
    if (!apiKey)
        throw new Error('OpenRouter key not available for vision');
    ...
    const data = await res.json();
    const provider = model === VISION_OPENROUTER_MODEL ? 'openrouter' : 'openrouter-fallback';
    return {
        content: (data.choices?.[0]?.message?.content ?? '').trim(),
        model,
        provider,
...
async function openRouterVisionStream(messages, opts, modelOverride) {
    const apiKey = await getOpenRouterKey();
    if (!apiKey)
        throw new Error('OpenRouter key not available for vision');
    const model = modelOverride || VISION_OPENROUTER_MODEL;
...
async function ollamaVisionChat(messages, opts) {
    const res = await fetch(`${OLLAMA_BASE}/api/chat`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
...
async function ollamaVisionStream(messages, opts) {
    const res = await fetch(`${OLLAMA_BASE}/api/chat`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
 ...
 * Paid tier:  OpenRouter gpt-4o → OpenRouter gemini-2.0-flash → Ollama qwen2.5vl:7b
 */
export async function chatCompletionWithVision(tier, messages, opts = {}) {
    ...
    if (orKey) {
        ...
        try {
            ...
            console.warn(`[VisionRouter] gpt-4o failed: ${err.message} — trying Gemini flash`);
        }
        ...
        try {
            ...
            console.warn(`[VisionRouter] Gemini flash failed: ${err.message} — falling back to Ollama`);
        }
    }
    // 3. Ollama (last resort)
 ...
 */
export async function chatCompletionStreamWithVision(tier, messages, opts = {}) {
    ...
    if (orKey) {
        ...
        try {
            ...
            console.warn(`[VisionRouter] gpt-4o stream failed: ${err.message} — trying Gemini flash`);
        }
        // 2. Try gemini-flash
        try {
            ...
            console.warn(`[VisionRouter] Gemini flash stream failed: ${err.message} — falling back to Ollama`);
        }
    }
    // 3. Ollama (last resort)
