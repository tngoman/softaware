...
import { AIMessage } from './ai/AIProvider.js';
...
export class CodeAgentService {
  private systemPrompt = `You are a code editing assistant. You receive instructions to modify code files.
...
- For modifications, include the entire file content`;
  ...
  async executeInstruction(request: CodeEditRequest): Promise<CodeEditResponse> {
    // Validate directory
  ...
  private async readContextFiles(baseDir: string, files: string[]): Promise<string> {
    const contents = [];
  ...
  private async readDirRecursive(dir: string, baseDir: string): Promise<string[]> {
    const files = [];
