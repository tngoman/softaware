...
export interface SmsMessage {
  /** E.164 destination number, e.g. "27821234567" */
  destination: string;
  /** SMS body text (max 160 GSM-7 chars = 1 credit) */
  content: string;
}

export interface SendOptions {
  /** When true the API validates but does not send or deduct credits */
  testMode?: boolean;
  /** Optional campaign name for reporting */
  campaignName?: string;
  /** ISO 8601 UTC date to schedule the send */
...
interface SmsCredentials {
  clientId: string;
  secret: string;  // empty string when provider doesn't issue a secret
}
...
async function getCredentials(): Promise<SmsCredentials> {
  ...
  const secretRow = await db.queryOne<any>(
    `SELECT credential_value FROM credentials
  ...
  if (!row?.credential_value) {
    throw new Error(
      '[SMS] No active SMS credentials found. Expected rows with service_name ' +
      "'SMS KEY' + 'SMS SECRET', or a single 'SMS' row.",
    );
  }

  let raw: string = decrypt(row.credential_value);
  ...
  const secret: string = extra?.secret ? decrypt(extra.secret) : '';
  if (!secret) {
    throw new Error(
      '[SMS] API secret not found. Store it as a separate credentials row ' +
...
function decrypt(val: string): string {
  ...
  if (parts.length === 3 && parts.every((p: string) => /^[0-9a-f]{16,}$/i.test(p))) {
    ...
    if (!result) throw new Error(`[SMS] Failed to decrypt value starting with ${val.slice(0, 12)}…`);
    return result;
  }
  return val;
}
...
async function getAuthToken(): Promise<string> {
  ...
  try {
    ...
    await db.execute(
      'UPDATE credentials SET last_used_at = NOW() WHERE service_name = ? AND is_active = 1',
      ['SMS'],
    ).catch(() => {/* non-critical */});
...
export async function sendBulkSms(
  ...
  if (!messages.length) throw new Error('[SMS] messages array is empty');
  ...
  if (options.testMode) sendOptions.testMode = true;
  if (options.campaignName) sendOptions.campaignName = options.campaignName;
  if (options.scheduledDelivery) sendOptions.scheduledDelivery = options.scheduledDelivery;
  if (options.duplicateCheck) sendOptions.duplicateCheck = true;
  if (Object.keys(sendOptions).length) body.sendOptions = sendOptions;
  ...
  try {
    ...
    const result: SmsSendResult = res.data;

    console.log(
      `[SMS] Sent ${normalisedMessages.length} message(s) — ` +
    ...
    if (err instanceof AxiosError && err.response?.status === 401) {
      ...
      const retryResult: SmsSendResult = retryRes.data;
 ...
 */
export async function getBalance(): Promise<SmsBalance> {
  const token = await getAuthToken();
