 ...
 */
export async function registerFcmToken(userId, token, deviceName, platform) {
    // Upsert: if token already exists (maybe for same or different user), update it
    await db.execute(`INSERT INTO fcm_tokens (user_id, token, device_name, platform)
     VALUES (?, ?, ?, ?)
     ON DUPLICATE KEY UPDATE user_id = VALUES(user_id), device_name = VALUES(device_name),
       platform = VALUES(platform), updated_at = NOW()`, [userId, token, deviceName || null, platform || null]);
}
/**
 ...
 */
export async function listFcmTokens(userId) {
    return db.query(`SELECT id, token, device_name, platform, created_at, updated_at FROM fcm_tokens WHERE user_id = ? ORDER BY updated_at DESC`, [userId]);
}
/**
...
export async function sendPushToUser(userId, payload) {
    ...
    const devices = await db.query(`SELECT id, token FROM fcm_tokens WHERE user_id = ?`, [userId]);
    if (devices.length === 0)
        return { sent: 0, failed: 0 };
    ...
    try {
        ...
        if (staleTokenIds.length > 0) {
            await db.execute(`DELETE FROM fcm_tokens WHERE id IN (${staleTokenIds.map(() => '?').join(',')})`, staleTokenIds);
 ...
 */
export async function createNotificationWithPush(userId, notification) {
    // Check user notification preferences
    const userPrefs = await db.queryOne(`SELECT notifications_enabled, web_notifications_enabled, push_notifications_enabled 
    ...
    if (!userPrefs || userPrefs.web_notifications_enabled !== false) {
        await db.execute(`INSERT INTO notifications (user_id, title, message, type, data, created_at)
       VALUES (?, ?, ?, ?, ?, NOW())`, [userId, notification.title, notification.message, notification.type || 'info', notification.data ? JSON.stringify(notification.data) : null]);
    }
