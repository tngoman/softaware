 ...
 */
export function parseLeadCapture(aiResponse) {
    try {
 ...
 */
export async function storeCapturedLead(clientId, leadData, chatContext) {
    const leadId = uuidv4();
    await db.execute(`INSERT INTO widget_leads_captured 
 ...
 */
export async function sendLeadNotification(clientId, leadData, businessName) {
    try {
        // Get client configuration
        const [rows] = await db.execute(`SELECT wc.lead_notification_email, wc.website_url, u.email as user_email, u.name as user_name
...
Manage settings: https://portal.softaware.net.za
    `.trim();
        ...
        await db.execute(`UPDATE widget_leads_captured 
       SET notification_sent = TRUE 
       WHERE client_id = ? AND visitor_email = ?
       ORDER BY captured_at DESC
       LIMIT 1`, [clientId, leadData.email]);
 ...
 */
export async function getLeadStats(clientId, days = 30) {
    const [rows] = await db.execute(`SELECT 
     ...
     ORDER BY capture_date DESC`, [clientId, days]);
    return rows || [];
}
/**
