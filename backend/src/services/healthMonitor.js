 ...
 */
export function trackApiError(entry) {
...
async function checkDatabase() {
    ...
    try {
        ...
        try {
            const threadsRow = await db.queryOne("SHOW STATUS LIKE 'Threads_connected'");
            const maxRow = await db.queryOne("SHOW VARIABLES LIKE 'max_connections'");
        ...
        if (responseTime > 1000 && status === 'healthy')
            status = 'error';
        else if (responseTime > 500 && status === 'healthy')
            status = 'warning';
        return {
            status,
...
function checkDiskSpace() {
    try {
        ...
        let status = 'healthy';
        let msg;
        if (usedPct >= DISK_CRIT_PCT) {
            status = 'error';
            msg = `Disk space critical: ${usedPct}% used`;
        }
        else if (usedPct >= DISK_WARN_PCT) {
            status = 'warning';
            msg = `Disk space elevated: ${usedPct}% used`;
        }
...
function checkProcessHealth() {
    try {
        ...
        try {
            ...
            const backend = pm2Data.find((p) => p.name === 'softaware-backend');
            if (backend)
                pm2Restarts = backend.pm2_env?.restart_time || 0;
        }
        catch { /* non-fatal */ }
...
async function checkIngestionQueue() {
    try {
        const pendingJobs = await db.queryOne('SELECT COUNT(*) as count FROM ingestion_jobs WHERE status = ?', ['pending']);
        const stuckJobs = await db.queryOne("SELECT COUNT(*) as count FROM ingestion_jobs WHERE status = 'processing' AND updated_at < DATE_SUB(NOW(), INTERVAL 1 HOUR)", []);
        const details = { pending_count: pendingJobs?.count || 0, stuck_count: stuckJobs?.count || 0 };
        if (details.stuck_count > 0)
            return { status: 'warning', error_message: `${details.stuck_count} jobs stuck for over 1 hour`, details };
        if (details.pending_count > 100)
            return { status: 'warning', error_message: `Queue backlog: ${details.pending_count} pending`, details };
...
async function checkEnterpriseEndpoint(endpointId) {
    const startTime = Date.now();
    try {
        const endpoints = getAllEndpoints();
        const endpoint = endpoints.find(e => e.id === endpointId);
        if (!endpoint)
            return { status: 'error', error_message: 'Endpoint not found' };
        if (endpoint.status !== 'active')
            return { status: 'warning', error_message: `Endpoint is ${endpoint.status}` };
...
function checkAuthHealth() {
    const oneMinuteAgo = Date.now() - 60_000;
    const recentAuthErrors = recentApiErrors.filter(e => e.timestamp > oneMinuteAgo && e.path.includes('/auth/login') && e.statusCode >= 500);
    let status = 'healthy';
    let msg;
    if (recentAuthErrors.length >= 5) {
        status = 'error';
        msg = `Auth service errors: ${recentAuthErrors.length} login 500s in last minute`;
    }
    else if (recentAuthErrors.length >= 2) {
        status = 'warning';
        msg = `Auth issues: ${recentAuthErrors.length} login 500s in last minute`;
    }
...
// ═══════════════════════════════════════════════════════════════════
async function recordHealthCheck(checkType, checkName, result) {
    ...
    try {
        ...
        const existing = await db.queryOne('SELECT * FROM system_health_checks WHERE check_type = ? AND check_name = ?', [checkType, checkName]);
        ...
        const consecutiveFailures = isFailure
            ...
            : 0;
        const shouldCreateCase = (result.status === 'error' && consecutiveFailures >= FAILURE_THRESHOLD) ||
            (result.status === 'warning' && consecutiveFailures >= WARNING_THRESHOLD);
        if (existing) {
            await db.execute(`UPDATE system_health_checks SET 
         ...
         WHERE id = ?`, [
            ...
            if (result.status === 'healthy' && existing.case_id) {
                await db.execute(`UPDATE cases SET status = 'resolved', resolution = ?, resolved_at = ? WHERE id = ? AND status NOT IN ('resolved','closed')`, [`Automatically resolved — ${checkName} health restored`, now, existing.case_id]);
                await db.execute('UPDATE system_health_checks SET case_id = NULL WHERE id = ?', [existing.id]);
            }
        }
        else {
            await db.execute(`INSERT INTO system_health_checks (
...
async function flushDeferredCases() {
    while (deferredCases.length > 0) {
        const deferred = deferredCases[0];
        try {
            const existing = await db.queryOne('SELECT case_id FROM system_health_checks WHERE check_type = ? AND check_name = ?', [deferred.checkType, deferred.checkName]);
            if (!existing?.case_id) {
                ...
                if (caseId && existing) {
                    await db.execute('UPDATE system_health_checks SET case_id = ? WHERE check_type = ? AND check_name = ?', [caseId, deferred.checkType, deferred.checkName]);
                }
            }
            deferredCases.shift();
        }
...
async function createAutoDetectedCase(checkType, checkName, result, consecutiveFailures) {
    try {
        ...
        await db.execute('INSERT INTO case_activity (id, case_id, action, new_value, created_at) VALUES (?, ?, ?, ?, ?)', [generateId(), caseId, 'auto_created', `Health monitor: ${checkType} ${result.status} — ${result.error_message || checkName}`, now]);
...
export async function getHealthStatus() {
    const checks = await db.query('SELECT * FROM system_health_checks ORDER BY FIELD(status, "error", "warning", "unknown", "healthy"), check_type, check_name');
    const summary = {
