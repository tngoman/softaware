...
(async () => {
  ...
  if (ffmpegAvailable) console.log('[MediaProcessor] ffmpeg available ✅');
  else console.log('[MediaProcessor] ffmpeg not found — video processing disabled');
  if (ffprobeAvailable) console.log('[MediaProcessor] ffprobe available ✅');
  else console.log('[MediaProcessor] ffprobe not found — audio duration disabled');
})();

// ── Image processing ────────────────────────────────────

export interface ImageProcessResult {
  /** Path to the compressed original */
  filePath: string;
  /** Path to the generated thumbnail (200px wide) */
  thumbnailPath: string;
  /** Thumbnail relative URL for DB */
  thumbnailUrl: string;
 ...
 */
export async function processAudio(inputPath: string): Promise<AudioProcessResult | null> {
