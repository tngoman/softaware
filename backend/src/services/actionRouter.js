 ...
 */
export function getToolsForTier(tier, enabledTools) {
    const tools = [];
 ...
 */
export function getToolsSystemPrompt(tools) {
    if (tools.length === 0)
        return '';
 ...
 */
export async function executeToolCall(toolCall, assistantId, assistantConfig) {
 ...
 */
async function executeCaptureLeadTool(args, assistantId, config) {
    const { name, email, phone, interest, urgency } = args;
    try {
        const leadId = `lead-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
        const sessionId = `session-${Date.now()}`;
        const now = toMySQLDate(new Date());
        // Store lead in database using existing table structure
        await db.execute(`INSERT INTO lead_captures (id, sessionId, sourcePage, companyName, contactName, email, phone, useCase, requirements, status, score, messageCount, createdAt, updatedAt)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'NEW', 0, 0, ?, ?)`, [leadId, sessionId, assistantId, null, name || null, email, phone || null, interest, urgency || null, now, now]);
 ...
 */
async function executeScheduleCallbackTool(args, assistantId, config) {
    const { name, phone, preferred_time, reason } = args;
    try {
        ...
        await db.execute(`INSERT INTO lead_captures (id, sessionId, sourcePage, companyName, contactName, email, phone, useCase, requirements, status, score, messageCount, createdAt, updatedAt)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'CALLBACK', 0, 0, ?, ?)`, [leadId, sessionId, assistantId, null, name || 'Callback Request', null, phone, `CALLBACK: ${reason}`, preferred_time || null, now, now]);
        // Send notification email if configured
        if (config.leadCaptureEmail) {
            await sendLeadNotificationEmail(config.leadCaptureEmail, {
                assistantName: config.name,
 ...
 */
async function executeTriggerWebhookTool(args, assistantId, config) {
    ...
    try {
        ...
        await db.execute(`INSERT INTO widget_usage_logs (id, assistant_id, action, data, response_code, created_at)
       VALUES (UUID(), ?, ?, ?, ?, ?)`, [assistantId, `webhook:${action}`, JSON.stringify(data), response.status, toMySQLDate(new Date())]);
        return {
            success: true,
...
async function sendLeadNotificationEmail(to, data) {
