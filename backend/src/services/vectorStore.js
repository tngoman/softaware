  ...
  `);
    const tx = db.transaction((items) => {
        for (const c of items) {
            insertChunk.run(c.id, c.assistantId, c.jobId, c.content, c.source, c.sourceType, c.chunkIndex, c.charCount, c.createdAt);
 ...
 */
export function search(assistantId, queryEmbedding, topK = 5) {
    const db = getDb();
...
export function deleteByJob(jobId) {
    ...
    const ids = db.prepare(`SELECT id FROM knowledge_chunks WHERE job_id = ?`).all(jobId);
    if (ids.length === 0)
        return 0;
 ...
 */
export function deleteByAssistant(assistantId) {
    const db = getDb();
    const ids = db.prepare(`SELECT id FROM knowledge_chunks WHERE assistant_id = ?`).all(assistantId);
    if (ids.length === 0)
        return 0;
    const deleteVec = db.prepare(`DELETE FROM knowledge_vectors WHERE chunk_id = ?`);
    const deleteChunk = db.prepare(`DELETE FROM knowledge_chunks WHERE id = ?`);
 ...
 */
export function stats(assistantId) {
    const db = getDb();
    const total = db.prepare(`SELECT COUNT(*) as cnt FROM knowledge_chunks WHERE assistant_id = ?`).get(assistantId);
