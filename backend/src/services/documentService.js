...
export const documentService = {
    ...
    async storeChunk(params) {
        ...
        await db.execute(`INSERT INTO document_metadata (
     ...
     */
    async crawlWebsite(clientId, url) {
        try {
            // Fetch HTML
            ...
            await db.execute('UPDATE widget_clients SET pages_ingested = pages_ingested + 1 WHERE id = ?', [clientId]);
            return {
                success: true,
    ...
    async storeFileContent(params) {
        try {
            ...
            await db.execute('UPDATE widget_clients SET pages_ingested = pages_ingested + 1 WHERE id = ?', [params.clientId]);
            return {
                success: true,
                chunksCreated: chunks.length
            };
    ...
    async getClientDocuments(clientId) {
        return db.query('SELECT * FROM document_metadata WHERE client_id = ? ORDER BY source_url, chunk_index', [clientId]);
    },
    /**
     * Get document sources (grouped by URL/filename)
     */
    async getDocumentSources(clientId) {
       ...
       ORDER BY MAX(created_at) DESC`, [clientId]);
    },
    /**
     * Delete all documents for a client
     */
    async deleteClientDocuments(clientId) {
        await db.execute('DELETE FROM document_metadata WHERE client_id = ?', [clientId]);
        // Reset pages_ingested count
        await db.execute('UPDATE widget_clients SET pages_ingested = 0 WHERE id = ?', [clientId]);
    },
    /**
     * Delete documents by source
     */
    async deleteDocumentsBySource(clientId, sourceUrl) {
        // Get count first
        const countResult = await db.queryOne('SELECT COUNT(DISTINCT source_url) as count FROM document_metadata WHERE client_id = ? AND source_url = ?', [clientId, sourceUrl]);
        // Delete documents
        await db.execute('DELETE FROM document_metadata WHERE client_id = ? AND source_url = ?', [clientId, sourceUrl]);
        // Decrement pages_ingested
        if (countResult && countResult.count > 0) {
            await db.execute('UPDATE widget_clients SET pages_ingested = GREATEST(pages_ingested - 1, 0) WHERE id = ?', [clientId]);
        }
    }
};
