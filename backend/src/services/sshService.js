...
export class SSHService {
     ...
     */
    async writeFile(config, path, content) {
        const client = await this.getConnection(config);
     ...
     */
    async moveFile(config, oldPath, newPath) {
        const client = await this.getConnection(config);
     ...
     */
    async disconnectAll() {
        const disconnectPromises = Array.from(this.connections.values()).map(client => client.end().catch(err => console.error('Error closing SSH connection:', err)));
