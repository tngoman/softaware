...
function mapCaseRow(row, aiOverride) {
    if (!row)
        return row;
...
const AddCommentSchema = z.object({
    ...
    is_internal: z.preprocess((val) => val === 'true' || val === true || val === '1', z.boolean().default(false)),
...
casesRouter.post('/', requireAuth, async (req, res) => {
    try {
        ...
        // Log activity
        await db.execute('INSERT INTO case_activity (id, case_id, user_id, action, new_value, created_at) VALUES (?, ?, ?, ?, ?, ?)', [generateId(), caseId, userId, 'created', caseNumber, now]);
        // Notify admins
        ...
        const createdCase = await db.queryOne('SELECT * FROM cases WHERE id = ?', [caseId]);
        res.status(201).json({
            success: true,
        ...
        console.error('[Cases] Error stack:', err.stack);
...
casesRouter.get('/:id', requireAuth, async (req, res) => {
    try {
        ...
        const caseData = await db.queryOne(`SELECT c.*, COALESCE(u.name, u.email) AS reported_by_name, u.email AS reported_by_email,
       ...
       WHERE c.id = ?`, [caseId]);
        ...
        const adminRow = await db.queryOne('SELECT is_admin FROM users WHERE id = ?', [userId]);
        ...
        const comments = await db.query(commentsQuery, [caseId]);
        // Get activity log
        const activity = await db.query(`SELECT ca.*, COALESCE(u.name, u.email) AS user_name
...
casesRouter.patch('/:id', requireAuth, async (req, res) => {
    try {
        ...
        const userId = req.userId;
        const caseId = req.params.id;
        const existing = await db.queryOne('SELECT * FROM cases WHERE id = ?', [caseId]);
        if (!existing) {
            return res.status(404).json({ success: false, error: 'Case not found' });
        }
        // Check access: reporter or admin
        const adminRow2 = await db.queryOne('SELECT is_admin FROM users WHERE id = ?', [userId]);
        ...
        if (updates.length > 0) {
            ...
            for (const activity of activities) {
                await db.execute('INSERT INTO case_activity (id, case_id, user_id, action, old_value, new_value, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)', [generateId(), caseId, userId, activity.action, String(activity.old || ''), String(activity.new), now]);
            }
        ...
        const updated = await db.queryOne('SELECT * FROM cases WHERE id = ?', [caseId]);
        res.json({
            success: true,
...
// ═════════════════════════════════════════════════════════════════════════
casesRouter.post('/:id/comments', requireAuth, upload.array('attachments', 5), async (req, res) => {
    try {
        ...
        const userId = req.userId;
        const caseId = req.params.id;
        const caseData = await db.queryOne('SELECT * FROM cases WHERE id = ?', [caseId]);
        if (!caseData) {
            return res.status(404).json({ success: false, error: 'Case not found' });
        }
        ...
        await db.execute('INSERT INTO case_comments (id, case_id, user_id, comment, is_internal, attachments, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', [commentId, caseId, userId, data.comment, data.is_internal, JSON.stringify(allAttachments), now, now]);
        // Log activity
        await db.execute('INSERT INTO case_activity (id, case_id, user_id, action, new_value, created_at) VALUES (?, ?, ?, ?, ?, ?)', [generateId(), caseId, userId, 'commented', data.comment.substring(0, 100), now]);
        ...
        const comment = await db.queryOne('SELECT cc.*, COALESCE(u.name, u.email) AS user_name FROM case_comments cc LEFT JOIN users u ON cc.user_id = u.id WHERE cc.id = ?', [commentId]);
        res.status(201).json({
            success: true,
            comment: {
                ...comment,
...
// ═════════════════════════════════════════════════════════════════════════
casesRouter.post('/:id/rate', requireAuth, async (req, res) => {
    try {
        ...
        await db.execute('UPDATE cases SET rating = ?, rating_comment = ?, updated_at = ? WHERE id = ?', [data.rating, data.rating_comment || null, toMySQLDate(new Date()), caseId]);
        // Log activity
        await db.execute('INSERT INTO case_activity (id, case_id, user_id, action, new_value, created_at) VALUES (?, ?, ?, ?, ?, ?)', [generateId(), caseId, userId, 'rated', `${data.rating} stars`, toMySQLDate(new Date())]);
