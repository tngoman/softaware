...
import { db, type team_members, type Team } from '../db/mysql.js';
...
subscriptionRouter.get('/current', requireAuth, async (req: AuthRequest, res, next) => {
  try {
    ...
    const membership = await db.queryOne<team_members & { teamName: string }>(
      `SELECT tm.*, t.name as teamName FROM team_members tm JOIN teams t ON tm.teamId = t.id WHERE tm.userId = ? LIMIT 1`,
      [userId]
    );
...
subscriptionRouter.post('/start-trial', requireAuth, async (req: AuthRequest, res, next) => {
  try {
    const userId = req.userId!;
 ...
 */
subscriptionRouter.post('/cancel', requireAuth, async (req: AuthRequest, res, next) => {
  try {
    const userId = req.userId!;
 ...
 */
subscriptionRouter.get('/invoices', requireAuth, async (req: AuthRequest, res, next) => {
  try {
    const userId = req.userId!;
...
subscriptionRouter.get('/admin/all', requireAuth, async (req: AuthRequest, res, next) => {
  try {
    const userId = req.userId!;
...
subscriptionRouter.post('/admin/seed-plans', requireAuth, async (req: AuthRequest, res, next) => {
  try {
