 ...
 */
async function proxyToExternal(baseUrl, path, method, apiKey, body) {
...
async function resolveTaskSource(req) {
    const rawApiUrl = req.query.apiUrl || req.body?.apiUrl || '';
    ...
    let source = null;
    if (softwareId) {
        source = await db.queryOne(`SELECT id, base_url, api_key
       FROM task_sources
 ...
 */
async function sendTaskAssignmentNotification(task, assignerUserId) {
    const assignedUserId = String(task.assigned_to);
    // Find the assigned user in our local system users
    const assignedUser = await db.queryOne('SELECT id, name, email FROM users WHERE id = ?', [assignedUserId]);
    if (!assignedUser) {
    ...
    if (assignerUserId) {
        const assigner = await db.queryOne('SELECT name, email FROM users WHERE id = ?', [assignerUserId]);
        if (assigner) {
 ...
 */
async function sendTaskPhaseChangeNotification(task, changerUserId) {
    if (!task.assigned_to || !task.workflow_phase)
        return;
    const assignedUserId = String(task.assigned_to);
    // Don't notify if the assigned user changed the phase themselves
    if (changerUserId && assignedUserId === changerUserId)
        return;
    const assignedUser = await db.queryOne('SELECT id, name, email FROM users WHERE id = ?', [assignedUserId]);
    if (!assignedUser)
        return;
    ...
    if (changerUserId) {
        const changer = await db.queryOne('SELECT name, email FROM users WHERE id = ?', [changerUserId]);
        if (changer)
            changerName = changer.name || changer.email || 'Someone';
    }
    const taskTitle = task.title || `Task #${task.id}`;
    await createNotificationWithPush(assignedUser.id, {
...
softawareTasksRouter.get('/', requireAuth, async (req, res) => {
    try {
        ...
        const result = await proxyToExternal(baseUrl, `/api/tasks-api?page=${page}&limit=${limit}`, 'GET', apiKey);
...
softawareTasksRouter.post('/', requireAuth, async (req, res) => {
    try {
        const { baseUrl, apiKey } = await resolveTaskSource(req);
        const { task } = req.body;
...
// ─── PUT /softaware/tasks ──────────────────────────────────────
softawareTasksRouter.put('/', requireAuth, async (req, res) => {
    try {
        const { baseUrl, apiKey } = await resolveTaskSource(req);
        const { task } = req.body;
...
softawareTasksRouter.delete('/:id', requireAuth, async (req, res) => {
    try {
        ...
        if (result.status >= 200 && result.status < 300) {
            try {
                await db.execute('UPDATE local_tasks SET task_deleted = 1, updated_at = NOW() WHERE external_id = ? AND task_deleted = 0', [String(id)]);
            }
            catch (localErr) {
...
softawareTasksRouter.post('/reorder', requireAuth, async (req, res) => {
    try {
        const { baseUrl, apiKey } = await resolveTaskSource(req);
...
softawareTasksRouter.get('/:id/associations', requireAuth, async (req, res) => {
    try {
        const { baseUrl, apiKey } = await resolveTaskSource(req);
        const { id } = req.params;
...
softawareTasksRouter.post('/:id/associations', requireAuth, async (req, res) => {
    try {
        const { baseUrl, apiKey } = await resolveTaskSource(req);
        const { id } = req.params;
...
softawareTasksRouter.delete('/:id/associations', requireAuth, async (req, res) => {
    try {
        const { baseUrl, apiKey } = await resolveTaskSource(req);
        const { id } = req.params;
...
softawareTasksRouter.get('/:id/attachments', requireAuth, async (req, res) => {
    try {
        const { baseUrl, apiKey } = await resolveTaskSource(req);
        const { id } = req.params;
...
softawareTasksRouter.post('/:id/attachments', requireAuth, async (req, res) => {
    try {
        const { baseUrl, apiKey } = await resolveTaskSource(req);
        const { id } = req.params;
...
softawareTasksRouter.delete('/:id/attachments/:attachmentId', requireAuth, async (req, res) => {
    try {
        const { baseUrl, apiKey } = await resolveTaskSource(req);
        ...
        const match = attachmentList.find((a) => String(a.attachment_id) === String(attachmentId));
        if (!match?.file_path) {
            return res.status(404).json({ success: false, error: 'Attachment not found' });
        }
        const result = await proxyToExternal(baseUrl, `/api/tasks-api/attachments/${encodeURIComponent(match.file_path)}`, 'DELETE', apiKey);
        res.status(result.status).json(result.data);
    }
    catch (err) {
        res.status(400).json({ success: false, error: err.message });
...
softawareTasksRouter.get('/:id/comments', requireAuth, async (req, res) => {
    try {
        const { baseUrl, apiKey } = await resolveTaskSource(req);
        const { id } = req.params;
...
softawareTasksRouter.post('/:id/comments/with-attachment', requireAuth, async (req, res) => {
    try {
        const { baseUrl, apiKey } = await resolveTaskSource(req);
        const { id } = req.params;
        ...
        const commentResult = await proxyToExternal(baseUrl, `/api/tasks-api/${id}/comments`, 'POST', apiKey, commentBody);
...
softawareTasksRouter.post('/:id/comments', requireAuth, async (req, res) => {
    try {
        const { baseUrl, apiKey } = await resolveTaskSource(req);
        const { id } = req.params;
...
softawareTasksRouter.post('/authenticate', requireAuth, async (_req, res) => {
    return res.json({
        success: true,
...
softawareTasksRouter.post('/:id/start', requireAuth, async (req, res) => {
    try {
        const { baseUrl, apiKey } = await resolveTaskSource(req);
        const { id } = req.params;
...
softawareTasksRouter.post('/:id/complete', requireAuth, async (req, res) => {
    try {
        const { baseUrl, apiKey } = await resolveTaskSource(req);
        const { id } = req.params;
...
softawareTasksRouter.post('/:id/approve', requireAuth, async (req, res) => {
    try {
        const { baseUrl, apiKey } = await resolveTaskSource(req);
        const { id } = req.params;
...
softawareTasksRouter.get('/pending-approval', requireAuth, async (req, res) => {
    try {
        const { baseUrl, apiKey } = await resolveTaskSource(req);
...
softawareTasksRouter.get('/stats', requireAuth, async (req, res) => {
    try {
        const { baseUrl, apiKey } = await resolveTaskSource(req);
...
softawareTasksRouter.post('/sync', requireAuth, async (req, res) => {
    try {
        const { baseUrl, apiKey } = await resolveTaskSource(req);
...
softawareTasksRouter.post('/invoice-tasks', requireAuth, async (req, res) => {
    try {
        const { baseUrl, apiKey } = await resolveTaskSource(req);
...
softawareTasksRouter.post('/bill', requireAuth, async (req, res) => {
    try {
        const { baseUrl, apiKey } = await resolveTaskSource(req);
...
softawareTasksRouter.put('/time', requireAuth, async (req, res) => {
    try {
        const { baseUrl, apiKey } = await resolveTaskSource(req);
...
softawareTasksRouter.get('/statement', requireAuth, async (req, res) => {
    try {
        const { baseUrl, apiKey } = await resolveTaskSource(req);
...
softawareTasksRouter.get('/orders/latest', requireAuth, async (req, res) => {
    try {
        const { baseUrl, apiKey } = await resolveTaskSource(req);
...
softawareTasksRouter.get('/orders/budgets', requireAuth, async (req, res) => {
    try {
        const { baseUrl, apiKey } = await resolveTaskSource(req);
...
softawareTasksRouter.get('/orders/:orderNumber/budget', requireAuth, async (req, res) => {
    try {
        const { baseUrl, apiKey } = await resolveTaskSource(req);
...
softawareTasksRouter.get('/:id/parent', requireAuth, async (req, res) => {
    try {
        const { baseUrl, apiKey } = await resolveTaskSource(req);
        const { id } = req.params;
...
softawareTasksRouter.delete('/comments/:commentId', requireAuth, async (req, res) => {
    try {
        const { baseUrl, apiKey } = await resolveTaskSource(req);
...
softawareTasksRouter.post('/comments/:commentId/convert-to-task', requireAuth, async (req, res) => {
    try {
        const { baseUrl, apiKey } = await resolveTaskSource(req);
        const { commentId } = req.params;
...
softawareTasksRouter.get('/attachments/:filename', requireAuth, async (req, res) => {
    try {
        const { baseUrl, apiKey } = await resolveTaskSource(req);
...
softawareTasksRouter.get('/:id', requireAuth, async (req, res) => {
    try {
        const { baseUrl, apiKey } = await resolveTaskSource(req);
        const { id } = req.params;
