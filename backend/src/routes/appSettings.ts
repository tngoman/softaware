...
appSettingsRouter.get('/', async (req, res, next) => {
  try {
    ...
    if (category) {
      ...
      rows = await db.query<any>('SELECT setting_key, setting_value FROM app_settings');
    }
...
appSettingsRouter.get('/:key', async (req, res, next) => {
  try {
    const row = await db.queryOne<any>(
      'SELECT setting_key, setting_value, data_type, description FROM app_settings WHERE setting_key = ?',
      [req.params.key]
    );
    if (!row) throw notFound(`Setting "${req.params.key}" not found`);
...
appSettingsRouter.put('/', requireAdmin, async (req, res, next) => {
  try {
    ...
    if (!settings || typeof settings !== 'object') throw badRequest('Expected object body');
...
appSettingsRouter.put('/:key', requireAdmin, async (req, res, next) => {
  try {
    ...
    await db.execute(
      `INSERT INTO app_settings (setting_key, setting_value, data_type) VALUES (?, ?, ?)
       ON DUPLICATE KEY UPDATE setting_value = VALUES(setting_value), data_type = COALESCE(VALUES(data_type), data_type)`,
      [req.params.key, value, type ?? null]
    );
