...
async function storeRecentErrors(softwareKey, clientIdentifier, hostname, appVersion, osInfo, recentErrors) {
    let stored = 0;
    for (const error of recentErrors) {
        ...
        if (!errorType || !errorLevel || !errorMessage)
            continue;
        try {
            await db.insert(`INSERT INTO error_reports (
    ...
    if (stored > 0) {
        ...
        try {
            await db.execute(`INSERT INTO client_error_summaries
...
updHeartbeatRouter.get('/check', async (req, res, next) => {
    try {
        const softwareKey = req.query.software_key || req.header('X-Software-Key');
        if (!softwareKey)
            throw badRequest('Missing software_key (query param or X-Software-Key header)');
        const currentVersion = req.query.version || req.query.app_version;
        const sw = await db.queryOne('SELECT id, name FROM update_software WHERE software_key = ?', [softwareKey]);
        if (!sw)
            throw badRequest('Invalid software_key');
        const latest = await db.queryOne(`SELECT id, version, description, has_migrations, released_at, file_path
       FROM update_releases WHERE software_id = ?
       ORDER BY id DESC LIMIT 1`, [sw.id]);
...
updHeartbeatRouter.post('/', async (req, res, next) => {
    try {
        ...
        if (!softwareKey)
            throw badRequest('Missing software_key');
        // Resolve software
        const sw = await db.queryOne('SELECT id, name FROM update_software WHERE software_key = ?', [softwareKey]);
        if (!sw)
            throw badRequest('Invalid software_key');
        ...
        const existing = await db.queryOne('SELECT * FROM update_clients WHERE software_id = ? AND client_identifier = ?', [sw.id, clientId]);
        // If blocked, reject
        ...
        if (body.update_installed && body.update_id) {
            await db.execute(`INSERT IGNORE INTO update_installed (update_id, status) VALUES (?, 'installed')`, [body.update_id]);
        }
        ...
        if (existing) {
            ...
            await db.execute(`UPDATE update_clients SET
         ...
         WHERE id = ?`, [
            ...
            // Create new client
            const result = await db.insert(`INSERT INTO update_clients (software_id, client_identifier, ip_address, hostname,
        ...
        const latest = await db.queryOne(`SELECT id, version, description, has_migrations, released_at
       FROM update_releases WHERE software_id = ?
       ORDER BY id DESC LIMIT 1`, [sw.id]);
        ...
        if (body.recent_errors && Array.isArray(body.recent_errors) && body.recent_errors.length > 0) {
            errorsStored = await storeRecentErrors(softwareKey, clientId, body.hostname || null, body.app_version || null, body.os_info || null, body.recent_errors);
        }
        // Build response
        const response = {
            success: true,
        ...
        if (existing) {
            ...
            if (existing.server_message) {
                ...
                await db.execute('UPDATE update_clients SET server_message = NULL, server_message_id = NULL WHERE id = ?', [existing.id]);
            }
        }
        res.json(response);
    }
