        ...
        const local = localBranches.split('\n').filter(Boolean).map(line => {
            ...
            if (trackMatch) {
                trackMatch.forEach(m => {
                    if (m.includes('ahead'))
                        ahead = parseInt(m.match(/\d+/)?.[0] || '0');
                    if (m.includes('behind'))
                        behind = parseInt(m.match(/\d+/)?.[0] || '0');
                });
            }
            return {
                name,
...
router.get('/commit/:hash', async (req, res) => {
    try {
        ...
        const metadataRaw = await execGit(`git show --format="%H|%h|%s|%an|%ae|%aI|%at" -s ${hash}`);
        const [fullHash, shortHash, subject, author, email, date, timestampStr] = metadataRaw.split('|');
        // Get full commit message (body)
        const fullMessageRaw = await execGit(`git log -1 --format="%B" ${hash}`);
        const fullMessage = fullMessageRaw.trim();
        // Get file changes
        const fileChangesRaw = await execGit(`git show --name-status --format="" ${hash}`);
...
router.get('/history', async (req, res) => {
    try {
        ...
        const output = await execGit(`git log --follow --format="%H|%h|%s|%an|%ae|%aI|%at" -- "${filePath}"`);
        if (!output) {
            return res.json({
                path: filePath,
                commits: []
...
router.get('/diff', async (req, res) => {
    try {
        ...
        const file = req.query.file ? sanitizePath(req.query.file) : '';
