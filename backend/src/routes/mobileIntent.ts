...
router.post('/intent', requireAuth, async (req: AuthRequest, res) => {
  try {
    ...
    const user = await db.queryOne<{ account_status: string }>(
    ...
    if (resolvedAssistantId) {
      ...
      const assistant = await db.queryOne<{ id: string }>(
        ...
        [resolvedAssistantId, userId],
      );
      if (!assistant) throw notFound('Assistant not found or access denied.');
    } else {
      // Auto-select the user's primary assistant
      const primary = await db.queryOne<{ id: string }>(
        "SELECT id FROM assistants WHERE userId = ? AND is_primary = 1 AND status = 'active' LIMIT 1",
        [userId],
      );
      if (primary) resolvedAssistantId = primary.id;
    }
...
router.get('/assistants', requireAuth, async (req: AuthRequest, res) => {
  try {
    const userId = req.userId;
    if (!userId) throw unauthorized('Authentication required.');
    ...
    const assistants = await db.query<{
      ...
      personality: string | null; personality_flare: string | null;
      custom_greeting: string | null; voice_style: string | null;
      is_staff_agent: number; is_primary: number; status: string; tier: string;
    }>(
      `SELECT id, name, description, personality, personality_flare,
       ...
       ORDER BY is_primary DESC, is_staff_agent DESC, created_at DESC`,
      [userId],
    );
...
router.get('/conversations', requireAuth, async (req: AuthRequest, res) => {
  try {
    ...
    const conversations = await db.query<{
      id: string; role: string; assistant_id: string | null; created_at: string; updated_at: string; preview: string | null;
    }>(
      `SELECT c.id, c.role, c.assistant_id, c.created_at, c.updated_at,
...
router.get('/conversations/:id/messages', requireAuth, async (req: AuthRequest, res) => {
  try {
    ...
    const messages = await db.query<{
      id: string; role: string; content: string; tool_name: string | null; created_at: string;
    }>(
