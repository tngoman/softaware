...
dashboardRouter.get('/metrics', requireAuth, async (req, res, next) => {
    try {
        ...
        const subscription = await db.queryOne(`SELECT 
        s.status,
        sp.tier,
        sp.maxAgents,
        sp.maxUsers
      ...
      LIMIT 1`, [membership.teamId]);
        ...
        const assistantCount = await db.queryOne('SELECT COUNT(*) as count FROM assistants WHERE userId = ? OR userId IS NULL', [userId]);
        console.log('[Dashboard] Assistant count result:', assistantCount);
        // Count total indexed pages across all user's assistants
        const assistantIds = await db.query('SELECT id FROM assistants WHERE userId = ? OR userId IS NULL', [userId]);
        let totalIndexedPages = 0;
        if (assistantIds.length > 0) {
            const ids = assistantIds.map(a => a.id);
            const pageCount = await db.queryOne(`SELECT COUNT(*) as count FROM ingestion_jobs 
         WHERE assistant_id IN (${ids.map(() => '?').join(',')}) 
         AND status = 'completed'`, ids);
        ...
        if (assistantIds.length > 0) {
            ...
            const activityCount = await db.queryOne(`SELECT COUNT(*) as count FROM ingestion_jobs 
         WHERE assistant_id IN (${ids.map(() => '?').join(',')})`, ids);
...
dashboardRouter.get('/stats', requireAuth, async (req, res, next) => {
    try {
        ...
        // ── Revenue ────────────────────────────────────────
        const revenueRow = await db.queryOne(`SELECT
       ...
       WHERE i.active = 1 ${dateFilter}`, []);
        ...
        const expenseRow = await db.queryOne(`SELECT COALESCE(SUM(debit_amount), 0) AS expenses
       FROM transactions
       WHERE 1=1 ${txDateFilter}`, []);
        ...
        // ── Invoice counts ────────────────────────────────
        const invCounts = await db.queryOne(`SELECT
       ...
       WHERE active = 1 ${dateFilter}`, []);
        // ── Quotation counts ──────────────────────────────
        const quoCounts = await db.queryOne(`SELECT COUNT(*) AS total_count, 0 AS accepted_count
       FROM quotations
       WHERE active = 1 ${quoDateFilter}`, []);
        // ── Customers ─────────────────────────────────────
        const custCounts = await db.queryOne(`SELECT COUNT(*) AS customer_count, 0 AS supplier_count
       FROM contacts WHERE active = 1`, []);
        // ── Payments ──────────────────────────────────────
        const payCounts = await db.queryOne(`SELECT
         COUNT(*) AS total_count,
         COALESCE(AVG(payment_amount), 0) AS average_amount
       FROM payments
       WHERE 1=1 ${payDateFilter}`, []);
        // ── Outstanding aging ─────────────────────────────
        const aging = await db.queryOne(`SELECT
       ...
       WHERE i.active = 1 AND i.paid = 0`, []);
        ...
        const recentInvoices = await db.query(`SELECT
         i.id AS invoice_id,
         i.invoice_number,
       ...
       LIMIT 5`, []);
        ...
        const recentQuotations = await db.query(`SELECT
