...
async function requireAgentAccess(userId, agentId) {
    const agent = await db.queryOne('SELECT * FROM agents_config WHERE id = ?', [agentId]);
    if (!agent)
        throw notFound('Agent not found');
    const member = await db.queryOne('SELECT * FROM team_members WHERE teamId = ? AND userId = ?', [agent.teamId, userId]);
    if (!member)
...
agentsRouter.get('/', async (req, res, next) => {
    try {
        ...
        const memberships = await db.query('SELECT * FROM team_members WHERE userId = ?', [userId]);
        const teamIds = memberships.map((m) => m.teamId);
        if (teamIds.length === 0) {
            return res.json({ agents: [] });
        }
        const placeholders = teamIds.map(() => '?').join(',');
        const agents = await db.query(`SELECT id, teamId, name, version, region, compliance, publishedAt, createdAt, updatedAt 
       FROM agents_config WHERE teamId IN (${placeholders}) ORDER BY updatedAt DESC`, teamIds);
        res.json({ agents });
    }
    catch (err) {
        next(err);
...
agentsRouter.post('/', async (req, res, next) => {
    try {
        ...
        const member = await db.queryOne('SELECT * FROM team_members WHERE teamId = ? AND userId = ?', [input.teamId, userId]);
        if (!member)
            throw forbidden('Not a member of this team');
        const agentId = generateId();
        const now = toMySQLDate(new Date());
        await db.execute(`INSERT INTO agents_config (id, teamId, name, version, region, compliance, blueprint, createdByUserId, createdAt, updatedAt) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
            agentId,
            input.teamId,
            input.name,
...
agentsRouter.put('/:id', async (req, res, next) => {
    try {
        ...
        const updated = await db.queryOne('SELECT * FROM agents_config WHERE id = ?', [agent.id]);
...
agentsRouter.delete('/:id', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const { agent, member } = await requireAgentAccess(userId, req.params.id);
        if (member.role !== 'ADMIN' && member.role !== 'ARCHITECT')
            throw forbidden('Insufficient role');
...
agentsRouter.post('/:id/publish', async (req, res, next) => {
    try {
        ...
        const published = await db.queryOne('SELECT * FROM agents_config WHERE id = ?', [agent.id]);
