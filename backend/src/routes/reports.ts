 ...
 */
reportsRouter.get('/', requireAuth, async (req: AuthRequest, res: Response, next) => {
  try {
    ...
    switch (type) {
      case 'trial-balance': {
        ...
        const totalDebit = accounts.reduce((sum: number, a: any) => sum + Number(a.total_debit), 0);
        const totalCredit = accounts.reduce((sum: number, a: any) => sum + Number(a.total_credit), 0);
      ...
      case 'vat': {
        ...
        const taxRates = await db.query<any>('SELECT * FROM tax_rates WHERE active = 1');

        // Calculate output VAT (from invoices)
        const outputVat = await db.queryOne<any>(
          `SELECT COALESCE(SUM(invoice_amount * 0.15), 0) as vat_amount, COUNT(*) as invoice_count
           FROM invoices WHERE active = 1 AND invoice_date BETWEEN ? AND ?`,
          [from, to]
        );

        // Calculate input VAT (from expense transactions)
        const inputVat = await db.queryOne<any>(
          `SELECT COALESCE(SUM(t.debit_amount * 0.15), 0) as vat_amount, COUNT(*) as transaction_count
           FROM transactions t
      ...
      case 'income-statement': {
        ...
        const totalIncome = income.reduce((sum: number, a: any) => sum + Number(a.amount), 0);
        const totalExpenses = expenses.reduce((sum: number, a: any) => sum + Number(a.amount), 0);
