...
emailRouter.get('/config', requireAdmin, async (_req, res, next) => {
    try {
        const row = await db.queryOne(`SELECT credential_value, additional_data, last_used_at, updated_at
         FROM credentials
...
emailRouter.put('/config', requireAdmin, async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const input = UpdateConfigSchema.parse(req.body);
        // Check if row exists
        const existing = await db.queryOne(`SELECT id, credential_value FROM credentials WHERE service_name = ? LIMIT 1`, ['SMTP']);
        ...
        if (existing) {
            ...
            if (input.password) {
                await db.execute(`UPDATE credentials 
           SET credential_value = ?, additional_data = ?, updated_by = ?, is_active = 1
           WHERE service_name = ?`, [input.password, additionalData, userId, 'SMTP']);
            }
            else {
                await db.execute(`UPDATE credentials 
           SET additional_data = ?, updated_by = ?, is_active = 1
           WHERE service_name = ?`, [additionalData, userId, 'SMTP']);
            }
        }
        else {
            // Insert new
            ...
            await db.execute(`INSERT INTO credentials (service_name, credential_type, identifier, credential_value, additional_data, environment, is_active, created_by, notes)
         VALUES (?, 'password', ?, ?, ?, 'production', 1, ?, 'SMTP email service credentials')`, ['SMTP', input.username, input.password, additionalData, userId]);
        }
...
emailRouter.get('/logs', requireAdmin, async (req, res, next) => {
    try {
        ...
        const rows = await db.query(`SELECT id, to_address, subject, status, message_id, error, created_at
         FROM email_log
        ORDER BY created_at DESC
        LIMIT ? OFFSET ?`, [limit, offset]);
        const countRow = await db.queryOne('SELECT COUNT(*) as total FROM email_log');
        res.json({
            success: true,
            data: rows,
