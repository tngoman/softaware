...
function parseEnrichmentJson(raw) {
    try {
        ...
        if (!tagline && !about && serviceDescs.length === 0)
            return null;
...
(async () => {
    try {
        const cols = await db.query(`SHOW COLUMNS FROM generated_sites WHERE Field IN ('generated_html', 'generation_error')`);
        ...
        console.warn('[SiteBuilder] Column migration note:', err.message);
    }
})();
...
router.post('/', requireAuth, async (req, res) => {
    try {
        ...
        const { businessName, tagline, logoUrl, heroImageUrl, aboutUs, services, contactEmail, contactPhone, ftpServer, ftpUsername, ftpPassword, ftpPort, ftpProtocol, ftpDirectory, themeColor, widgetClientId } = req.body;
        if (!businessName) {
...
router.get('/:siteId/preview', async (req, res) => {
    try {
        ...
        if (!site.generated_html)
            return res.status(404).send('<h1>No generated content yet</h1>');
 ...
 */
router.post('/:siteId/generate', requireAuth, async (req, res) => {
    try {
        const userId = req.userId;
        const { siteId } = req.params;
        const site = await siteBuilderService.getSiteById(siteId);
 ...
 */
router.post('/:siteId/deploy', requireAuth, async (req, res) => {
    try {
        const userId = req.userId;
        const { siteId } = req.params;
        const site = await siteBuilderService.getSiteById(siteId);
 ...
 */
router.get('/:siteId/deployments', requireAuth, async (req, res) => {
    try {
        const userId = req.userId;
        const { siteId } = req.params;
        const site = await siteBuilderService.getSiteById(siteId);
 ...
 */
router.get('/:siteId/generation-status', requireAuth, async (req, res) => {
    try {
        const userId = req.userId;
        const { siteId } = req.params;
        const site = await siteBuilderService.getSiteById(siteId);
        ...
        if (site.user_id !== userId)
            return res.status(403).json({ error: 'Access denied' });
        const result = { success: true, status: site.status };
...
router.post('/generate-ai', requireAuth, async (req, res) => {
    try {
        ...
        const { siteId, businessName, tagline, aboutText, services, logoUrl, heroImageUrl, clientId, tier = 'free' } = req.body;
        ...
        if (resolvedTier === 'free') {
            const row = await db.queryOne(`SELECT COUNT(*) AS cnt FROM generated_sites WHERE status = 'generating'`);
...
- Include one object in the services array for each service listed above, in order.`;
                if (resolvedTier === 'paid') {
                    ...
                    try {
                        ...
                        console.warn(`[ai-generation] GLM failed: ${glmErr.message} — trying OpenRouter`);
                    }
                    ...
                    if (!paidEnriched) {
                        try {
                            ...
                            console.warn('[ai-generation] OpenRouter failed:', orErr.message, '— falling back to Ollama');
                        }
                    }
                ...
                if (targetSiteId) {
                    ...
                    if (siteRecord?.theme_color)
                        themeColor = siteRecord.theme_color;
                }
                ...
                if (targetSiteId) {
                    ...
                    if (axios.isAxiosError(error)) {
                        if (error.code === 'ECONNREFUSED')
                            userMsg = 'AI service unavailable — could not connect to Ollama.';
                        else if (error.code === 'ETIMEDOUT' || error.code === 'ECONNABORTED')
                            userMsg = 'AI generation timed out. Please try again.';
                    }
 ...
 */
router.post('/:siteId/skip-queue', requireAuth, async (req, res) => {
    try {
        const userId = req.userId;
        if (!userId)
            return res.status(401).json({ error: 'Unauthorized' });
...
- Include one object in the services array for each service listed above, in order.`;
                ...
                try {
                    ...
                    console.warn('[ai-generation] Skip-queue GLM failed:', e.message);
                }
                // 2. Try OpenRouter
                if (!paidEnriched) {
                    try {
                        ...
                        console.warn('[ai-generation] Skip-queue OpenRouter failed:', e.message);
                    }
                }
                // 3. Ollama fallback
                if (!paidEnriched) {
                    try {
                        ...
                        console.log('[ai-generation] Skip-queue Ollama fallback also failed:', e.message);
                    }
                }
                const { generateSiteHtml } = await import('../services/siteBuilderTemplate.js');
 ...
 */
router.post('/:siteId/polish', requireAuth, async (req, res) => {
    try {
        const userId = req.userId;
        if (!userId)
            return res.status(401).json({ error: 'Unauthorized' });
...
${currentHtml}`;
                ...
                try {
                    ...
                    console.warn('[ai-polish] GLM failed:', e.message);
                }
                // 2. OpenRouter
                if (!polished) {
                    try {
                        ...
                        console.warn('[ai-polish] OpenRouter failed:', e.message);
                    }
                }
                // 3. Ollama fallback
                if (!polished) {
                    try {
                        ...
                        console.warn('[ai-polish] Ollama failed:', e.message);
                    }
                }
