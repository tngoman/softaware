...
import { db, generateId, toMySQLDate, type ai_model_config, type team_members } from '../db/mysql.js';
...
aiConfigRouter.get('/', requireAuth, async (req, res, next) => {
  try {
    ...
    if (!membership) throw notFound('No team found for user');

    const config = await db.queryOne<ai_model_config>(
      'SELECT * FROM ai_model_config WHERE teamId = ?',
      [membership.teamId]
    );
...
aiConfigRouter.put('/', requireAuth, async (req, res, next) => {
  try {
    ...
    const existing = await db.queryOne<ai_model_config>(
      'SELECT * FROM ai_model_config WHERE teamId = ?',
      [membership.teamId]
    );
    ...
    if (existing) {
      ...
      if (input.defaultTextProvider !== undefined) { updates.push('defaultTextProvider = ?'); values.push(input.defaultTextProvider); }
      if (input.defaultTextModel !== undefined) { updates.push('defaultTextModel = ?'); values.push(input.defaultTextModel); }
      if (input.visionProvider !== undefined) { updates.push('visionProvider = ?'); values.push(input.visionProvider); }
      if (input.visionModel !== undefined) { updates.push('visionModel = ?'); values.push(input.visionModel); }
      if (input.codeProvider !== undefined) { updates.push('codeProvider = ?'); values.push(input.codeProvider); }
      if (input.codeModel !== undefined) { updates.push('codeModel = ?'); values.push(input.codeModel); }
      ...
      await db.execute(
        `INSERT INTO ai_model_config (id, teamId, defaultTextProvider, defaultTextModel, visionProvider, visionModel, codeProvider, codeModel, createdAt, updatedAt) 
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          configId,
          membership.teamId,
