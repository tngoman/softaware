...
// ═══════════════════════════════════════════════════════════════════════
settingsRouter.get('/public', async (_req, res: Response, next) => {
  try {
    const rows = await db.query<any>(
      'SELECT `key`, `value`, `type` FROM sys_settings WHERE is_public = 1 ORDER BY `key`'
    );
...
// Must be registered BEFORE the /:id param route
// ═══════════════════════════════════════════════════════════════════════
settingsRouter.get('/key/:key', requireAuth, async (req: AuthRequest, res: Response, next) => {
  try {
    const row = await db.queryOne<any>(
      'SELECT * FROM sys_settings WHERE `key` = ?',
      [req.params.key]
    );
    if (!row) {
...
// ═══════════════════════════════════════════════════════════════════════
settingsRouter.get('/:id', requireAuth, async (req: AuthRequest, res: Response, next) => {
  try {
    const row = await db.queryOne<any>(
      'SELECT * FROM sys_settings WHERE id = ?',
      [req.params.id]
    );
    if (!row) {
      return res.status(404).json({ success: false, error: 'Setting not found' });
...
// ═══════════════════════════════════════════════════════════════════════
settingsRouter.post('/', requireAuth, requireAdmin, async (req: AuthRequest, res: Response, next) => {
  try {
    const data = CreateSettingSchema.parse(req.body);

    // Check for duplicate key
    const existing = await db.queryOne<any>(
      'SELECT id FROM sys_settings WHERE `key` = ?',
      [data.key]
    );
    if (existing) {
    ...
    await db.execute(
      'INSERT INTO sys_settings (`key`, `value`, `type`, `description`, `is_public`, `created_at`) VALUES (?, ?, ?, ?, ?, ?)',
      [data.key, data.value, data.type, data.description || null, data.is_public ? 1 : 0, now]
    );
...
// ═══════════════════════════════════════════════════════════════════════
settingsRouter.put('/:id', requireAuth, requireAdmin, async (req: AuthRequest, res: Response, next) => {
  try {
    ...
    if (data.key && data.key !== existing.key) {
      const dup = await db.queryOne<any>(
        'SELECT id FROM sys_settings WHERE `key` = ? AND id != ?',
        [data.key, id]
      );
      if (dup) {
        return res.status(409).json({ success: false, error: `Setting with key "${data.key}" already exists` });
    ...
    values.push(id);
    await db.execute(
      `UPDATE sys_settings SET ${fields.join(', ')} WHERE id = ?`,
      values
    );
...
// ═══════════════════════════════════════════════════════════════════════
settingsRouter.delete('/:id', requireAuth, requireAdmin, async (req: AuthRequest, res: Response, next) => {
  try {
    const id = req.params.id;
