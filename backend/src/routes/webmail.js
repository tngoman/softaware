...
import { ensureWebmailTable, listMailboxes, getMailbox, createMailbox, updateMailbox, deleteMailbox, setDefaultMailbox, testConnection, listFolders, listMessages, getMessage, getAttachment, deleteMessage, moveMessage, flagMessage, sendMail, getWebmailSettings, saveWebmailSettings, getUnreadCount, } from '../services/webmailService.js';
...
webmailRouter.put('/accounts/:id', async (req, res, next) => {
    try {
        ...
        if (isNaN(id))
            throw badRequest('Invalid mailbox ID');
...
webmailRouter.delete('/accounts/:id', async (req, res, next) => {
    try {
        ...
        if (!deleted)
            throw notFound('Mailbox not found');
...
async function resolveAccount(req) {
    ...
    if (isNaN(accountId))
        throw badRequest('account_id query parameter required');
    const account = await getMailbox(accountId, userId);
    if (!account)
        throw notFound('Mailbox not found');
    if (!account.is_active)
        throw badRequest('Mailbox is disabled');
    return account;
}
// ─── GET /webmail/folders — List IMAP folders ────────────────────────────
...
webmailRouter.get('/messages/:uid', async (req, res, next) => {
    try {
        ...
        if (isNaN(uid))
            throw badRequest('Invalid message UID');
...
webmailRouter.get('/attachment', async (req, res, next) => {
    try {
        ...
        if (isNaN(uid) || !partId)
            throw badRequest('uid and partId query parameters required');
...
webmailRouter.put('/settings', requireAdmin, async (req, res, next) => {
    try {
        const data = WebmailSettingsSchema.parse(req.body);
