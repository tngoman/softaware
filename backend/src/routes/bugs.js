...
// ─── WORKFLOW CONSTANTS ─────────────────────────────────────────
const WORKFLOW_PHASES = ['intake', 'qa', 'development'];
const VALID_STATUSES = ['open', 'in-progress', 'pending-qa', 'resolved', 'closed', 'reopened'];
const VALID_SEVERITIES = ['critical', 'high', 'medium', 'low'];
// ─── NOTIFICATION HELPERS ────────────────────────────────────────
/** Fetch all admin users */
async function getAdminUsers() {
    try {
        const rows = await db.query(`SELECT id, email, COALESCE(name, email) AS name FROM users WHERE is_admin = 1`);
...
//  LIST BUGS
// ═══════════════════════════════════════════════════════════════
bugsRouter.get('/', requireAuth, async (req, res) => {
    try {
        ...
        // Count total
        const [countRow] = await db.query(`SELECT COUNT(*) as total FROM bugs b ${where}`, params);
...
//  BUG STATISTICS
// ═══════════════════════════════════════════════════════════════
bugsRouter.get('/stats', requireAuth, async (_req, res) => {
    try {
...
// ═══════════════════════════════════════════════════════════════
bugsRouter.get('/:id', requireAuth, async (req, res) => {
    try {
        const { id } = req.params;
        const bug = await db.queryOne('SELECT * FROM bugs WHERE id = ?', [id]);
        if (!bug)
            return res.status(404).json({ status: 0, message: 'Bug not found' });
        const comments = await db.query('SELECT * FROM bug_comments WHERE bug_id = ? ORDER BY created_at ASC', [id]);
        const attachments = await db.query('SELECT * FROM bug_attachments WHERE bug_id = ? ORDER BY created_at ASC', [id]);
        // If there's a linked task, fetch basic info
        let linked_task = null;
        if (bug.linked_task_id) {
            linked_task = await db.queryOne('SELECT id, title, status, workflow_phase, external_id FROM local_tasks WHERE id = ?', [bug.linked_task_id]);
        }
        res.json({
            status: 1,
            data: {
...
//  CREATE BUG
// ═══════════════════════════════════════════════════════════════
bugsRouter.post('/', requireAuth, async (req, res) => {
    try {
        const { title, description, current_behaviour, expected_behaviour, reporter_name, software_id, software_name, severity, assigned_to, assigned_to_name, linked_task_id, } = req.body;
...
//  UPDATE BUG
// ═══════════════════════════════════════════════════════════════
bugsRouter.put('/:id', requireAuth, async (req, res) => {
    try {
        const { id } = req.params;
        const existing = await db.queryOne('SELECT * FROM bugs WHERE id = ?', [id]);
        if (!existing)
            return res.status(404).json({ status: 0, message: 'Bug not found' });
        const allowed = [
            'title', 'description', 'current_behaviour', 'expected_behaviour',
...
//  DELETE BUG
// ═══════════════════════════════════════════════════════════════
bugsRouter.delete('/:id', requireAuth, async (req, res) => {
    try {
        const { id } = req.params;
        const bug = await db.queryOne('SELECT id, title FROM bugs WHERE id = ?', [id]);
        if (!bug)
            return res.status(404).json({ status: 0, message: 'Bug not found' });
        // Delete attachment files from disk
        const attachments = await db.query('SELECT file_path FROM bug_attachments WHERE bug_id = ?', [id]);
...
//  COMMENTS
// ═══════════════════════════════════════════════════════════════
bugsRouter.post('/:id/comments', requireAuth, async (req, res) => {
    try {
        const { id } = req.params;
        const bug = await db.queryOne('SELECT * FROM bugs WHERE id = ?', [id]);
        if (!bug)
    ...
    `, [id, author_name, req.userId || null, content, is_internal ? 1 : 0, comment_type || 'comment']);
        ...
        if (isUserComment) {
            ...
            // Notify bug creator
            if (bug.created_by && bug.created_by !== req.userId) {
                bugNotify({
            ...
            // Notify assignee (if different from creator)
            if (bug.assigned_to && bug.assigned_to !== bug.created_by && bug.assigned_to !== req.userId) {
                bugNotify({
        ...
        const comment = await db.queryOne('SELECT * FROM bug_comments WHERE id = ?', [commentId]);
...
bugsRouter.delete('/:id/comments/:commentId', requireAuth, async (req, res) => {
    try {
        const { id, commentId } = req.params;
        const comment = await db.queryOne('SELECT id FROM bug_comments WHERE id = ? AND bug_id = ?', [commentId, id]);
        if (!comment)
...
//  ATTACHMENTS
// ═══════════════════════════════════════════════════════════════
bugsRouter.post('/:id/attachments', requireAuth, upload.array('files', 10), async (req, res) => {
    try {
        const { id } = req.params;
        const bug = await db.queryOne('SELECT id FROM bugs WHERE id = ?', [id]);
        if (!bug)
            return res.status(404).json({ status: 0, message: 'Bug not found' });
      ...
      `, [id, file.filename, file.originalname, file.mimetype, file.size, relPath, req.body.uploaded_by || null, req.userId || null]);
            const att = await db.queryOne('SELECT * FROM bug_attachments WHERE id = ?', [attId]);
            inserted.push(att);
        }
...
bugsRouter.delete('/:id/attachments/:attId', requireAuth, async (req, res) => {
    try {
        const { id, attId } = req.params;
        const att = await db.queryOne('SELECT * FROM bug_attachments WHERE id = ? AND bug_id = ?', [attId, id]);
        if (!att)
            return res.status(404).json({ status: 0, message: 'Attachment not found' });
...
bugsRouter.get('/:id/attachments/:attId/download', async (req, res) => {
    try {
        const { id, attId } = req.params;
        const att = await db.queryOne('SELECT * FROM bug_attachments WHERE id = ? AND bug_id = ?', [attId, id]);
        if (!att)
...
// ═══════════════════════════════════════════════════════════════
bugsRouter.put('/:id/workflow', requireAuth, async (req, res) => {
    try {
        const { id } = req.params;
        const { workflow_phase, user_name } = req.body;
        if (!workflow_phase || !WORKFLOW_PHASES.includes(workflow_phase)) {
    ...
    `, [
        ...
        const updated = await db.queryOne('SELECT * FROM bugs WHERE id = ?', [id]);
...
//  ASSIGN
// ═══════════════════════════════════════════════════════════════
bugsRouter.put('/:id/assign', requireAuth, async (req, res) => {
    try {
        const { id } = req.params;
        ...
        if (!bug)
            return res.status(404).json({ status: 0, message: 'Bug not found' });
        await db.execute('UPDATE bugs SET assigned_to = ?, assigned_to_name = ? WHERE id = ?', [assigned_to || null, assigned_to_name || null, id]);
...
//  LINK / UNLINK TASK
// ═══════════════════════════════════════════════════════════════
bugsRouter.put('/:id/link-task', requireAuth, async (req, res) => {
    try {
        const { id } = req.params;
        ...
        if (linked_task_id) {
            const task = await db.queryOne('SELECT id FROM local_tasks WHERE id = ?', [linked_task_id]);
            if (!task)
...
//  CONVERT BUG → TASK
// ═══════════════════════════════════════════════════════════════
bugsRouter.post('/:id/convert-to-task', requireAuth, async (req, res) => {
    try {
        const { id } = req.params;
        const bug = await db.queryOne('SELECT * FROM bugs WHERE id = ?', [id]);
        if (!bug)
        ...
        let source = null;
        if (bug.software_id) {
            source = await db.queryOne('SELECT id FROM task_sources WHERE software_id = ? LIMIT 1', [bug.software_id]);
        }
        if (!source) {
            source = await db.queryOne('SELECT id FROM task_sources ORDER BY id LIMIT 1');
        }
        if (!source) {
    ...
    `, [
        ...
        await db.execute('UPDATE bugs SET converted_to_task = ?, linked_task_id = ? WHERE id = ?', [taskId, taskId, id]);
        // Add conversion comment
        await db.insert(`
      INSERT INTO bug_comments (bug_id, author_name, author_id, content, comment_type)
      VALUES (?, ?, ?, ?, 'status_change')
    `, [id, req.body.user_name || 'System', req.userId || null, `Bug converted to Task #${taskId}`]);
        const task = await db.queryOne('SELECT * FROM local_tasks WHERE id = ?', [taskId]);
...
//  CONVERT TASK → BUG
// ═══════════════════════════════════════════════════════════════
bugsRouter.post('/from-task/:taskId', requireAuth, async (req, res) => {
    try {
        const { taskId } = req.params;
        const task = await db.queryOne('SELECT * FROM local_tasks WHERE id = ?', [taskId]);
        if (!task)
            return res.status(404).json({ status: 0, message: 'Task not found' });
        const { reporter_name, current_behaviour, expected_behaviour, severity, software_name, } = req.body;
        const bugId = await db.insert(`
      INSERT INTO bugs
        (title, description, current_behaviour, expected_behaviour,
         reporter_name, software_id, software_name, severity,
