...
updClientsRouter.get('/', async (req, res, next) => {
    try {
        ...
        const clients = await db.query(`SELECT c.*,
       ...
       ${where}
       ORDER BY c.last_heartbeat DESC`, params);
...
updClientsRouter.put('/', async (req, res, next) => {
    try {
        ...
        const client = await db.queryOne('SELECT id FROM update_clients WHERE id = ?', [body.id]);
        if (!client)
            throw notFound('Client not found');
        switch (body.action) {
            case 'block':
                await db.execute('UPDATE update_clients SET is_blocked = 1, blocked_at = NOW(), blocked_reason = ? WHERE id = ?', [body.reason || null, body.id]);
                return res.json({ success: true, action: 'blocked' });
            case 'unblock':
                await db.execute('UPDATE update_clients SET is_blocked = 0, blocked_at = NULL, blocked_reason = NULL WHERE id = ?', [body.id]);
            ...
            case 'send_message':
                if (!body.message)
                    throw badRequest('Message required for send_message action');
                const msgId = crypto.randomUUID();
                await db.execute('UPDATE update_clients SET server_message = ?, server_message_id = ? WHERE id = ?', [body.message, msgId, body.id]);
