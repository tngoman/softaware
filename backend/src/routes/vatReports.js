 ...
 */
vatReportsRouter.get('/', requireAuth, async (req, res, next) => {
    try {
        ...
        switch (type) {
           ...
           WHERE transaction_date BETWEEN ? AND ?`, [period_start, period_end]);
           ...
           FROM transactions_vat
           WHERE transaction_date BETWEEN ? AND ?`, [yearStart, yearEnd]);
                // Get expenses by category
                const expensesByCategory = await db.query(`SELECT 
           ...
           ORDER BY category_name ASC`, [yearStart, yearEnd]);
            ...
            SUM(CASE WHEN transaction_type = 'expense' THEN total_amount ELSE 0 END) as expense_total
           FROM transactions_vat
           WHERE transaction_date BETWEEN ? AND ?`, [yearStart, to_date]);
