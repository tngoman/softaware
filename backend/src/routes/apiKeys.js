...
apiKeysRouter.get('/', async (req, res, next) => {
    try {
        const apiKeys = await db.query(`SELECT id, name, \`key\`, isActive, lastUsedAt, createdAt, expiresAt 
       FROM api_keys WHERE userId = ? ORDER BY createdAt DESC`, [req.user.id]);
...
apiKeysRouter.post('/', async (req, res, next) => {
    try {
        ...
        const expiresAt = expiresInDays
            ...
            : null;
        await db.execute(`INSERT INTO api_keys (id, name, \`key\`, userId, isActive, createdAt, expiresAt) 
       VALUES (?, ?, ?, ?, ?, ?, ?)`, [keyId, name, apiKey, req.user.id, true, now, expiresAt]);
        const newKey = await db.queryOne('SELECT * FROM api_keys WHERE id = ?', [keyId]);
        res.json({
            id: newKey.id,
            name: newKey.name,
...
apiKeysRouter.delete('/:id', async (req, res, next) => {
    try {
        const { id } = req.params;
        const apiKey = await db.queryOne('SELECT * FROM api_keys WHERE id = ? AND userId = ?', [id, req.user.id]);
        if (!apiKey) {
...
apiKeysRouter.patch('/:id/toggle', async (req, res, next) => {
    try {
        const { id } = req.params;
        const apiKey = await db.queryOne('SELECT * FROM api_keys WHERE id = ? AND userId = ?', [id, req.user.id]);
        if (!apiKey) {
