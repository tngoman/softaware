...
syncRouter.post('/agents', async (req, res, next) => {
    try {
        ...
        // Check if exists
        const existing = await db.queryOne('SELECT id FROM client_agents WHERE deviceId = ? AND agentId = ?', [input.deviceId, input.agent.agentId]);
        if (existing) {
            // Update
            await db.execute(`UPDATE client_agents SET name = ?, version = ?, region = ?, compliance = ?, blueprint = ?, updatedAt = ? 
         WHERE deviceId = ? AND agentId = ?`, [
            ...
            // Insert
            const id = generateId();
            await db.execute(`INSERT INTO client_agents (id, deviceId, agentId, name, version, region, compliance, blueprint, createdAt, updatedAt) 
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
                id,
                input.deviceId,
                input.agent.agentId,
        ...
        const upserted = await db.queryOne('SELECT deviceId, agentId, name, version, region, createdAt, updatedAt FROM client_agents WHERE deviceId = ? AND agentId = ?', [input.deviceId, input.agent.agentId]);
...
syncRouter.get('/my-agents', requireAuth, async (req, res, next) => {
    try {
        ...
        const devices = await db.query('SELECT deviceId FROM device_activations WHERE userId = ?', [userId]);
        ...
        const agents = await db.query(`SELECT deviceId, agentId, name, version, region, compliance, blueprint, createdAt, updatedAt 
       FROM client_agents WHERE deviceId IN (${placeholders}) ORDER BY updatedAt DESC`, deviceIds);
