...
updErrorReportRouter.get('/summaries', requireAuth, requireAdmin, async (req, res, next) => {
    try {
        const rows = await db.query(`SELECT s.*, sw.name AS software_name
...
updErrorReportRouter.get('/', requireAuth, requireAdmin, async (req, res, next) => {
    try {
        ...
        const [rows, countRow] = await Promise.all([
            db.query(`SELECT e.*, sw.name AS software_name
         FROM error_reports e
         LEFT JOIN update_software sw ON e.software_key = sw.software_key
         ${where}
         ORDER BY e.received_at DESC
         LIMIT ? OFFSET ?`, [...params, limit, offset]),
            db.queryOne(`SELECT COUNT(*) AS total FROM error_reports e ${where}`, params),
        ]);
        res.json({
            success: true,
            errors: rows,
...
function normalizeError(e) {
    return {
        ...
        level: (e.level || e.error_level || 'error'),
...
updErrorReportRouter.post('/', async (req, res, next) => {
    try {
        ...
        // Verify software key
        const sw = await db.queryOne('SELECT id, name FROM update_software WHERE software_key = ?', [body.software_key]);
        if (!sw)
            throw notFound('Unknown software key');
        // Check if client is blocked
        const client = await db.queryOne('SELECT id, is_blocked, blocked_reason FROM update_clients WHERE software_id = ? AND client_identifier = ?', [sw.id, body.client_identifier]);
