...
import { requireAuth, getAuth, signAccessToken, setAuthCookie } from '../middleware/auth.js';
...
function createTOTP(secretBase32, userEmail) {
    return new TOTP({
...
async function sendOtpByMethod(userId, method, userEmail, userName, userPhone) {
    if (method === 'totp')
        return; // TOTP doesn't need sending — user has the app
    const otp = generateOtp();
    const otpHash = crypto.createHash('sha256').update(otp).digest('hex');
    // Store hashed OTP — use MySQL NOW() so expiry is in the DB's own timezone
    await db.execute(`UPDATE user_two_factor SET otp_code = ?, otp_expires_at = DATE_ADD(NOW(), INTERVAL 5 MINUTE), updated_at = NOW() WHERE user_id = ?`, [otpHash, userId]);
    if (method === 'email') {
        ...
        if (!userPhone)
            throw badRequest('No phone number on file. Please update your profile to use SMS 2FA.');
...
twoFactorRouter.get('/status', requireAuth, async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const row = await db.queryOne(`SELECT is_enabled, preferred_method FROM user_two_factor WHERE user_id = ?`, [userId]);
...
twoFactorRouter.post('/setup', requireAuth, async (req, res, next) => {
    try {
        ...
        const existing = await db.queryOne(`SELECT is_enabled, preferred_method FROM user_two_factor WHERE user_id = ?`, [userId]);
        ...
        if (existing?.is_enabled) {
            await db.execute(`UPDATE user_two_factor SET is_enabled = 0, otp_code = NULL, otp_expires_at = NULL, updated_at = NOW() WHERE user_id = ?`, [userId]);
        }
        // Get user email for the authenticator label
        const user = await db.queryOne('SELECT email, name, phone FROM users WHERE id = ?', [userId]);
        if (!user)
        ...
        if (input.method === 'totp') {
            ...
            if (existing) {
                await db.execute(`UPDATE user_two_factor SET secret = ?, preferred_method = ?, is_enabled = 0, backup_codes = NULL, updated_at = NOW() WHERE user_id = ?`, [secretBase32, 'totp', userId]);
            }
            else {
                await db.execute(`INSERT INTO user_two_factor (user_id, secret, preferred_method, is_enabled) VALUES (?, ?, ?, 0)`, [userId, secretBase32, 'totp']);
            }
            return res.json({
                success: true,
        ...
        if (existing) {
            await db.execute(`UPDATE user_two_factor SET secret = ?, preferred_method = ?, is_enabled = 0, backup_codes = NULL, updated_at = NOW() WHERE user_id = ?`, [placeholderSecret, input.method, userId]);
        }
        else {
            await db.execute(`INSERT INTO user_two_factor (user_id, secret, preferred_method, is_enabled) VALUES (?, ?, ?, 0)`, [userId, placeholderSecret, input.method]);
        }
        // Send a verification OTP to confirm the method works
        await sendOtpByMethod(userId, input.method, user.email, user.name, user.phone);
        res.json({
            success: true,
...
twoFactorRouter.post('/verify-setup', requireAuth, async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const input = VerifySetupSchema.parse(req.body);
        const row = await db.queryOne(`SELECT * FROM user_two_factor WHERE user_id = ?`, [userId]);
        if (!row)
            throw badRequest('Two-factor setup not initiated. Call /auth/2fa/setup first.');
        if (row.is_enabled)
        ...
        if (!user)
            throw badRequest('User not found');
        const method = row.preferred_method || 'totp';
        let isValid = false;
        if (method === 'totp') {
            // Verify TOTP code
        ...
        if (!isValid)
            throw badRequest('Invalid verification code. Please try again.');
        ...
        await db.execute(`UPDATE user_two_factor SET is_enabled = 1, otp_code = NULL, otp_expires_at = NULL, backup_codes = ?, updated_at = NOW() WHERE user_id = ?`, [JSON.stringify(hashedCodes), userId]);
        res.json({
            success: true,
...
twoFactorRouter.post('/verify', async (req, res, next) => {
    try {
        ...
        const row = await db.queryOne(`SELECT * FROM user_two_factor WHERE user_id = ? AND is_enabled = 1`, [userId]);
        if (!row)
        ...
        if (method === 'totp') {
            ...
            if (row.otp_code && row.otp_expires_at) {
                ...
                if (now <= expiresAt) {
                    ...
                    if (isValid) {
                        // Clear used OTP
                        await db.execute(`UPDATE user_two_factor SET otp_code = NULL, otp_expires_at = NULL, updated_at = NOW() WHERE user_id = ?`, [userId]);
                    }
                }
            }
        }
        ...
        if (!isValid) {
            ...
            if (matchIdx !== -1) {
                ...
                await db.execute(`UPDATE user_two_factor SET backup_codes = ?, updated_at = NOW() WHERE user_id = ?`, [JSON.stringify(backupCodes), userId]);
            }
        }
        if (!isValid) {
...
twoFactorRouter.post('/send-otp', async (req, res, next) => {
    try {
        ...
        const userId = String(decoded.userId);
        const row = await db.queryOne(`SELECT preferred_method FROM user_two_factor WHERE user_id = ? AND is_enabled = 1`, [userId]);
        if (!row)
            throw badRequest('Two-factor authentication is not enabled for this account.');
        const method = row.preferred_method || 'totp';
        if (method === 'totp') {
        ...
        if (!user)
            throw badRequest('User not found');
        await sendOtpByMethod(userId, method, user.email, user.name, user.phone);
        res.json({
            success: true,
...
twoFactorRouter.post('/disable', requireAuth, async (req, res, next) => {
    try {
        ...
        // Verify password
        const user = await db.queryOne('SELECT passwordHash FROM users WHERE id = ?', [userId]);
        if (!user)
            throw badRequest('User not found');
...
twoFactorRouter.put('/method', requireAuth, async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        ...
        // Verify password
        const user = await db.queryOne('SELECT passwordHash, email, name, phone FROM users WHERE id = ?', [userId]);
        if (!user)
            throw badRequest('User not found');
        const ok = await bcrypt.compare(input.password, user.passwordHash);
        if (!ok)
        ...
        // Check if 2FA exists
        const existing = await db.queryOne(`SELECT is_enabled FROM user_two_factor WHERE user_id = ?`, [userId]);
        ...
        if (input.method === 'totp') {
            ...
            await db.execute(`UPDATE user_two_factor SET secret = ?, preferred_method = ?, is_enabled = 0, updated_at = NOW() WHERE user_id = ?`, [secretBase32, 'totp', userId]);
            return res.json({
                success: true,
        ...
        const placeholderSecret = new Secret().base32;
        await db.execute(`UPDATE user_two_factor SET secret = ?, preferred_method = ?, is_enabled = 0, updated_at = NOW() WHERE user_id = ?`, [placeholderSecret, input.method, userId]);
        await sendOtpByMethod(userId, input.method, user.email, user.name, user.phone);
        res.json({
            success: true,
...
twoFactorRouter.post('/backup-codes', requireAuth, async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        ...
        const row = await db.queryOne(`SELECT is_enabled FROM user_two_factor WHERE user_id = ? AND is_enabled = 1`, [userId]);
        if (!row)
        ...
        await db.execute(`UPDATE user_two_factor SET backup_codes = ?, updated_at = NOW() WHERE user_id = ?`, [JSON.stringify(hashedCodes), userId]);
        res.json({
            success: true,
...
async function ensureMobileChallengeTable() {
    try {
        await db.execute(`
...
twoFactorRouter.post('/mobile-challenge', async (req, res, next) => {
    try {
        ...
        await db.execute(`UPDATE mobile_auth_challenges SET status = 'expired' WHERE user_id = ? AND status = 'pending'`, [userId]);
        // Create new challenge
        const challengeId = crypto.randomUUID();
        const challengeSecret = crypto.randomBytes(32).toString('hex');
        await db.execute(`INSERT INTO mobile_auth_challenges (id, user_id, challenge_secret, status, remember_me, expires_at)
       VALUES (?, ?, ?, 'pending', ?, DATE_ADD(NOW(), INTERVAL 5 MINUTE))`, [challengeId, userId, challengeSecret, decoded.rememberMe ? 1 : 0]);
        res.json({
            success: true,
...
twoFactorRouter.get('/mobile-qr', requireAuth, async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        // Find the latest pending challenge for this user
        const challenge = await db.queryOne(`SELECT id, challenge_secret, expires_at 
...
twoFactorRouter.get('/mobile-qr/status/:challengeId', requireAuth, async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const { challengeId } = req.params;
        const challenge = await db.queryOne(`SELECT status FROM mobile_auth_challenges WHERE id = ? AND user_id = ?`, [challengeId, userId]);
        res.json({
            success: true,
            data: {
...
twoFactorRouter.post('/mobile-verify', async (req, res, next) => {
    try {
        ...
        const challenge = await db.queryOne(`SELECT * FROM mobile_auth_challenges WHERE id = ? AND status = 'pending'`, [input.challenge_id]);
        if (!challenge) {
        ...
        if (now > expiresAt) {
            await db.execute(`UPDATE mobile_auth_challenges SET status = 'expired' WHERE id = ?`, [challenge.id]);
        ...
        await db.execute(`UPDATE mobile_auth_challenges SET status = 'completed' WHERE id = ?`, [challenge.id]);
