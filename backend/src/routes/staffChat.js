...
import { emitNewMessage, emitMessageEdited, emitMessageDeleted, emitMessageStatusUpdate, emitReactionUpdate, emitConversationUpdated, emitConversationDeleted, emitCallRinging, emitCallEnded, emitCallMissed, emitScheduledCall, isUserOnline, } from '../services/chatSocket.js';
import { sendPushToUser } from '../services/firebaseService.js';
import { getLinkPreviewForContent, fetchLinkPreview } from '../services/linkPreview.js';
import { processImage, processVideo, processAudio } from '../services/mediaProcessor.js';
...
async function requireMembership(conversationId, userId) {
    const member = await db.queryOne(`SELECT * FROM conversation_members
     WHERE conversation_id = ? AND user_id = ? AND removed_at IS NULL`, [conversationId, userId]);
    if (!member)
...
// ═════════════════════════════════════════════════════════════
staffChatRouter.get('/users/available', async (req, res, next) => {
    try {
 ...
 */
staffChatRouter.get('/conversations', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
 ...
 */
staffChatRouter.post('/conversations', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
      ...
      `, [userId, otherId]);
        ...
        // Create conversation
        const convId = await db.insert(`INSERT INTO conversations (type, name, description, created_by, created_at)
       VALUES (?, ?, ?, ?, NOW())`, [input.type, input.name || null, input.description || null, userId]);
        // Add creator as admin
        await db.execute(`INSERT INTO conversation_members (conversation_id, user_id, role, joined_at)
       VALUES (?, ?, 'admin', NOW())`, [convId, userId]);
        // Add other members
        for (const memberId of input.member_ids) {
            if (memberId === userId)
                continue; // already added
            await db.execute(`INSERT INTO conversation_members (conversation_id, user_id, role, joined_at)
         VALUES (?, ?, 'member', NOW())`, [convId, memberId]);
        }
        // For groups, add a system message
        if (input.type === 'group' && input.name) {
            await db.insert(`INSERT INTO messages (conversation_id, sender_id, content, message_type, created_at)
         VALUES (?, ?, ?, 'system', NOW())`, [convId, userId, `created the group "${input.name}"`]);
        }
 ...
 */
staffChatRouter.get('/conversations/:id', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
 ...
 */
staffChatRouter.put('/conversations/:id', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const convId = req.params.id;
        const input = updateConversationSchema.parse(req.body);
        const member = await requireMembership(convId, userId);
        const conv = await db.queryOne('SELECT * FROM conversations WHERE id = ?', [convId]);
        if (!conv)
            throw notFound('Conversation not found');
        ...
        const updated = await db.queryOne('SELECT * FROM conversations WHERE id = ?', [convId]);
 ...
 */
staffChatRouter.delete('/conversations/:id', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const convId = req.params.id;
        const member = await requireMembership(convId, userId);
        ...
        if (!conv)
            throw notFound('Conversation not found');
        if (conv.type === 'direct')
            throw badRequest('Cannot delete a DM — archive it instead');
        if (member.role !== 'admin')
            throw forbidden('Only admin can delete group');
 ...
 */
staffChatRouter.post('/conversations/:id/clear', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const convId = req.params.id;
        await requireMembership(convId, userId);
        await db.execute(`UPDATE conversation_members SET cleared_at = NOW()
       WHERE conversation_id = ? AND user_id = ? AND removed_at IS NULL`, [convId, userId]);
 ...
 */
staffChatRouter.post('/conversations/:id/members', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const convId = req.params.id;
        ...
        for (const uid of input.user_ids) {
            const existing = await db.queryOne(`SELECT id, removed_at FROM conversation_members
         WHERE conversation_id = ? AND user_id = ?`, [convId, uid]);
            if (existing && !existing.removed_at)
                continue; // already active
            if (existing) {
                // Re-add previously removed member
                await db.execute(`UPDATE conversation_members SET removed_at = NULL, joined_at = NOW()
           WHERE conversation_id = ? AND user_id = ?`, [convId, uid]);
            }
            else {
                await db.execute(`INSERT INTO conversation_members (conversation_id, user_id, role, joined_at)
           VALUES (?, ?, 'member', NOW())`, [convId, uid]);
            }
            added++;
        }
 ...
 */
staffChatRouter.delete('/conversations/:id/members/:userId', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const convId = req.params.id;
        ...
        await db.execute(`UPDATE conversation_members SET removed_at = NOW()
       WHERE conversation_id = ? AND user_id = ? AND removed_at IS NULL`, [convId, targetId]);
        res.json({ success: true });
 ...
 */
staffChatRouter.patch('/conversations/:id/members/me', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const convId = req.params.id;
        ...
        await db.execute(`UPDATE conversation_members SET ${updates.join(', ')}
       WHERE conversation_id = ? AND user_id = ? AND removed_at IS NULL`, params);
        res.json({ success: true });
    }
    catch (err) {
        next(err);
 ...
 */
staffChatRouter.get('/conversations/:id/messages', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const convId = req.params.id;
        ...
        await requireMembership(convId, userId);
        let cursorWhere = '';
        const params = [convId, userId, convId, userId];
        if (before) {
      ...
      `, msgIds);
            ...
            for (const msg of messages) {
                ...
                if (msg.link_preview_json && typeof msg.link_preview_json === 'string') {
                    try {
                        msg.link_preview = JSON.parse(msg.link_preview_json);
                    }
                    catch {
                        msg.link_preview = null;
                    }
                }
                else {
 ...
 */
staffChatRouter.post('/conversations/:id/messages', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const convId = req.params.id;
        ...
        await requireMembership(convId, userId);
        // Insert message
        const msgId = await db.insert(`INSERT INTO messages
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())`, [
        ...
        const members = await db.query(`SELECT user_id FROM conversation_members
       WHERE conversation_id = ? AND user_id != ? AND removed_at IS NULL`, [convId, userId]);
        for (const m of members) {
            await db.execute(`INSERT INTO message_status (message_id, user_id, status, timestamp)
         VALUES (?, ?, 'sent', NOW())`, [msgId, m.user_id]);
        }
    ...
    `, [msgId]);
        ...
        `, [message.reply_to_id]);
            ...
            if (message.link_preview_json) {
                try {
                    message.link_preview = JSON.parse(message.link_preview_json);
                }
                catch {
 ...
 */
staffChatRouter.put('/conversations/:id/messages/:msgId', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const convId = req.params.id;
        ...
        await requireMembership(convId, userId);
        const msg = await db.queryOne('SELECT * FROM messages WHERE id = ? AND conversation_id = ?', [msgId, convId]);
        if (!msg)
            throw notFound('Message not found');
        if (msg.sender_id !== userId)
            throw forbidden('Can only edit your own messages');
        // 15-minute window
        const ageMs = Date.now() - new Date(msg.created_at).getTime();
        if (ageMs > 15 * 60 * 1000)
            throw badRequest('Can only edit messages within 15 minutes');
        await db.execute('UPDATE messages SET content = ?, edited_at = NOW() WHERE id = ?', [input.content, msgId]);
        const updated = await db.queryOne('SELECT * FROM messages WHERE id = ?', [msgId]);
        res.json({ success: true, data: updated });
 ...
 */
staffChatRouter.delete('/conversations/:id/messages/:msgId', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const convId = req.params.id;
        const msgId = req.params.msgId;
        ...
        if (deleteFor === 'everyone') {
            if (msg.sender_id !== userId)
                throw forbidden('Can only delete your own messages for everyone');
            const ageMs = Date.now() - new Date(msg.created_at).getTime();
            if (ageMs > 5 * 60 * 1000)
                throw badRequest('Can only delete for everyone within 5 minutes');
            await db.execute('UPDATE messages SET deleted_for_everyone_at = NOW() WHERE id = ?', [msgId]);
            emitMessageDeleted(Number(convId), Number(msgId), true);
        }
        else {
            // Delete for me — insert into deleted_messages
            await db.execute(`INSERT IGNORE INTO deleted_messages (user_id, message_id, deleted_at)
         VALUES (?, ?, NOW())`, [userId, msgId]);
        }
        res.json({ success: true });
    }
    catch (err) {
 ...
 */
staffChatRouter.post('/messages/:msgId/forward', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const msgId = req.params.msgId;
        const input = forwardSchema.parse(req.body);
        const original = await db.queryOne('SELECT * FROM messages WHERE id = ?', [msgId]);
        if (!original)
            throw notFound('Message not found');
        ...
        for (const targetConvId of input.conversation_ids) {
            ...
            const newMsgId = await db.insert(`INSERT INTO messages
           (conversation_id, sender_id, content, message_type,
            file_url, file_name, file_type, file_size, thumbnail_url,
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())`, [
            ...
            const members = await db.query(`SELECT user_id FROM conversation_members
         WHERE conversation_id = ? AND user_id != ? AND removed_at IS NULL`, [targetConvId, userId]);
            for (const m of members) {
                await db.execute(`INSERT INTO message_status (message_id, user_id, status, timestamp) VALUES (?, ?, 'sent', NOW())`, [newMsgId, m.user_id]);
            }
            const fwdMsg = await db.queryOne(`
 ...
 */
staffChatRouter.post('/messages/:msgId/reactions', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const msgId = req.params.msgId;
        const input = reactionSchema.parse(req.body);
        const msg = await db.queryOne('SELECT conversation_id FROM messages WHERE id = ?', [msgId]);
        if (!msg)
            throw notFound('Message not found');
        await requireMembership(msg.conversation_id, userId);
        // Toggle: if exists → remove, else → add
        const existing = await db.queryOne(`SELECT id FROM message_reactions WHERE message_id = ? AND user_id = ? AND emoji = ?`, [msgId, userId, input.emoji]);
        if (existing) {
            await db.execute('DELETE FROM message_reactions WHERE id = ?', [existing.id]);
        }
        else {
            await db.execute(`INSERT INTO message_reactions (message_id, user_id, emoji, created_at) VALUES (?, ?, ?, NOW())`, [msgId, userId, input.emoji]);
        }
 ...
 */
staffChatRouter.get('/messages/:msgId/reactions', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const msgId = req.params.msgId;
        const msg = await db.queryOne('SELECT conversation_id FROM messages WHERE id = ?', [msgId]);
 ...
 */
staffChatRouter.post('/messages/:msgId/star', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const msgId = req.params.msgId;
        const msg = await db.queryOne('SELECT conversation_id FROM messages WHERE id = ?', [msgId]);
        if (!msg)
            throw notFound('Message not found');
        await requireMembership(msg.conversation_id, userId);
        const existing = await db.queryOne('SELECT 1 FROM starred_messages WHERE user_id = ? AND message_id = ?', [userId, msgId]);
        if (existing) {
            await db.execute('DELETE FROM starred_messages WHERE user_id = ? AND message_id = ?', [userId, msgId]);
            res.json({ success: true, starred: false });
        }
        else {
            await db.execute('INSERT INTO starred_messages (user_id, message_id, created_at) VALUES (?, ?, NOW())', [userId, msgId]);
 ...
 */
staffChatRouter.get('/starred', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const starred = await db.query(`
      SELECT
 ...
 */
staffChatRouter.post('/conversations/:id/read', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const convId = req.params.id;
        ...
        if (input.message_id) {
            await db.execute(`UPDATE message_status ms
         ...
         WHERE m.conversation_id = ? AND ms.user_id = ? AND m.id <= ?
           AND ms.status != 'read'`, [convId, userId, input.message_id]);
            await db.execute(`UPDATE conversation_members SET last_read_message_id = ?
         WHERE conversation_id = ? AND user_id = ?`, [input.message_id, convId, userId]);
        }
        else {
         ...
         INNER JOIN messages m ON m.id = ms.message_id
         SET ms.status = 'read', ms.timestamp = NOW()
         WHERE m.conversation_id = ? AND ms.user_id = ? AND ms.status != 'read'`, [convId, userId]);
            // Set last_read to the latest message
            const latest = await db.queryOne(`SELECT id FROM messages WHERE conversation_id = ? ORDER BY id DESC LIMIT 1`, [convId]);
            if (latest) {
                await db.execute(`UPDATE conversation_members SET last_read_message_id = ?
           WHERE conversation_id = ? AND user_id = ?`, [latest.id, convId, userId]);
            }
        }
        res.json({ success: true });
        // Notify senders that their messages were read
        const senders = await db.query(`SELECT DISTINCT m.sender_id, m.id FROM messages m
       WHERE m.conversation_id = ? AND m.id <= ? AND m.sender_id != ?`, [convId, input.message_id, userId]);
 ...
 */
staffChatRouter.get('/search', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
 ...
 */
staffChatRouter.get('/conversations/:id/search', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const convId = req.params.id;
        const q = (req.query.q || '').trim();
 ...
 */
staffChatRouter.get('/conversations/:id/media', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const convId = req.params.id;
        const mediaType = req.query.type || 'images';
        ...
        switch (mediaType) {
            case 'images':
                typeFilter = "m.message_type = 'image'";
                break;
            case 'videos':
                typeFilter = "m.message_type = 'video'";
                break;
            case 'docs':
                typeFilter = "m.message_type = 'file'";
                break;
            case 'links':
                typeFilter = "m.link_preview_json IS NOT NULL";
                break;
 ...
 */
staffChatRouter.get('/users/:id/profile', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
    ...
    `, [userId, targetId]);
        // Presence
        const presence = await db.queryOne('SELECT status, last_seen_at FROM user_presence WHERE user_id = ?', [targetId]);
        res.json({
            success: true,
            data: {
                user: {
 ...
 */
staffChatRouter.get('/sync', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const since = req.query.since;
        if (!since)
            throw badRequest('since parameter required');
        // New messages
    ...
    `, [userId, since, userId]);
        // Edited messages
        const editedMessages = await db.query(`
    ...
    `, [userId, since]);
        ...
        const deletedIds = await db.query(`SELECT m.id FROM messages m
       INNER JOIN conversation_members cm ON cm.conversation_id = m.conversation_id
         AND cm.user_id = ? AND cm.removed_at IS NULL
       WHERE m.deleted_for_everyone_at > ?`, [userId, since]);
        // Status updates
        const statusUpdates = await db.query(`SELECT ms.message_id, ms.status, ms.timestamp
       FROM message_status ms
       INNER JOIN messages m ON m.id = ms.message_id AND m.sender_id = ?
       WHERE ms.timestamp > ?`, [userId, since]);
        res.json({
            success: true,
            data: {
 ...
 */
staffChatRouter.get('/dnd', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const row = await db.queryOne(`SELECT dnd_enabled, dnd_start, dnd_end FROM user_presence WHERE user_id = ?`, [userId]);
        res.json({
            data: {
...
staffChatRouter.put('/dnd', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const body = dndSchema.parse(req.body);
        // Ensure user_presence row exists
        const exists = await db.queryOne('SELECT 1 FROM user_presence WHERE user_id = ?', [userId]);
        if (!exists) {
            await db.execute(`INSERT INTO user_presence (user_id, status, last_seen_at, dnd_enabled, dnd_start, dnd_end)
         VALUES (?, 'online', NOW(), ?, ?, ?)`, [userId, body.enabled ? 1 : 0, body.start ? `${body.start}:00` : null, body.end ? `${body.end}:00` : null]);
        }
        else {
            await db.execute(`UPDATE user_presence SET dnd_enabled = ?, dnd_start = ?, dnd_end = ? WHERE user_id = ?`, [body.enabled ? 1 : 0, body.start ? `${body.start}:00` : null, body.end ? `${body.end}:00` : null, userId]);
        }
 ...
 */
staffChatRouter.put('/conversations/:id/notification-sound', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const convId = req.params.id;
        const { sound } = req.body; // string | null
        await requireMembership(convId, userId);
        await db.execute(`UPDATE conversation_members SET notification_sound = ? WHERE conversation_id = ? AND user_id = ?`, [sound || null, convId, userId]);
 ...
 */
staffChatRouter.post('/conversations/:id/upload', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const convId = req.params.id;
        await requireMembership(convId, userId);
 ...
 */
staffChatRouter.post('/profile/avatar', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const { file_data, file_name } = req.body;
        if (!file_data)
            throw badRequest('file_data (base64) required');
        const base64Data = file_data.replace(/^data:[^;]+;base64,/, '');
        const buffer = Buffer.from(base64Data, 'base64');
        const __dirname = path.dirname(fileURLToPath(import.meta.url));
 ...
 */
staffChatRouter.post('/messages/:msgId/report', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const msgId = req.params.msgId;
        const input = reportSchema.parse(req.body);
        const msg = await db.queryOne('SELECT * FROM messages WHERE id = ?', [msgId]);
        if (!msg)
            throw notFound('Message not found');
        await requireMembership(msg.conversation_id, userId);
        // Create a case in the existing cases system (if table exists)
        try {
            await db.insert(`INSERT INTO cases (title, description, status, priority, created_by, created_at)
...
async function getConversationDetail(convId, requestingUserId) {
    const conv = await db.queryOne('SELECT * FROM conversations WHERE id = ?', [convId]);
    if (!conv)
        return null;
...
async function getMessageReactions(msgId, requestingUserId) {
 ...
 */
staffChatRouter.get('/gifs', async (req, res, next) => {
    try {
 ...
 */
staffChatRouter.get('/link-preview', async (req, res, next) => {
    try {
        const url = req.query.url;
        if (!url)
            throw badRequest('url query parameter required');
 ...
 */
staffChatRouter.get('/calls/ice-config', async (_req, res, next) => {
    try {
        const iceServers = [
 ...
 */
staffChatRouter.post('/calls/initiate', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        ...
        await db.execute(`UPDATE call_sessions SET status = 'missed', ended_at = NOW()
       WHERE conversation_id = ? AND status = 'ringing'
         AND started_at < DATE_SUB(NOW(), INTERVAL 60 SECOND)`, [parsed.conversation_id]);
        const existing = await db.queryOne(`SELECT id FROM call_sessions
       WHERE conversation_id = ? AND status IN ('ringing', 'active')`, [parsed.conversation_id]);
        if (existing)
            throw badRequest('There is already an active call in this conversation');
        // Create call session
        const callId = await db.insert(`INSERT INTO call_sessions (conversation_id, type, initiated_by, status)
       VALUES (?, ?, ?, 'ringing')`, [parsed.conversation_id, parsed.call_type, userId]);
        // Add all members as participants
        const members = await db.query(`SELECT user_id FROM conversation_members
       WHERE conversation_id = ? AND removed_at IS NULL`, [parsed.conversation_id]);
        for (const m of members) {
            const joinedAt = m.user_id === userId ? 'NOW()' : null;
            await db.execute(`INSERT INTO call_participants (call_id, user_id, joined_at)
         VALUES (?, ?, ${joinedAt ? 'NOW()' : 'NULL'})`, [callId, m.user_id]);
        }
        // Get caller info
        const caller = await db.queryOne(`SELECT name, email, avatar_url FROM users WHERE id = ?`, [userId]);
        const callerName = displayName(caller || {});
        // Get conversation name for push notifications
        const conv = await db.queryOne(`SELECT name, type FROM conversations WHERE id = ?`, [parsed.conversation_id]);
        ...
        setTimeout(async () => {
            try {
                const session = await db.queryOne(`SELECT status FROM call_sessions WHERE id = ?`, [callId]);
 ...
 */
staffChatRouter.post('/calls/:id/accept', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const callId = req.params.id;
        // Update call status to active (if still ringing), and set started_at to now
        await db.execute(`UPDATE call_sessions SET status = 'active', started_at = NOW() WHERE id = ? AND status = 'ringing'`, [callId]);
        // Update participant joined_at
        await db.execute(`UPDATE call_participants SET joined_at = NOW() WHERE call_id = ? AND user_id = ? AND joined_at IS NULL`, [callId, userId]);
        res.json({ success: true });
    }
    catch (err) {
        next(err);
 ...
 */
staffChatRouter.get('/calls/history', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const limit = Math.min(Number(req.query.limit) || 50, 100);
       ...
       LIMIT ? OFFSET ?`, [userId, limit, offset]);
        ...
        for (const call of calls) {
            if (call.conversation_type === 'direct') {
                const other = await db.queryOne(`SELECT u.id, u.name, u.avatar_url
           FROM conversation_members cm
           JOIN users u ON u.id = cm.user_id
           WHERE cm.conversation_id = ? AND cm.user_id != ? AND cm.removed_at IS NULL
           LIMIT 1`, [call.conversation_id, userId]);
                if (other) {
 ...
 */
staffChatRouter.get('/calls/:id', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const callId = req.params.id;
        const call = await db.queryOne(`SELECT
        cs.*,
        c.name AS conversation_name,
        c.type AS conversation_type,
        u.name AS caller_name,
       ...
       WHERE cs.id = ?`, [callId]);
        ...
        const participant = await db.queryOne(`SELECT * FROM call_participants WHERE call_id = ? AND user_id = ?`, [callId, userId]);
        if (!participant)
            throw forbidden('Not a participant of this call');
        // Get all participants
        const participants = await db.query(`SELECT
        cp.*,
 ...
 */
staffChatRouter.post('/calls/:id/end', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const callId = req.params.id;
        const session = await db.queryOne(`SELECT * FROM call_sessions WHERE id = ?`, [callId]);
        if (!session)
            throw notFound('Call not found');
        if (session.status === 'ended')
            throw badRequest('Call already ended');
       ...
       WHERE id = ?`, [durationSeconds, callId]);
        await db.execute(`UPDATE call_participants SET left_at = NOW() WHERE call_id = ? AND left_at IS NULL`, [callId]);
 ...
 */
staffChatRouter.post('/scheduled-calls', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        ...
        const membership = await db.queryOne(`SELECT user_id FROM conversation_members WHERE conversation_id = ? AND user_id = ? AND removed_at IS NULL`, [parsed.conversation_id, userId]);
        if (!membership)
            throw forbidden('Not a member of this conversation');
        ...
        if (parsed.participant_ids && parsed.participant_ids.length > 0) {
            ...
            const members = await db.query(`SELECT user_id FROM conversation_members WHERE conversation_id = ? AND removed_at IS NULL`, [parsed.conversation_id]);
        ...
        const { scId, result } = await db.transaction(async (conn) => {
            ...
            const [insertResult] = await conn.query(`INSERT INTO scheduled_calls
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'scheduled')`, [
            ...
            await conn.query(`INSERT INTO messages (conversation_id, sender_id, content, message_type, created_at)
         VALUES (?, ?, ?, 'system', NOW())`, [
                parsed.conversation_id,
                userId,
            ...
            const [scheduledRows] = await conn.query(`SELECT sc.*, u.name AS creator_name, u.email AS creator_email
         FROM scheduled_calls sc
         LEFT JOIN users u ON u.id = sc.created_by
         WHERE sc.id = ?`, [scId]);
            const scheduled = scheduledRows[0];
            const [participantRows] = await conn.query(`SELECT scp.user_id, scp.rsvp, scp.created_at,
                u.name, u.email, u.avatar_url
         FROM scheduled_call_participants scp
         LEFT JOIN users u ON u.id = scp.user_id
         WHERE scp.scheduled_call_id = ?`, [scId]);
            return { scId, result: { ...scheduled, participants: participantRows } };
        });
 ...
 */
staffChatRouter.get('/scheduled-calls', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
       ...
       LIMIT ? OFFSET ?`, [userId, ...statusParams, ...convParams, limit, offset]);
        // Enrich DM calls with other-user info (same pattern as call history)
        for (const call of calls) {
            if (call.conversation_type === 'direct') {
                const other = await db.queryOne(`SELECT u.id, u.name, u.email, u.avatar_url
           FROM conversation_members cm
           JOIN users u ON u.id = cm.user_id
           WHERE cm.conversation_id = ? AND cm.user_id != ? AND cm.removed_at IS NULL
           LIMIT 1`, [call.conversation_id, userId]);
 ...
 */
staffChatRouter.get('/scheduled-calls/:id', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const scId = req.params.id;
        const call = await db.queryOne(`SELECT sc.*, u.name AS creator_name, u.email AS creator_email, u.avatar_url AS creator_avatar,
              c.name AS conversation_name, c.type AS conversation_type
       FROM scheduled_calls sc
       JOIN conversations c ON c.id = sc.conversation_id
       LEFT JOIN users u ON u.id = sc.created_by
       WHERE sc.id = ?`, [scId]);
        if (!call)
            throw notFound('Scheduled call not found');
        // Verify user is a participant
        const myParticipant = await db.queryOne(`SELECT * FROM scheduled_call_participants WHERE scheduled_call_id = ? AND user_id = ?`, [scId, userId]);
        if (!myParticipant)
            throw forbidden('Not a participant of this scheduled call');
        const participants = await db.query(`SELECT scp.user_id, scp.rsvp, scp.created_at,
              u.name, u.email, u.avatar_url
       FROM scheduled_call_participants scp
       LEFT JOIN users u ON u.id = scp.user_id
       WHERE scp.scheduled_call_id = ?`, [scId]);
 ...
 */
staffChatRouter.put('/scheduled-calls/:id', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const scId = req.params.id;
        const call = await db.queryOne(`SELECT * FROM scheduled_calls WHERE id = ?`, [scId]);
        if (!call)
            throw notFound('Scheduled call not found');
        if (call.created_by !== userId)
            throw forbidden('Only the creator can update this call');
        if (call.status !== 'scheduled')
            throw badRequest('Cannot update a call that is not scheduled');
        const parsed = z.object({
        ...
        if (parsed.description !== undefined) {
            ...
            vals.push(parsed.description);
        }
        if (parsed.call_type !== undefined) {
            sets.push('call_type = ?');
            vals.push(parsed.call_type);
        }
        if (parsed.screen_share !== undefined) {
            sets.push('screen_share = ?');
            vals.push(parsed.screen_share ? 1 : 0);
        }
        if (parsed.scheduled_at !== undefined) {
            sets.push('scheduled_at = ?');
            vals.push(parsed.scheduled_at.replace('T', ' ').replace('Z', ''));
        }
        if (parsed.duration_minutes !== undefined) {
            sets.push('duration_minutes = ?');
            vals.push(parsed.duration_minutes);
        }
        if (parsed.recurrence !== undefined) {
            sets.push('recurrence = ?');
            vals.push(parsed.recurrence);
        }
        if (parsed.recurrence_end !== undefined) {
            sets.push('recurrence_end = ?');
            vals.push(parsed.recurrence_end ? parsed.recurrence_end.replace('T', ' ').replace('Z', '') : null);
        }
        if (sets.length > 0) {
            vals.push(scId);
        ...
        const updated = await db.queryOne(`SELECT sc.*, u.name AS creator_name, u.email AS creator_email, u.avatar_url AS creator_avatar
       FROM scheduled_calls sc
       LEFT JOIN users u ON u.id = sc.created_by
       WHERE sc.id = ?`, [scId]);
 ...
 */
staffChatRouter.delete('/scheduled-calls/:id', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const scId = req.params.id;
        const call = await db.queryOne(`SELECT * FROM scheduled_calls WHERE id = ?`, [scId]);
        if (!call)
            throw notFound('Scheduled call not found');
        if (call.created_by !== userId)
            throw forbidden('Only the creator can cancel this call');
        if (call.status !== 'scheduled')
            throw badRequest('Cannot cancel a call that is not scheduled');
        ...
        // Notify participants
        const participants = await db.query(`SELECT user_id FROM scheduled_call_participants WHERE scheduled_call_id = ?`, [scId]);
        const creator = await db.queryOne(`SELECT name, email FROM users WHERE id = ?`, [userId]);
        const creatorName = creator?.name || creator?.email || 'Someone';
 ...
 */
staffChatRouter.post('/scheduled-calls/:id/rsvp', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const scId = req.params.id;
        ...
        if (!call)
            throw notFound('Scheduled call not found');
        if (call.status !== 'scheduled')
            throw badRequest('Cannot RSVP to a call that is not in scheduled state');
        const participant = await db.queryOne(`SELECT * FROM scheduled_call_participants WHERE scheduled_call_id = ? AND user_id = ?`, [scId, userId]);
        if (!participant)
            throw forbidden('Not a participant of this call');
        await db.execute(`UPDATE scheduled_call_participants SET rsvp = ? WHERE scheduled_call_id = ? AND user_id = ?`, [rsvp, scId, userId]);
        const user = await db.queryOne(`SELECT name, email FROM users WHERE id = ?`, [userId]);
 ...
 */
staffChatRouter.post('/scheduled-calls/:id/start', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const scId = req.params.id;
        const scheduled = await db.queryOne(`SELECT * FROM scheduled_calls WHERE id = ?`, [scId]);
        if (!scheduled)
            throw notFound('Scheduled call not found');
        if (scheduled.status !== 'scheduled')
            throw badRequest('Call is not in scheduled state');
        // Verify user is a participant
        const participant = await db.queryOne(`SELECT * FROM scheduled_call_participants WHERE scheduled_call_id = ? AND user_id = ?`, [scId, userId]);
        if (!participant)
            throw forbidden('Not a participant of this call');
        // Check if there's already an active call in the conversation
        const existing = await db.queryOne(`SELECT id FROM call_sessions WHERE conversation_id = ? AND status IN ('ringing', 'active')`, [scheduled.conversation_id]);
        if (existing)
            throw badRequest('There is already an active call in this conversation');
        // Create the real call session
        const callId = await db.insert(`INSERT INTO call_sessions (conversation_id, type, initiated_by, status) VALUES (?, ?, ?, 'ringing')`, [scheduled.conversation_id, scheduled.call_type, userId]);
        // Add accepted participants to call_participants
        const acceptedParticipants = await db.query(`SELECT user_id FROM scheduled_call_participants WHERE scheduled_call_id = ? AND rsvp = 'accepted'`, [scId]);
        for (const p of acceptedParticipants) {
            await db.execute(`INSERT INTO call_participants (call_id, user_id, joined_at) VALUES (?, ?, ?)`, [callId, p.user_id, p.user_id === userId ? new Date().toISOString().replace('T', ' ').slice(0, 19) : null]);
        }
        // Update scheduled call to active + link to call_session
        await db.execute(`UPDATE scheduled_calls SET status = 'active', call_session_id = ? WHERE id = ?`, [callId, scId]);
        // Emit ringing to conversation
        const caller = await db.queryOne(`SELECT name, email FROM users WHERE id = ?`, [userId]);
 ...
 */
staffChatRouter.post('/scheduled-calls/:id/participants', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const scId = req.params.id;
        ...
        if (!call)
            throw notFound('Scheduled call not found');
        if (call.created_by !== userId)
            throw forbidden('Only the creator can manage participants');
        if (call.status !== 'scheduled')
            throw badRequest('Cannot modify participants of a non-scheduled call');
        // Verify all user_ids are conversation members
        const members = await db.query(`SELECT user_id FROM conversation_members WHERE conversation_id = ? AND removed_at IS NULL AND user_id IN (${user_ids.map(() => '?').join(',')})`, [call.conversation_id, ...user_ids]);
        const validIds = new Set(members.map(m => m.user_id));
        const toAdd = user_ids.filter(id => validIds.has(id));
        if (toAdd.length === 0)
            throw badRequest('No valid conversation members in the provided list');
        ...
        await db.execute(`INSERT IGNORE INTO scheduled_call_participants (scheduled_call_id, user_id, rsvp) VALUES ${placeholders}`, values);
 ...
 */
staffChatRouter.delete('/scheduled-calls/:id/participants/:userId', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const scId = req.params.id;
        ...
        if (targetUserId === userId)
            throw badRequest('Cannot remove yourself from the call');
        const affected = await db.execute(`DELETE FROM scheduled_call_participants WHERE scheduled_call_id = ? AND user_id = ?`, [scId, targetUserId]);
        if (affected === 0)
...
function startScheduledCallMaintenance() {
    if (_cleanupInterval)
        return;
    _cleanupInterval = setInterval(async () => {
        try {
            // Issue #6: Clean up stale ringing sessions
            const stale = await db.query(`SELECT cs.id AS call_id, cs.conversation_id, sc.id AS sc_id
         FROM call_sessions cs
         ...
         WHERE cs.status = 'ringing' AND cs.started_at < NOW() - INTERVAL 2 MINUTE`);
            ...
            const upcoming = await db.query(`SELECT sc.id, sc.conversation_id, sc.title, sc.call_type, sc.scheduled_at, sc.screen_share
         FROM scheduled_calls sc
         WHERE sc.status = 'scheduled'
           ...
           AND sc.scheduled_at > NOW()`);
            for (const call of upcoming) {
                ...
                const participants = await db.query(`SELECT user_id FROM scheduled_call_participants WHERE scheduled_call_id = ? AND rsvp != 'declined'`, [call.id]);
            ...
            const completedRecurring = await db.query(`SELECT * FROM scheduled_calls
           ...
           AND (recurrence_end IS NULL OR recurrence_end > NOW())`);
            for (const call of completedRecurring) {
                ...
                switch (call.recurrence) {
                    case 'daily':
                        next = new Date(prev.getTime() + 86400000);
                        break;
                    case 'weekly':
                        next = new Date(prev.getTime() + 7 * 86400000);
                        break;
                    case 'biweekly':
                        next = new Date(prev.getTime() + 14 * 86400000);
                        break;
                    case 'monthly':
                        next = new Date(prev);
                ...
                const existing = await db.queryOne(`SELECT id FROM scheduled_calls
           WHERE conversation_id = ? AND created_by = ? AND title = ? AND status = 'scheduled'
             AND scheduled_at = ?`, [call.conversation_id, call.created_by, call.title, next.toISOString().slice(0, 19).replace('T', ' ')]);
                if (existing)
                    continue;
                // Create next occurrence
                const nextId = await db.insert(`INSERT INTO scheduled_calls
            (conversation_id, created_by, title, description, call_type, screen_share,
             scheduled_at, duration_minutes, recurrence, recurrence_end, status)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'scheduled')`, [
                ...
                const participants = await db.query(`SELECT user_id, rsvp FROM scheduled_call_participants WHERE scheduled_call_id = ?`, [call.id]);
                if (participants.length > 0) {
                    const placeholders = participants.map(() => '(?, ?, ?)').join(', ');
                    const values = participants.flatMap(p => [nextId, p.user_id, p.user_id === call.created_by ? 'accepted' : 'pending']);
                    await db.execute(`INSERT INTO scheduled_call_participants (scheduled_call_id, user_id, rsvp) VALUES ${placeholders}`, values);
                }
 ...
 */
async function generateLinkPreview(messageId, conversationId, content) {
    try {
        const preview = await getLinkPreviewForContent(content);
        if (!preview)
            return;
        await db.execute('UPDATE messages SET link_preview_json = ? WHERE id = ?', [JSON.stringify(preview), messageId]);
...
async function pushToOfflineMembers(memberIds, conversationId, message, senderId) {
        ...
        try {
            ...
            if (nameMentions) {
                const names = nameMentions
                    ...
                    .filter((n) => n.length >= 2 && !/^[0-9a-f]{8}-/.test(n)); // skip UUIDs
                for (const name of names) {
                    const user = await db.queryOne(`SELECT id FROM users
    ...
    for (const memberId of memberIds) {
        try {
            ...
            if (!isMentioned) {
                const membership = await db.queryOne(`SELECT muted_until, notification_sound FROM conversation_members
           WHERE conversation_id = ? AND user_id = ? AND removed_at IS NULL`, [conversationId, memberId]);
                ...
                const membership = await db.queryOne(`SELECT notification_sound FROM conversation_members
           WHERE conversation_id = ? AND user_id = ? AND removed_at IS NULL`, [conversationId, memberId]);
                notificationSound = membership?.notification_sound || null;
            }
            // Check DND schedule (but skip if @mentioned)
            if (!isMentioned) {
                const dnd = await db.queryOne(`SELECT dnd_enabled, dnd_start, dnd_end FROM user_presence WHERE user_id = ?`, [memberId]);
                if (dnd?.dnd_enabled && dnd.dnd_start && dnd.dnd_end) {
                    ...
                    const inDnd = start <= end
                        ...
                        : (nowTime >= start || nowTime < end);
                    if (inDnd)
                        continue; // In DND window — skip push
                }
            }
            // Check if online
            const online = await isUserOnline(memberId);
            if (online)
                continue; // connected to socket — skip push
