...
quotationsRouter.get('/', requireAuth, async (req, res, next) => {
    try {
        ...
        const page = parseInt(req.query.page) >= 0 ? parseInt(req.query.page) + 1 : 1;
        const limit = parseInt(req.query.limit) || 50;
        const offset = (page - 1) * limit;
        const search = req.query.search || '';
        const sortBy = req.query.sortBy || 'quotation_id';
 ...
 */
quotationsRouter.get('/:id', requireAuth, async (req, res, next) => {
    try {
        const { id } = req.params;
        const quotation = await db.queryOne(`SELECT ${QUOTATION_SELECT}
       FROM quotations q
       LEFT JOIN contacts c ON c.id = q.contact_id
       WHERE q.id = ?`, [id]);
        if (!quotation) {
        ...
        const items = await db.query(`SELECT id AS item_id, quotation_id AS item_quote_id,
              ...
              0 AS item_profit,
              0 AS item_cost
       FROM quote_items WHERE quotation_id = ? ORDER BY id`, [id]);
 ...
 */
quotationsRouter.post('/', requireAuth, async (req, res, next) => {
    try {
        ...
        if (!quotationNumber) {
            ...
            const lastQuote = await db.queryOne('SELECT quotation_number FROM quotations WHERE quotation_number LIKE "QUO-%" ORDER BY id DESC LIMIT 1');
            let nextNumber = 1;
 ...
 */
quotationsRouter.put('/:id', requireAuth, async (req, res, next) => {
    try {
        const { id } = req.params;
        ...
        if (totalAmount !== undefined)
            updateData.quotation_amount = totalAmount;
        if (data.quotation_date)
            updateData.quotation_date = data.quotation_date;
        if (data.quotation_notes !== undefined || data.remarks !== undefined)
            updateData.remarks = data.quotation_notes || data.remarks || null;
        if (data.quotation_status !== undefined)
            updateData.active = data.quotation_status;
        await db.execute(`UPDATE quotations SET ${Object.keys(updateData).map(k => `${k} = ?`).join(', ')} WHERE id = ?`, [...Object.values(updateData), id]);
        ...
        const updated = await db.queryOne(`SELECT ${QUOTATION_SELECT}
       FROM quotations q
       LEFT JOIN contacts c ON c.id = q.contact_id
       WHERE q.id = ?`, [id]);
 ...
 */
quotationsRouter.delete('/:id', requireAuth, async (req, res, next) => {
    try {
        const { id } = req.params;
        const quotation = await db.queryOne('SELECT id FROM quotations WHERE id = ?', [id]);
        if (!quotation) {
        ...
        await db.execute('UPDATE quotations SET active = 0, updated_at = ? WHERE id = ?', [toMySQLDate(new Date()), id]);
 ...
 */
quotationsRouter.post('/:id/items', requireAuth, async (req, res, next) => {
    try {
        const { id } = req.params;
        ...
        const item = await db.queryOne('SELECT * FROM quote_items WHERE id = ?', [insertId]);
 ...
 */
quotationsRouter.delete('/:id/items/:itemId', requireAuth, async (req, res, next) => {
    try {
        const { id, itemId } = req.params;
        const item = await db.queryOne('SELECT id FROM quote_items WHERE id = ? AND quotation_id = ?', [itemId, id]);
        if (!item) {
 ...
 */
quotationsRouter.post('/:id/generate-pdf', requireAuth, async (req, res, next) => {
    try {
        const { id } = req.params;
        const quotation = await db.queryOne(`SELECT ${QUOTATION_SELECT}
       FROM quotations q
 ...
 */
quotationsRouter.post('/:id/send-email', requireAuth, async (req, res, next) => {
    try {
        const { id } = req.params;
 ...
 */
quotationsRouter.post('/:id/convert-to-invoice', requireAuth, async (req, res, next) => {
    try {
        const { id } = req.params;
        ...
        const quotation = await db.queryOne('SELECT * FROM quotations WHERE id = ?', [id]);
        if (!quotation) {
            throw notFound('Quotation not found');
        }
        const quoteItems = await db.query('SELECT * FROM quote_items WHERE quotation_id = ?', [id]);
        // Start transaction
        ...
        try {
            ...
            const invoice = await db.queryOne('SELECT * FROM invoices WHERE id = ?', [invoiceId]);
            res.status(201).json({
                success: true,
