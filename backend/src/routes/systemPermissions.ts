 ...
 */
permissionsRouter.get('/user', requireAuth, async (req: AuthRequest, res: Response, next) => {
  try {
...
permissionsRouter.get('/:id', requireAuth, async (req: AuthRequest, res: Response, next) => {
  try {
    ...
    if (!perm) throw notFound('Permission not found');
...
permissionsRouter.post('/', requireAuth, async (req: AuthRequest, res: Response, next) => {
  try {
    ...
    const existing = await db.queryOne<any>('SELECT id FROM permissions WHERE slug = ?', [slug]);
    if (existing) throw badRequest('Permission slug already exists');
...
permissionsRouter.put('/:id', requireAuth, async (req: AuthRequest, res: Response, next) => {
  try {
    ...
    const updated = await db.queryOne<any>('SELECT * FROM permissions WHERE id = ?', [id]);
...
permissionsRouter.post('/:id/assign', requireAuth, async (req: AuthRequest, res: Response, next) => {
  try {
    ...
    const perm = await db.queryOne<any>('SELECT id FROM permissions WHERE id = ?', [id]);
    if (!perm) throw notFound('Permission not found');
