...
import { registerFcmToken, unregisterFcmToken, listFcmTokens, isFirebaseEnabled, } from '../services/firebaseService.js';
...
fcmTokensRouter.post('/', requireAuth, async (req, res, next) => {
    try {
...
fcmTokensRouter.get('/', requireAuth, async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
...
fcmTokensRouter.delete('/:token', requireAuth, async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
...
fcmTokensRouter.get('/status', requireAuth, async (_req, res) => {
    res.json({
        success: true,
        data: {
            fcm_enabled: isFirebaseEnabled(),
