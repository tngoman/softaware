...
// ═════════════════════════════════════════════════════════════════════════
adminCasesRouter.get('/', async (req, res) => {
    try {
...
// ═════════════════════════════════════════════════════════════════════════
adminCasesRouter.get('/analytics', async (_req, res) => {
    try {
        // Core counts
        const totals = await db.queryOne(`SELECT 
        COUNT(*) as totalCases,
       ...
       FROM cases`);
        ...
        const resTime = await db.queryOne(`SELECT AVG(TIMESTAMPDIFF(HOUR, created_at, resolved_at)) as avg_hours
       FROM cases WHERE resolved_at IS NOT NULL`);
        ...
        const avgResolutionTime = avgHours >= 24
            ...
            : `${avgHours}h`;
        // Cases by severity
        const bySeverity = await db.query(`SELECT severity, COUNT(*) as count FROM cases GROUP BY severity`);
        // Cases by category
        const byCategory = await db.query(`SELECT COALESCE(category, 'other') as category, COUNT(*) as count FROM cases GROUP BY category`);
        // Cases by status
        const byStatus = await db.query(`SELECT status, COUNT(*) as count FROM cases GROUP BY status`);
        // Recent trend (last 14 days)
        const recentTrend = await db.query(`SELECT DATE(created_at) as date, COUNT(*) as count
       FROM cases
...
// ═════════════════════════════════════════════════════════════════════════
adminCasesRouter.post('/bulk-assign', async (req, res) => {
    try {
        ...
        const assigner = await db.queryOne('SELECT COALESCE(name, email) AS name FROM users WHERE id = ?', [userId]);
        const assignerName = assigner?.name || 'An admin';
        for (const caseId of case_ids) {
            await db.execute('UPDATE cases SET assigned_to = ?, updated_at = ? WHERE id = ?', [assigned_to, now, caseId]);
            await db.execute('INSERT INTO case_activity (id, case_id, user_id, action, new_value, created_at) VALUES (?, ?, ?, ?, ?, ?)', [generateId(), caseId, userId, 'assigned', assigned_to, now]);
        }
        // Notify assignee about all assigned cases
        const assignedCases = await db.query(`SELECT id, title, case_number FROM cases WHERE id IN (${case_ids.map(() => '?').join(',')})`, case_ids);
...
// ═════════════════════════════════════════════════════════════════════════
adminCasesRouter.post('/bulk-update-status', async (req, res) => {
    try {
        ...
        for (const caseId of case_ids) {
            ...
            await db.execute('INSERT INTO case_activity (id, case_id, user_id, action, new_value, created_at) VALUES (?, ?, ?, ?, ?, ?)', [generateId(), caseId, userId, 'status_changed', status, now]);
        }
        // Notify reporters about status changes on their cases
        const updatedCases = await db.query(`SELECT id, title, case_number, reported_by, assigned_to FROM cases WHERE id IN (${case_ids.map(() => '?').join(',')})`, case_ids);
...
// ═════════════════════════════════════════════════════════════════════════
adminCasesRouter.get('/health', async (_req, res) => {
    try {
...
// ═════════════════════════════════════════════════════════════════════════
adminCasesRouter.post('/health/run-checks', async (_req, res) => {
    try {
...
// ═════════════════════════════════════════════════════════════════════════
adminCasesRouter.post('/bulk-delete', async (req, res) => {
    try {
        ...
        const casesToDelete = case_ids.length > 0
            ? await db.query(`SELECT id, title, case_number, reported_by FROM cases WHERE id IN (${case_ids.map(() => '?').join(',')})`, case_ids)
            : [];
        for (const caseId of case_ids) {
...
// ═════════════════════════════════════════════════════════════════════════
adminCasesRouter.delete('/:id', async (req, res) => {
    try {
