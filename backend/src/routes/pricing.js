 ...
 */
pricingRouter.get('/', requireAuth, async (req, res, next) => {
    try {
 ...
 */
pricingRouter.get('/:id', requireAuth, async (req, res, next) => {
    try {
        const { id } = req.params;
        const item = await db.queryOne(`SELECT 
        p.id as pricing_id,
        p.item_name as pricing_item,
        CASE 
          WHEN p.description LIKE '%|%|%' THEN TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(p.description, ' | ', 2), ' | ', -1))
 ...
 */
pricingRouter.post('/', requireAuth, async (req, res, next) => {
    try {
 ...
 */
pricingRouter.put('/:id', requireAuth, async (req, res, next) => {
    try {
        const { id } = req.params;
        ...
        if (data.pricing_category_id !== undefined)
            dbData.category_id = data.pricing_category_id;
        if (data.pricing_item !== undefined)
            dbData.item_name = data.pricing_item;
        if (data.pricing_price !== undefined)
            dbData.unit_price = data.pricing_price;
        ...
        await db.execute(`UPDATE pricing SET ${Object.keys(dbData).map(k => `${k} = ?`).join(', ')} WHERE id = ?`, [...Object.values(dbData), id]);
        // Return with frontend field names
        const updated = await db.queryOne(`SELECT 
        p.id as pricing_id,
        p.item_name as pricing_item,
        CASE 
          WHEN p.description LIKE '%|%|%' THEN TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(p.description, ' | ', 2), ' | ', -1))
 ...
 */
pricingRouter.delete('/:id', requireAuth, async (req, res, next) => {
    try {
        const { id } = req.params;
        const existing = await db.queryOne('SELECT id FROM pricing WHERE id = ?', [id]);
        if (!existing) {
 ...
 */
pricingRouter.post('/import', requireAuth, async (req, res, next) => {
    try {
        ...
        const existingCats = await db.query('SELECT id, category_name FROM categories');
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            if ((unit === null || unit === '') && (price === null || price === 0 || isNaN(price))) {
            ...
            const existing = await db.queryOne('SELECT id FROM pricing WHERE id = ? OR item_name = ?', [id, description]);
            if (existing) {
                // Update existing
                await db.execute('UPDATE pricing SET item_name = ?, unit_price = ?, description = ?, category_id = ?, updated_at = ? WHERE id = ?', [description, price || 0, descriptionField, categoryId, new Date().toISOString(), existing.id]);
                updated++;
            }
            else {
                // Insert new
