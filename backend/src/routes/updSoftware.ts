...
import type { UpdSoftware } from '../db/updatesTypes.js';
...
updSoftwareRouter.get('/', async (req, res, next) => {
  try {
    ...
    if (id) {
      ...
      if (!sw) throw notFound('Software not found');
      return res.json({ success: true, software: sw });
    }

    const software = await db.query<any>(
      `SELECT s.*,
              u.name AS created_by_name,
              latest.version AS latest_version,
              latest.created_at AS latest_update_date,
...
updSoftwareRouter.post('/', requireAuth, requireAdmin, async (req, res, next) => {
  try {
    ...
    const existing = await db.queryOne<any>(
      'SELECT id FROM update_software WHERE software_key = ?',
      [body.software_key]
    );
    if (existing) throw badRequest('Software key already exists');
...
updSoftwareRouter.put('/', requireAuth, requireAdmin, async (req, res, next) => {
  try {
    ...
    if (!existing) throw notFound('Software not found');

    // Check unique key conflict
    if (body.software_key) {
      const conflict = await db.queryOne<any>(
        'SELECT id FROM update_software WHERE software_key = ? AND id != ?',
        [body.software_key, body.id]
      );
      if (conflict) throw badRequest('Software key already in use');
    }
...
updSoftwareRouter.delete('/', requireAuth, requireAdmin, async (req, res, next) => {
  try {
    ...
    const existing = await db.queryOne<UpdSoftware>('SELECT id FROM update_software WHERE id = ?', [id]);
    if (!existing) throw notFound('Software not found');
