...
updMiscRouter.get('/dashboard', requireAuth, async (_req, res, next) => {
    try {
        const [softwareCount] = await db.query('SELECT COUNT(*) AS cnt FROM update_software');
        const [updateCount] = await db.query('SELECT COUNT(*) AS cnt FROM update_releases');
        const [userCount] = await db.query('SELECT COUNT(*) AS cnt FROM users');
        const [activeClients] = await db.query('SELECT COUNT(*) AS cnt FROM update_clients WHERE last_heartbeat > DATE_SUB(NOW(), INTERVAL 24 HOUR)');
        const latestClients = await db.query(`SELECT c.id, c.software_id, s.name AS software_name,
       ...
       ORDER BY c.last_heartbeat DESC LIMIT 10`);
        const recentUpdates = await db.query(`SELECT u.id, u.software_id, s.name AS software_name,
...
updMiscRouter.get('/installed', async (_req, res, next) => {
    try {
        const installed = await db.query(`SELECT iu.update_id, iu.status, iu.installed_at,
...
updMiscRouter.get('/schema', async (req, res, next) => {
    try {
        ...
        const update = await db.queryOne('SELECT schema_file FROM update_releases WHERE id = ?', [id]);
        if (!update)
            throw notFound('Update not found');
        if (!update.schema_file)
            throw notFound('No schema file for this update');
        const filePath = path.resolve(process.cwd(), 'uploads', update.schema_file);
        if (!fs.existsSync(filePath))
            throw notFound('Schema file not found on disk');
...
updMiscRouter.post('/password_reset', async (req, res, next) => {
    try {
        ...
        const user = await db.queryOne('SELECT id, email, name FROM users WHERE email = ? OR name = ? LIMIT 1', [body.identifier, body.identifier]);
...
updMiscRouter.post('/reset_password', async (req, res, next) => {
    try {
        ...
        const user = await db.queryOne('SELECT id FROM users WHERE email = ? OR name = ? LIMIT 1', [body.identifier, body.identifier]);
        if (!user) {
            return res.status(400).json({ success: false, error: 'Invalid or expired reset code' });
        }
        const token = await db.queryOne('SELECT * FROM update_password_resets WHERE user_id = ? AND token = ? AND used = 0 AND expires_at > NOW() LIMIT 1', [user.id, body.otp]);
