...
paymentsRouter.get('/', requireAuth, async (req, res, next) => {
    try {
        ...
        const invoice_id = req.query.invoice_id ? parseInt(req.query.invoice_id) : undefined;
 ...
 */
paymentsRouter.get('/unprocessed', requireAuth, async (req, res, next) => {
    try {
        const limit = parseInt(req.query.limit) || 50;
        const invoice_id = req.query.invoice_id ? parseInt(req.query.invoice_id) : undefined;
        let query = `
 ...
 */
paymentsRouter.get('/invoice/:invoiceId', requireAuth, async (req, res, next) => {
    try {
        const { invoiceId } = req.params;
        const payments = await db.query(`SELECT ${PAYMENT_SELECT} FROM payments p WHERE p.invoice_id = ? ORDER BY p.payment_date DESC`, [invoiceId]);
        res.json({ success: true, data: payments });
    }
    catch (err) {
        next(err);
...
paymentsRouter.get('/:id', requireAuth, async (req, res, next) => {
    try {
        const { id } = req.params;
        const payment = await db.queryOne(`SELECT ${PAYMENT_SELECT} FROM payments p WHERE p.id = ?`, [id]);
        if (!payment) {
...
paymentsRouter.post('/', requireAuth, async (req, res, next) => {
    try {
        ...
        const invoice = await db.queryOne('SELECT * FROM invoices WHERE id = ?', [data.payment_invoice]);
        if (!invoice) {
        ...
        const totalPaidResult = await db.queryOne('SELECT SUM(payment_amount) as total_paid FROM payments WHERE invoice_id = ?', [data.payment_invoice]);
        const totalPaid = totalPaidResult?.total_paid || 0;
        if (totalPaid >= invoice.invoice_amount) {
            await db.execute('UPDATE invoices SET paid = 1, updated_at = ? WHERE id = ?', [new Date().toISOString(), data.payment_invoice]);
        }
        const payment = await db.queryOne(`SELECT ${PAYMENT_SELECT} FROM payments p WHERE p.id = ?`, [insertId]);
 ...
 */
paymentsRouter.put('/:id', requireAuth, async (req, res, next) => {
    try {
        const { id } = req.params;
        ...
        if (data.payment_invoice !== undefined)
            updateFields.invoice_id = data.payment_invoice;
        if (data.payment_amount !== undefined)
            updateFields.payment_amount = data.payment_amount;
        if (data.payment_date !== undefined)
            updateFields.payment_date = data.payment_date;
        await db.execute(`UPDATE payments SET ${Object.keys(updateFields).map(k => `${k} = ?`).join(', ')} WHERE id = ?`, [...Object.values(updateFields), id]);
        const updated = await db.queryOne(`SELECT ${PAYMENT_SELECT} FROM payments p WHERE p.id = ?`, [id]);
 ...
 */
paymentsRouter.delete('/:id', requireAuth, async (req, res, next) => {
    try {
        const { id } = req.params;
        const existing = await db.queryOne('SELECT id FROM payments WHERE id = ?', [id]);
        if (!existing) {
 ...
 */
paymentsRouter.post('/process', requireAuth, async (req, res, next) => {
    try {
        ...
        if (payment_ids && Array.isArray(payment_ids) && payment_ids.length > 0) {
            ...
            paymentsToProcess = await db.query(`SELECT * FROM payments WHERE id IN (${placeholders})`, payment_ids);
        }
        else if (invoice_id) {
            paymentsToProcess = await db.query('SELECT * FROM payments WHERE invoice_id = ?', [invoice_id]);
        }
        else {
