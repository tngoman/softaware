...
import {
  emitTeamMessage,
  emitTeamUpdated,
  emitTeamMembersChanged,
  emitTeamMessageDeleted,
  emitTeamDeleted,
} from '../services/teamChatSocket.js';
 ...
 */
teamChatRouter.get('/users/available', async (req: AuthRequest, res: Response, next) => {
  try {
...
async function requireMembership(teamId: string, userId: string) {
  const member = await db.queryOne<any>(
    'SELECT * FROM team_chat_members WHERE team_id = ? AND user_id = ? AND removed_at IS NULL',
    [teamId, userId],
  );
 ...
 */
teamChatRouter.get('/', async (req: AuthRequest, res: Response, next) => {
  try {
 ...
 */
teamChatRouter.post('/', async (req: AuthRequest, res: Response, next) => {
  try {
    const { userId } = getAuth(req);
    const input = createTeamSchema.parse(req.body);

    const id = await db.insert(
      'INSERT INTO team_chats (name, description, created_by, created_at) VALUES (?, ?, ?, NOW())',
      [input.name, input.description || null, userId],
    );

    // Add creator as admin member
    await db.insert(
      'INSERT INTO team_chat_members (team_id, user_id, role, joined_at) VALUES (?, ?, ?, NOW())',
      [id, userId, 'admin'],
    );

    const team = await db.queryOne<any>('SELECT * FROM team_chats WHERE id = ?', [id]);
 ...
 */
teamChatRouter.get('/:id', async (req: AuthRequest, res: Response, next) => {
  try {
    const { userId } = getAuth(req);
    ...
    const team = await db.queryOne<any>('SELECT * FROM team_chats WHERE id = ?', [teamId]);
 ...
 */
teamChatRouter.put('/:id', async (req: AuthRequest, res: Response, next) => {
  try {
    const { userId } = getAuth(req);
    const teamId = req.params.id;
    ...
    if (member.role !== 'admin') throw forbidden('Only admin can update team');
    ...
    if (team) emitTeamUpdated(parseInt(teamId), team);
  } catch (err) {
    next(err);
  }
});
 ...
 */
teamChatRouter.delete('/:id', async (req: AuthRequest, res: Response, next) => {
  try {
    const { userId } = getAuth(req);
    const teamId = req.params.id;

    const member = await requireMembership(teamId, userId);
    if (member.role !== 'admin') throw forbidden('Only admin can delete team');
 ...
 */
teamChatRouter.post('/:id/members', async (req: AuthRequest, res: Response, next) => {
  try {
    const { userId } = getAuth(req);
    const teamId = req.params.id;
    ...
    for (const uid of input.user_ids) {
      ...
      const existing = await db.queryOne<any>(
        'SELECT * FROM team_chat_members WHERE team_id = ? AND user_id = ?',
        [teamId, uid],
      );
      if (existing && !existing.removed_at) continue; // already active member

      if (existing) {
        // Re-add previously removed member
        await db.execute(
          'UPDATE team_chat_members SET removed_at = NULL, joined_at = NOW() WHERE team_id = ? AND user_id = ?',
          [teamId, uid],
        );
      } else {
        await db.insert(
    ...
    if (added > 0) emitTeamMembersChanged(parseInt(teamId));
  } catch (err) {
    next(err);
  }
});
 ...
 */
teamChatRouter.delete('/:id/members/:userId', async (req: AuthRequest, res: Response, next) => {
  try {
    const { userId } = getAuth(req);
    const teamId = req.params.id;
    ...
    await db.execute(
      'UPDATE team_chat_members SET removed_at = NOW() WHERE team_id = ? AND user_id = ?',
      [teamId, targetUserId],
    );
 ...
 */
teamChatRouter.get('/:id/messages', async (req: AuthRequest, res: Response, next) => {
  try {
    const { userId } = getAuth(req);
    const teamId = req.params.id;
 ...
 */
teamChatRouter.post('/:id/messages', async (req: AuthRequest, res: Response, next) => {
  try {
    const { userId } = getAuth(req);
    const teamId = req.params.id;
    ...
    if (message) emitTeamMessage(parseInt(teamId), message);
  } catch (err) {
    next(err);
  }
});
 ...
 */
teamChatRouter.delete('/:id/messages/:msgId', async (req: AuthRequest, res: Response, next) => {
  try {
    const { userId } = getAuth(req);
    const teamId = req.params.id;
    ...
    const msg = await db.queryOne<any>(
      'SELECT * FROM team_chat_messages WHERE id = ? AND team_id = ?',
      [msgId, teamId],
    );
 ...
 */
teamChatRouter.post('/:id/upload', async (req: AuthRequest, res: Response, next) => {
  try {
    const { userId } = getAuth(req);
    const teamId = req.params.id;
