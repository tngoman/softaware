...
Answer the user's question based on this context. If the context doesn't contain relevant information, politely say you don't have that information and suggest contacting the website directly.`;
        ...
        if (isPaidTier && client.external_api_provider) {
            ...
            assistantMessage = await callExternalLLM(client.external_api_provider, client.external_api_key_encrypted, messages);
            ...
            const result = await chatCompletion(tier, messages, {
                temperature: 0.4,
                max_tokens: 2048,
            });
        ...
        if (leadCapture && leadCapture.action === 'capture_lead' && leadCapture.leadData) {
            // Store lead
            await storeCapturedLead(clientId, leadCapture.leadData, JSON.stringify(conversationHistory.slice(-5)) // Last 5 messages for context
            );
            // Send notification email
            await sendLeadNotification(clientId, leadCapture.leadData, client.business_name || client.website_url);
            leadCaptured = true;
 ...
 */
async function callExternalLLM(provider, encryptedKey, messages) {
    // Decrypt API key
