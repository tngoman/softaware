...
rolesRouter.get('/', requireAuth, async (_req, res, next) => {
    try {
        const roles = await db.query(`SELECT r.*, 
...
rolesRouter.get('/:id', requireAuth, async (req, res, next) => {
    try {
        ...
        const permissions = await db.query(`SELECT p.* FROM permissions p
       JOIN role_permissions rp ON rp.permission_id = p.id
       WHERE rp.role_id = ?
       ORDER BY p.permission_group, p.name`, [id]);
...
rolesRouter.post('/', requireAuth, async (req, res, next) => {
    try {
        ...
        const existing = await db.queryOne('SELECT id FROM roles WHERE slug = ?', [roleSlug]);
        if (existing)
            throw badRequest('Role slug already exists');
 ...
 */
rolesRouter.put('/:id', requireAuth, async (req, res, next) => {
    try {
        const { id } = req.params;
        const { name, slug, description } = req.body;
        const role = await db.queryOne('SELECT * FROM roles WHERE id = ?', [id]);
        ...
        const updated = await db.queryOne('SELECT * FROM roles WHERE id = ?', [id]);
 ...
 */
rolesRouter.post('/:id/assign', requireAuth, async (req, res, next) => {
    try {
        const { id } = req.params;
        ...
        if (!role)
            throw notFound('Role not found');
        const existing = await db.queryOne('SELECT id FROM user_roles WHERE user_id = ? AND role_id = ?', [user_id, id]);
        if (!existing) {
 ...
 */
rolesRouter.post('/:id/remove', requireAuth, async (req, res, next) => {
    try {
        const { id } = req.params;
        const { user_id } = req.body;
        if (!user_id)
