...
function looksMaliciousPrompt(input) {
...
function sanitizeLead(lead) {
    const clean = (value, max = 500) => (value || '').trim().slice(0, max) || undefined;
    return {
...
async function upsertLeadCapture(params) {
    ...
    const existing = await db.queryOne('SELECT id, messageCount FROM lead_captures WHERE sessionId = ? LIMIT 1', [params.sessionId]);
    const score = params.readyToContact ? 80 : 40;
    if (existing) {
        await db.execute(`UPDATE lead_captures
       SET sourcePage = ?,
    ...
    const id = randomUUID();
    await db.execute(`INSERT INTO lead_captures (
...
async function callLeadAssistantModel(message, history = []) {
...
publicLeadAssistantRouter.post('/assistant', async (req, res) => {
