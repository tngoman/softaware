...
router.put('/', requireAuth, async (req, res) => {
    try {
        ...
        if (!userId)
            throw unauthorized('Authentication required.');
        await requireStaffRole(userId);
        const assistant = await db.queryOne('SELECT id FROM assistants WHERE userId = ? AND is_staff_agent = 1', [userId]);
        if (!assistant)
            throw notFound('No staff assistant found. Create one first.');
...
router.delete('/', requireAuth, async (req, res) => {
    try {
        ...
        const assistant = await db.queryOne('SELECT id FROM assistants WHERE userId = ? AND is_staff_agent = 1', [userId]);
        if (!assistant)
            throw notFound('No staff assistant found.');
