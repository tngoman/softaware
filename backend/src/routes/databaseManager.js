...
function createTunnel(tunnel, dbHost, dbPort) {
    return new Promise((resolve, reject) => {
        ...
        else if (tunnel.sshPassword)
            opts.password = tunnel.sshPassword;
        else {
            clearTimeout(timeout);
            return reject(new Error('SSH auth requires a key file or password'));
        }
        sshClient.connect(opts);
    });
}
function closeTunnel(h) {
    try {
        h.server.close();
    }
    catch { }
...
async function withMySQL(body, fn) {
    let tunnel;
...
router.post('/connections', async (req, res) => {
    try {
        ...
        const t = tunnel || {};
        await db.query(`INSERT INTO db_connections (id, name, host, port, user, password, \`database\`, type, ssh_host, ssh_port, ssh_user, ssh_password, ssh_key_file, created_by)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    try {
        ...
        if (body.type === 'mssql') {
            await withMSSQL(body, async (pool) => {
                ...
                res.json({ success: true, databases: r.recordset.map((x) => x.name) });
            });
        }
        else {
...
router.post('/tables', async (req, res) => {
    try {
        ...
        if (!db)
            return res.status(400).json({ success: false, error: 'Database name required' });
        if (body.type === 'mssql') {
            await withMSSQL(body, async (pool) => {
          ...
          ORDER BY TABLE_NAME`);
            ...
            await withMySQL(body, async (conn) => {
                const [rows] = await conn.query(`SELECT TABLE_NAME AS name, TABLE_TYPE AS type, TABLE_ROWS AS \`rows\`, ENGINE AS engine
...
router.post('/table-data', async (req, res) => {
    try {
        const body = req.body;
        const table = body.table;
        ...
        if (body.type === 'mssql') {
            await withMSSQL(body, async (pool) => {
                ...
                if (filters.length > 0) {
                    const clauses = filters.map(f => {
                        ...
                        if (op === 'LIKE')
                            return `CAST(${col} AS NVARCHAR(MAX)) LIKE '%${f.value.replace(/'/g, "''")}%'`;
                ...
                const r = await pool.request().query(`SELECT * FROM [${safe}]${where} ${orderBy} OFFSET ${offset} ROWS FETCH NEXT ${pageSize} ROWS ONLY`);
            ...
            await withMySQL(body, async (conn) => {
                ...
                const columns = fields && Array.isArray(fields) ? fields.map((f) => f.name) : (Array.isArray(rows) && rows.length > 0 ? Object.keys(rows[0]) : []);
...
router.post('/row-insert', async (req, res) => {
    try {
        const body = req.body;
        const { table, row } = body;
        if (!table || !row)
            return res.status(400).json({ success: false, error: 'Table and row data required' });
        if (body.type === 'mssql') {
            await withMSSQL(body, async (pool) => {
...
router.post('/row-update', async (req, res) => {
    try {
        ...
        if (!table || !row || !whereObj)
            return res.status(400).json({ success: false, error: 'Table, row data, and where clause required' });
        if (body.type === 'mssql') {
            await withMSSQL(body, async (pool) => {
...
router.post('/row-delete', async (req, res) => {
    try {
        ...
        if (!table || !whereObj)
            return res.status(400).json({ success: false, error: 'Table and where clause required' });
        if (body.type === 'mssql') {
            await withMSSQL(body, async (pool) => {
                const whereCols = Object.keys(whereObj);
                const request = pool.request();
...
router.post('/import-sql', async (req, res) => {
    try {
        ...
        const { sql } = body;
        if (!sql)
            return res.status(400).json({ success: false, error: 'SQL content required' });
...
router.post('/table-size', async (req, res) => {
    try {
        const body = req.body;
        const table = body.table;
        if (!table)
        ...
        if (body.type === 'mssql') {
          ...
          WHERE OBJECT_NAME(p.object_id)='${table.replace(/'/g, "''")}'`);
            ...
            await withMySQL(body, async (conn) => {
                const [rows] = await conn.query(`SELECT TABLE_ROWS AS row_count,
           ...
           FROM information_schema.TABLES WHERE TABLE_SCHEMA=? AND TABLE_NAME=?`, [body.database, table]);
...
router.post('/table-create-sql', async (req, res) => {
    try {
        const body = req.body;
        const table = body.table;
        if (!table)
        ...
        await withMySQL(body, async (conn) => {
            ...
            res.json({ success: true, sql: rows[0]?.['Create Table'] || '' });
        });
    }
    catch (err) {
        res.status(400).json({ success: false, error: err.message });
        ...
        if (body.type === 'mssql') {
          ...
          SELECT @@SERVERNAME AS server_name, @@VERSION AS version, DB_NAME() AS current_db,
            ...
            await withMySQL(body, async (conn) => {
                ...
                res.json({ success: true, status: { version: v[0]?.version, ...status } });
            });
        }
    }
    catch (err) {
    ...
    try {
        ...
        const exec = async () => {
            ...
            return withMySQL(body, async (conn) => {
                ...
                if (!Array.isArray(result))
                    return { columns: [], rows: [] };
                const columns = fields && Array.isArray(fields) ? fields.map((f) => f.name) : (result.length > 0 ? Object.keys(result[0]) : []);
