...
profileRouter.get('/', async (req, res, next) => {
    try {
        ...
        const user = await db.queryOne(`SELECT id, email, name, phone, avatarUrl, createdAt, updatedAt,
              notifications_enabled, push_notifications_enabled, web_notifications_enabled
       FROM users WHERE id = ?`, [userId]);
        if (!user)
            throw notFound('User not found');
        // Get team membership info
        const membership = await db.queryOne(`SELECT tm.*, t.name AS teamName 
       FROM team_members tm 
       JOIN teams t ON tm.teamId = t.id 
       WHERE tm.userId = ? LIMIT 1`, [userId]);
        // Get subscription summary
        let subscription = null;
        if (membership) {
            subscription = await db.queryOne(`SELECT s.id, s.status, s.trialEndsAt, s.currentPeriodEnd,
        ...
        let credits = null;
        if (membership) {
            credits = await db.queryOne('SELECT balance, totalPurchased, totalUsed FROM credit_balances WHERE teamId = ?', [membership.teamId]);
        }
        res.json({
            user: {
                id: user.id,
                ...
                web_notifications_enabled: user.web_notifications_enabled ?? true,
...
profileRouter.put('/', async (req, res, next) => {
    try {
        ...
        if (input.email !== undefined) {
            ...
            const dup = await db.queryOne('SELECT id FROM users WHERE email = ? AND id != ?', [input.email, userId]);
            if (dup)
                throw badRequest('Email address is already in use.');
            sets.push('email = ?');
        ...
        const updated = await db.queryOne(`SELECT id, email, name, phone, avatarUrl, 
              notifications_enabled, push_notifications_enabled, web_notifications_enabled, 
              updatedAt 
       FROM users WHERE id = ?`, [userId]);
        res.json({
            ...
            user: {
                ...
                web_notifications_enabled: updated.web_notifications_enabled,
...
profileRouter.post('/change-password', async (req, res, next) => {
    try {
        ...
        await db.execute('UPDATE users SET passwordHash = ?, updatedAt = ? WHERE id = ?', [newHash, now, userId]);
...
// ────────────────────────────────────────────────────────────────────
profileRouter.get('/team', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const membership = await db.queryOne('SELECT * FROM team_members WHERE userId = ? LIMIT 1', [userId]);
        if (!membership)
            throw notFound('No team found');
        const team = await db.queryOne('SELECT * FROM teams WHERE id = ?', [membership.teamId]);
        const members = await db.query(`SELECT tm.*, u.email AS userEmail, u.name AS userName
       FROM team_members tm
       JOIN users u ON tm.userId = u.id
       WHERE tm.teamId = ?
       ORDER BY tm.createdAt`, [membership.teamId]);
        res.json({
            team: {
                id: team.id,
                name: team.name,
...
// ────────────────────────────────────────────────────────────────────
profileRouter.get('/api-keys', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const keys = await db.query(`SELECT id, name, \`key\`, isActive, lastUsedAt, createdAt, expiresAt
       FROM api_keys WHERE userId = ? ORDER BY createdAt DESC`, [userId]);
        res.json({
            apiKeys: keys.map((k) => ({
                id: k.id,
                name: k.name,
...
// ────────────────────────────────────────────────────────────────────
profileRouter.get('/invoices', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const membership = await db.queryOne('SELECT teamId FROM team_members WHERE userId = ? LIMIT 1', [userId]);
        if (!membership)
            throw notFound('No team found');
        const subscription = await db.queryOne(`SELECT id FROM subscriptions WHERE teamId = ? ORDER BY createdAt DESC LIMIT 1`, [membership.teamId]);
        if (!subscription) {
            return res.json({ invoices: [] });
        }
        const invoices = await db.query(`SELECT id, invoiceNumber, description, subtotal, vatAmount, total,
              periodStart, periodEnd, dueDate, paidAt, pdfUrl, createdAt
       FROM billing_invoices WHERE subscriptionId = ?
       ORDER BY createdAt DESC LIMIT 50`, [subscription.id]);
        res.json({
