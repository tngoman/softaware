...
router.get('/list', async (req, res, next) => {
    try {
        ...
        const files = await sshService.listDirectory(sshConfig, path);
        res.json({ files });
    }
    catch (error) {
        next(error);
...
router.get('/read', async (req, res, next) => {
    try {
        ...
        const content = await sshService.readFile(sshConfig, path);
...
router.post('/write', async (req, res, next) => {
    try {
        ...
        await sshService.writeFile(sshConfig, path, content);
...
router.delete('/delete', async (req, res, next) => {
    try {
        ...
        await sshService.deleteFile(sshConfig, path);
...
router.post('/mkdir', async (req, res, next) => {
    try {
        ...
        await sshService.createDirectory(sshConfig, path);
...
router.post('/move', async (req, res, next) => {
    try {
        ...
        await sshService.moveFile(sshConfig, oldPath, newPath);
...
router.post('/info', async (req, res, next) => {
    try {
        const { sshConfig, path } = readFileSchema.parse(req.body);
        const info = await sshService.getFileInfo(sshConfig, path);
        res.json({ path, info });
    }
    catch (error) {
        next(error);
...
router.post('/execute', async (req, res, next) => {
    try {
        ...
        const output = await sshService.executeCommand(sshConfig, command);
