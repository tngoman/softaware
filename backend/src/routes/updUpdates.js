...
updUpdatesRouter.get('/', async (req, res, next) => {
    try {
        ...
        if (id) {
            const update = await db.queryOne(`SELECT u.*, s.name AS software_name, usr.name AS uploaded_by_name
...
updUpdatesRouter.put('/', requireAuth, requireAdmin, async (req, res, next) => {
    try {
        ...
        const existing = await db.queryOne('SELECT * FROM update_releases WHERE id = ?', [body.id]);
        if (!existing)
            throw notFound('Update not found');
        const fields = [];
        const params = [];
...
updUpdatesRouter.delete('/', requireAuth, requireAdmin, async (req, res, next) => {
    try {
        ...
        const existing = await db.queryOne('SELECT * FROM update_releases WHERE id = ?', [id]);
        if (!existing)
            throw notFound('Update not found');
