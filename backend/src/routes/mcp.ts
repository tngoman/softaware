 ...
 * MCP over SSE Routes
 * 
 * Provides endpoints for desktop apps to connect to the MCP server via SSE.
 * 
 * Flow:
 * 1. Client authenticates and calls POST /mcp/session to create a session
 * 2. Client connects to GET /mcp/sse/:sessionId to receive SSE messages
 * 3. Client sends tool calls via POST /mcp/message/:sessionId
 * 4. Server executes tools and sends responses via SSE
 */
 ...
 */
mcpRouter.post('/session', requireAuth, async (req: Request, res: Response) => {
  try {
 ...
 */
mcpRouter.get('/sse/:sessionId', requireAuth, (req: Request, res: Response) => {
 ...
 */
mcpRouter.post('/message/:sessionId', requireAuth, async (req: Request, res: Response) => {
  const { sessionId } = req.params;
  const { userId } = getAuth(req);
 ...
 * Close a session
 */
mcpRouter.delete('/session/:sessionId', requireAuth, (req: Request, res: Response) => {
  const { sessionId } = req.params;
  const { userId } = getAuth(req);
 ...
 * Get session status
 */
mcpRouter.get('/session/:sessionId', requireAuth, (req: Request, res: Response) => {
  const { sessionId } = req.params;
  const { userId } = getAuth(req);
 ...
 */
mcpRouter.get('/sessions', requireAuth, (req: Request, res: Response) => {
  const { userId } = getAuth(req);
