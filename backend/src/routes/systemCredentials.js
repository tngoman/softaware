...
credentialsRouter.get('/', requireAuth, async (req, res, next) => {
    try {
        ...
        const creds = await db.query(query, params);
        ...
        const result = creds.map((c) => {
            ...
            try {
                val = decryptPassword(c.credential_value) ?? val;
            }
            catch { /* plaintext */ }
            ...
            if (addData) {
                try {
                    ...
                    for (const k of Object.keys(obj)) {
                        try {
                            obj[k] = decryptPassword(obj[k]) ?? obj[k];
                        }
                        catch { /* plaintext */ }
                    }
                    addData = obj;
...
credentialsRouter.get('/search', requireAuth, async (req, res, next) => {
    try {
        ...
        const creds = await db.query(`SELECT * FROM credentials
        WHERE (service_name LIKE ? OR identifier LIKE ? OR notes LIKE ?)
          AND is_active = 1
        ORDER BY service_name`, [`%${q}%`, `%${q}%`, `%${q}%`]);
        const result = creds.map((c) => ({
            ...c,
            credential_value: c.credential_value ? '••••••••' : null,
            additional_data: c.additional_data ? '(encrypted)' : null,
...
credentialsRouter.get('/service/:serviceName', requireAuth, async (req, res, next) => {
    try {
        ...
        const cred = await db.queryOne(query, params);
        if (!cred)
            throw notFound(`Credential for service "${serviceName}" not found`);
        if (!decrypt) {
            ...
            try {
                cred.credential_value = decryptPassword(cred.credential_value) ?? cred.credential_value;
            }
            catch { /* plaintext */ }
 ...
 */
credentialsRouter.get('/expired', requireAuth, async (_req, res, next) => {
    try {
        const creds = await db.query('SELECT * FROM credentials WHERE expires_at IS NOT NULL AND expires_at < NOW() AND is_active = 1 ORDER BY expires_at');
 ...
 */
credentialsRouter.get('/expiring', requireAuth, async (_req, res, next) => {
    try {
        const creds = await db.query('SELECT * FROM credentials WHERE expires_at IS NOT NULL AND expires_at BETWEEN NOW() AND DATE_ADD(NOW(), INTERVAL 30 DAY) AND is_active = 1 ORDER BY expires_at');
        res.json({ success: true, data: creds });
    }
    catch (err) {
        next(err);
 ...
 */
credentialsRouter.get('/:id', requireAuth, async (req, res, next) => {
    try {
        const { id } = req.params;
        const decrypt = req.query.decrypt === 'true';
        const cred = await db.queryOne('SELECT * FROM credentials WHERE id = ?', [id]);
        if (!cred)
 ...
 */
credentialsRouter.post('/', requireAuth, async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const { service_name, credential_type, identifier, credential_value, additional_data, environment, expires_at, notes } = req.body;
        if (!service_name || !credential_value)
            throw badRequest('service_name and credential_value are required');
 ...
 */
credentialsRouter.put('/:id', requireAuth, async (req, res, next) => {
    try {
        const { id } = req.params;
        const { userId } = getAuth(req);
        const cred = await db.queryOne('SELECT * FROM credentials WHERE id = ?', [id]);
        ...
        const updated = await db.queryOne('SELECT * FROM credentials WHERE id = ?', [id]);
 ...
 */
credentialsRouter.delete('/:id', requireAuth, async (req, res, next) => {
    try {
        const { id } = req.params;
        const cred = await db.queryOne('SELECT * FROM credentials WHERE id = ?', [id]);
        if (!cred)
 ...
 */
credentialsRouter.post('/:id/deactivate', requireAuth, async (req, res, next) => {
    try {
        const { id } = req.params;
        const cred = await db.queryOne('SELECT * FROM credentials WHERE id = ?', [id]);
        if (!cred)
 ...
 */
credentialsRouter.post('/:id/rotate', requireAuth, async (req, res, next) => {
    try {
        const { id } = req.params;
        ...
        if (new_value) {
            ...
            await db.execute('UPDATE credentials SET credential_value = ?, updated_by = ? WHERE id = ?', [encryptedNew, req.userId, id]);
            invalidateCache(cred.service_name);
 ...
 */
credentialsRouter.post('/:id/test', requireAuth, async (req, res, next) => {
    try {
        const { id } = req.params;
        const cred = await db.queryOne('SELECT * FROM credentials WHERE id = ?', [id]);
        if (!cred)
