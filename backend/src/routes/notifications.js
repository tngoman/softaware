...
notificationsRouter.get('/', requireAuth, async (req, res, next) => {
    try {
        ...
        const notifications = await db.query(sql, [userId, limit]);
        const countRow = await db.queryOne(`SELECT COUNT(*) AS count FROM notifications WHERE user_id = ? AND read_at IS NULL`, [userId]);
        res.json({
            success: true,
            data: notifications,
...
notificationsRouter.put('/:id/read', requireAuth, async (req, res, next) => {
    try {
        const userId = req.userId;
        await db.execute(`UPDATE notifications SET read_at = NOW() WHERE id = ? AND user_id = ?`, [req.params.id, userId]);
...
notificationsRouter.put('/read-all', requireAuth, async (req, res, next) => {
    try {
        const userId = req.userId;
        await db.execute(`UPDATE notifications SET read_at = NOW() WHERE user_id = ? AND read_at IS NULL`, [userId]);
...
notificationsRouter.delete('/:id', requireAuth, async (req, res, next) => {
    try {
        const userId = req.userId;
        await db.execute(`DELETE FROM notifications WHERE id = ? AND user_id = ?`, [req.params.id, userId]);
...
notificationsRouter.post('/', requireAuth, async (req, res, next) => {
    try {
        ...
        if (input.send_push) {
            ...
            await db.execute(`INSERT INTO notifications (user_id, title, message, type, created_at) VALUES (?, ?, ?, ?, NOW())`, [targetUserId, input.title, input.message, input.type]);
        }
...
notificationsRouter.post('/test-push', requireAuth, async (req, res, next) => {
    try {
        const userId = req.userId;
