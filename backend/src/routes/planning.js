...
import { getMailbox, getMessage, } from '../services/webmailService.js';
            ...
            const safeAlter = async (sql) => {
                try {
                    ...
                    if (e.code !== 'ER_DUP_FIELDNAME' && e.errno !== 1060 &&
                        e.code !== 'ER_DUP_KEYNAME' && e.errno !== 1061)
                        throw e;
                }
            };
...
function parseICalEvents(icalText) {
    ...
    while ((match = veventRegex.exec(icalText)) !== null) {
        ...
        if (orgParams) {
            ...
            if (cnMatch)
                orgName = cnMatch[1].replace(/"/g, '').trim();
        }
        events.push({
...
function parseICalDate(str) {
    ...
    if (/^\d{8}$/.test(str))
        return `${str.slice(0, 4)}-${str.slice(4, 6)}-${str.slice(6, 8)}T00:00:00.000Z`;
    if (/^\d{8}T\d{6}Z?$/i.test(str))
        return `${str.slice(0, 4)}-${str.slice(4, 6)}-${str.slice(6, 8)}T${str.slice(9, 11)}:${str.slice(11, 13)}:${str.slice(13, 15)}.000Z`;
...
async function enrichEventWithAttendees(event) {
    if (!event)
        return event;
    const attendees = await db.query(`SELECT a.user_id, a.rsvp, a.is_organizer, a.added_at, a.responded_at,
            u.name, u.email, u.avatar_url
     FROM calendar_event_attendees a
     JOIN users u ON u.id = a.user_id
     WHERE a.event_id = ?
     ORDER BY a.is_organizer DESC, a.added_at ASC`, [event.id]);
    return { ...event, attendees };
}
async function enrichEventsWithAttendeeCounts(events) {
    if (!events.length)
        return events;
    const ids = events.map((e) => e.id);
    const placeholders = ids.map(() => '?').join(',');
    const counts = await db.query(`SELECT event_id, COUNT(*) as count,
    ...
    for (const c of counts)
        countMap[c.event_id] = { total: Number(c.count), accepted: Number(c.accepted_count) };
    return events.map((e) => ({
        ...e,
...
planningRouter.get('/events', async (req, res, next) => {
    try {
      ...
      WHERE (ce.user_id = ? OR cea.user_id = ?)`;
        ...
        if (params.start) {
            sql += ` AND ce.end_at >= ?`;
            values.push(toMySQLDate(params.start));
        }
        if (params.end) {
            sql += ` AND ce.start_at <= ?`;
            values.push(toMySQLDate(params.end));
        }
        if (params.source_type) {
            sql += ` AND ce.source_type = ?`;
...
planningRouter.post('/events', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const data = CreateEventSchema.parse(req.body);
        const inviteToken = (data.event_type === 'meeting' || data.event_type === 'call') ? generateInviteToken() : null;
        const insertId = await db.insert(`INSERT INTO calendar_events
       ...
       VALUES (?, ?, ?, ?, ?, 'manual', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
        ...
        await db.insert(`INSERT INTO calendar_event_attendees (event_id, user_id, rsvp, is_organizer) VALUES (?, ?, 'accepted', 1)`, [insertId, userId]);
        ...
        if (data.attendee_ids?.length) {
            for (const uid of data.attendee_ids) {
                if (uid === userId)
                    continue;
                try {
                    await db.insert(`INSERT INTO calendar_event_attendees (event_id, user_id, rsvp, is_organizer) VALUES (?, ?, 'pending', 0)`, [insertId, uid]);
                }
                catch (e) {
        ...
        const rows = await db.query(`SELECT * FROM calendar_events WHERE id = ?`, [insertId]);
...
planningRouter.get('/events/:id', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const rows = await db.query(`SELECT DISTINCT ce.* FROM calendar_events ce
       LEFT JOIN calendar_event_attendees cea ON cea.event_id = ce.id
       WHERE ce.id = ? AND (ce.user_id = ? OR cea.user_id = ?) LIMIT 1`, [req.params.id, userId, userId]);
        if (!rows.length)
...
planningRouter.put('/events/:id', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const existing = await db.query(`SELECT id, user_id FROM calendar_events WHERE id = ? AND user_id = ?`, [req.params.id, userId]);
        if (!existing.length)
        ...
        if (body.start_at) {
            ...
            values.push(toMySQLDate(body.start_at));
        }
        if (body.end_at) {
            updates.push('end_at = ?');
            values.push(toMySQLDate(body.end_at));
        }
        ...
        if (body.recurrence_end !== undefined) {
            ...
            values.push(body.recurrence_end ? toMySQLDate(body.recurrence_end) : null);
        }
        if (body.status) {
            updates.push('status = ?');
        ...
        if (body.reminder_minutes !== undefined) {
            ...
            values.push(body.reminder_minutes);
        }
        if (body.call_type) {
            updates.push('call_type = ?');
            values.push(body.call_type);
        }
        ...
        if (body.attendee_ids && Array.isArray(body.attendee_ids)) {
            ...
            await db.query(`DELETE FROM calendar_event_attendees WHERE event_id = ? AND is_organizer = 0 AND user_id NOT IN (${placeholders})`, [req.params.id, ...body.attendee_ids]);
            for (const uid of body.attendee_ids) {
                if (uid === userId)
                    continue;
                try {
                    await db.insert(`INSERT INTO calendar_event_attendees (event_id, user_id, rsvp, is_organizer) VALUES (?, ?, 'pending', 0)`, [req.params.id, uid]);
                }
                catch (e) {
                    if (e.code !== 'ER_DUP_ENTRY' && e.errno !== 1062)
                        throw e;
...
planningRouter.delete('/events/:id', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const existing = await db.query(`SELECT id FROM calendar_events WHERE id = ? AND user_id = ?`, [req.params.id, userId]);
        if (!existing.length)
            return res.status(404).json({ error: 'Event not found' });
...
// ATTENDEE MANAGEMENT
// ═══════════════════════════════════════════════════════════════════════════
planningRouter.post('/events/:id/attendees', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const existing = await db.query(`SELECT id FROM calendar_events WHERE id = ? AND user_id = ?`, [req.params.id, userId]);
        if (!existing.length)
...
planningRouter.delete('/events/:id/attendees/:userId', async (req, res, next) => {
    try {
        const auth = getAuth(req);
        const existing = await db.query(`SELECT id, user_id FROM calendar_events WHERE id = ?`, [req.params.id]);
        if (!existing.length)
            return res.status(404).json({ error: 'Event not found' });
        if (existing[0].user_id !== auth.userId && req.params.userId !== auth.userId) {
            return res.status(403).json({ error: 'Not authorized' });
        }
        await db.query(`DELETE FROM calendar_event_attendees WHERE event_id = ? AND user_id = ? AND is_organizer = 0`, [req.params.id, req.params.userId]);
...
// RESPOND TO EVENT
// ═══════════════════════════════════════════════════════════════════════════
planningRouter.post('/events/:id/respond', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const status = z.enum(['accepted', 'tentative', 'declined']).parse(req.body.status);
        const attendee = await db.query(`SELECT id FROM calendar_event_attendees WHERE event_id = ? AND user_id = ?`, [req.params.id, userId]);
        if (attendee.length) {
            await db.query(`UPDATE calendar_event_attendees SET rsvp = ?, responded_at = NOW() WHERE event_id = ? AND user_id = ?`, [status, req.params.id, userId]);
        }
        else {
            const mapped = status === 'accepted' ? 'confirmed' : status === 'declined' ? 'declined' : 'tentative';
            await db.query(`UPDATE calendar_events SET status = ? WHERE id = ? AND user_id = ?`, [mapped, req.params.id, userId]);
        }
        const rows = await db.query(`SELECT * FROM calendar_events WHERE id = ?`, [req.params.id]);
        if (!rows.length)
            return res.status(404).json({ error: 'Event not found' });
...
// INVITATION LINKS
// ═══════════════════════════════════════════════════════════════════════════
planningRouter.get('/invite/:token', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const rows = await db.query(`SELECT ce.*, u.name as creator_name, u.email as creator_email
       FROM calendar_events ce JOIN users u ON u.id = ce.user_id
       WHERE ce.invitation_token = ?`, [req.params.token]);
        if (!rows.length)
            return res.status(404).json({ error: 'Invitation not found or expired' });
        const existing = await db.query(`SELECT rsvp FROM calendar_event_attendees WHERE event_id = ? AND user_id = ?`, [rows[0].id, userId]);
        const enriched = await enrichEventWithAttendees(rows[0]);
        res.json({
...
planningRouter.post('/invite/:token/accept', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const rsvp = z.enum(['accepted', 'tentative', 'declined']).parse(req.body.rsvp || 'accepted');
        const rows = await db.query(`SELECT id, user_id FROM calendar_events WHERE invitation_token = ?`, [req.params.token]);
        if (!rows.length)
            return res.status(404).json({ error: 'Invitation not found' });
        try {
            await db.insert(`INSERT INTO calendar_event_attendees (event_id, user_id, rsvp, is_organizer, responded_at)
         VALUES (?, ?, ?, 0, NOW())`, [rows[0].id, userId, rsvp]);
        }
        catch (e) {
            if (e.code === 'ER_DUP_ENTRY' || e.errno === 1062) {
                await db.query(`UPDATE calendar_event_attendees SET rsvp = ?, responded_at = NOW() WHERE event_id = ? AND user_id = ?`, [rsvp, rows[0].id, userId]);
            }
            else
                throw e;
        }
        const updated = await db.query(`SELECT * FROM calendar_events WHERE id = ?`, [rows[0].id]);
...
// CALL INTEGRATION
// ═══════════════════════════════════════════════════════════════════════════
planningRouter.post('/events/:id/start-call', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        ...
        if (!rows.length)
            return res.status(404).json({ error: 'Event not found or you are not a participant' });
        const event = rows[0];
        ...
        const attendees = await db.query(`SELECT user_id FROM calendar_event_attendees WHERE event_id = ? AND user_id != ?`, [eventId, userId]);
        res.json({
            success: true,
            data: {
                action: 'initiate_call',
                event_id: event.id, call_type: callType,
                title: event.title,
...
// ═══════════════════════════════════════════════════════════════════════════
planningRouter.get('/users', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
...
// SCHEDULED CALL SYNC
// ═══════════════════════════════════════════════════════════════════════════
planningRouter.post('/sync/scheduled-calls', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
    ...
    `, [userId]);
        ...
        for (const call of calls) {
            const existingCal = await db.query(`SELECT id FROM calendar_events WHERE user_id = ? AND scheduled_call_id = ?`, [userId, call.id]);
            if (existingCal.length)
                continue;
            const endAt = new Date(call.scheduled_at);
            endAt.setMinutes(endAt.getMinutes() + (call.duration_minutes || 30));
            const newId = await db.insert(`INSERT INTO calendar_events
         ...
         VALUES (?, ?, ?, 'call', 'scheduled_call', ?, ?, ?, ?, 'confirmed', ?, ?, ?, ?, ?, ?)`, [
            ...
            await db.insert(`INSERT IGNORE INTO calendar_event_attendees (event_id, user_id, rsvp, is_organizer) VALUES (?, ?, 'accepted', 1)`, [newId, userId]);
            const scParticipants = await db.query(`SELECT user_id, rsvp FROM scheduled_call_participants WHERE scheduled_call_id = ?`, [call.id]);
            for (const p of scParticipants) {
                if (p.user_id === userId)
                    continue;
                try {
                    await db.insert(`INSERT INTO calendar_event_attendees (event_id, user_id, rsvp, is_organizer) VALUES (?, ?, ?, 0)`, [newId, p.user_id, p.rsvp || 'pending']);
                }
                catch (e) {
                    if (e.code !== 'ER_DUP_ENTRY' && e.errno !== 1062)
                        throw e;
...
planningRouter.post('/import/email-invite', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const { account_id, folder, uid } = ImportEmailSchema.parse(req.body);
        const account = await getMailbox(account_id, userId);
        if (!account)
            return res.status(404).json({ error: 'Mailbox account not found' });
        ...
        if (!icalText) {
            ...
            if (vcalMatch)
                icalText = vcalMatch[0];
        }
        if (!icalText) {
            const startAt = new Date(message.date);
            const endAt = new Date(startAt.getTime() + 60 * 60 * 1000);
            const fallbackId = await db.insert(`INSERT INTO calendar_events
          (user_id, title, description, event_type, source_type,
         ...
         VALUES (?, ?, ?, 'email_invite', 'email_invite', ?, ?, 'tentative',
            ...
            const rows = await db.query(`SELECT * FROM calendar_events WHERE id = ?`, [fallbackId]);
            const enriched = await enrichEventWithAttendees(rows[0]);
        ...
        if (!parsedEvents.length)
            return res.status(400).json({ error: 'No VEVENT found in iCalendar data' });
        const importedArr = [];
        for (const evt of parsedEvents) {
            if (evt.uid) {
                const dup = await db.query(`SELECT id FROM calendar_events WHERE user_id = ? AND ical_uid = ?`, [userId, evt.uid]);
                if (dup.length) {
                    await db.query(`UPDATE calendar_events SET title = ?, description = ?, location = ?,
             start_at = ?, end_at = ?, all_day = ?, status = ?,
             organizer_name = ?, organizer_email = ? WHERE id = ?`, [evt.summary || message.subject || 'Calendar Event', evt.description || null,
                    ...
                    const updatedRows = await db.query(`SELECT * FROM calendar_events WHERE id = ?`, [dup[0].id]);
                    importedArr.push(await enrichEventWithAttendees(updatedRows[0]));
                    continue;
                }
            }
            const icalInsertId = await db.insert(`INSERT INTO calendar_events
          (user_id, title, description, location, event_type, source_type,
           start_at, end_at, all_day, status,
         ...
         VALUES (?, ?, ?, ?, 'email_invite', 'email_invite', ?, ?, ?, ?,
            ...
            const icalRows = await db.query(`SELECT * FROM calendar_events WHERE id = ?`, [icalInsertId]);
...
planningRouter.get('/scan/email-invites', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        ...
        try {
            ...
            try {
                ...
                for await (const msg of messages) {
                    ...
                    if (hasCalendar) {
                        const existingImport = await db.query(`SELECT id FROM calendar_events WHERE user_id = ? AND email_account_id = ? AND email_uid = ?`, [userId, accountId, msg.uid]);
                        invitations.push({
                            uid: msg.uid,
...
function checkBodyForCalendar(structure) {
    if (!structure)
        return false;
    if (structure.type === 'text/calendar' || structure.type === 'application/ics')
        return true;
    if (structure.disposition === 'attachment' && structure.parameters?.name?.endsWith('.ics'))
        return true;
    if (structure.childNodes) {
        for (const child of structure.childNodes) {
            if (checkBodyForCalendar(child))
                return true;
        }
    }
    return false;
...
// STATS
// ═══════════════════════════════════════════════════════════════════════════
planningRouter.get('/stats', async (req, res, next) => {
    try {
        const { userId } = getAuth(req);
        const now = new Date();
        ...
        const todayRows = await db.query(`SELECT COUNT(DISTINCT ce.id) as count FROM calendar_events ce
       LEFT JOIN calendar_event_attendees cea ON cea.event_id = ce.id
       WHERE (ce.user_id = ? OR cea.user_id = ?) AND ce.start_at >= ? AND ce.start_at < ? AND ce.status != 'cancelled'`, [userId, userId, todayStart, todayEnd]);
        const weekRows = await db.query(`SELECT COUNT(DISTINCT ce.id) as count FROM calendar_events ce
       LEFT JOIN calendar_event_attendees cea ON cea.event_id = ce.id
       WHERE (ce.user_id = ? OR cea.user_id = ?) AND ce.start_at >= ? AND ce.start_at < ? AND ce.status != 'cancelled'`, [userId, userId, todayStart, weekEnd]);
        const inviteRows = await db.query(`SELECT COUNT(*) as count FROM calendar_event_attendees WHERE user_id = ? AND rsvp = 'pending'`, [userId]);
        res.json({
            success: true,
            data: {
