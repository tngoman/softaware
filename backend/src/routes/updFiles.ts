...
updFilesRouter.post('/upload', upload.single('updatePackage'), async (req: Request, res, next) => {
  try {
    ...
    const sw = await db.queryOne<any>('SELECT id FROM update_software WHERE id = ?', [body.software_id]);
    if (!sw) {
    ...
    if (body.update_id) {
      // Replace existing
      const existing = await db.queryOne<UpdUpdate>('SELECT * FROM update_releases WHERE id = ?', [body.update_id]);
      if (!existing) throw notFound('Update not found for replacement');
      ...
      await db.execute(
        `UPDATE update_releases SET software_id = ?, version = ?, description = ?,
...
updFilesRouter.get('/download', async (req, res, next) => {
  try {
    ...
    const softwareKey = req.header('X-Software-Key') || (req.query.software_key as string);

    if (!softwareKey) throw badRequest('Missing software key (X-Software-Key header or ?software_key= param)');

    // Verify software key is valid
    const sw = await db.queryOne<any>('SELECT id FROM update_software WHERE software_key = ?', [softwareKey]);
    if (!sw) throw notFound('Invalid software key');

    const update = await db.queryOne<UpdUpdate>('SELECT * FROM update_releases WHERE id = ?', [updateId]);
    if (!update) throw notFound('Update not found');
    if (!update.file_path) throw notFound('No file associated with this update');

    const filePath = path.resolve(process.cwd(), update.file_path);
    if (!fs.existsSync(filePath)) throw notFound('File not found on disk');
