...
import type { UpdModule } from '../db/updatesTypes.js';
...
updModulesRouter.get('/', requireAuth, async (req, res, next) => {
  try {
    ...
    const modules = await db.query<any>(
      `SELECT m.*, s.name AS software_name,
    ...
    if (id && modules.length === 0) throw notFound('Module not found');
...
updModulesRouter.post('/', requireAuth, requireAdmin, async (req, res, next) => {
  try {
    ...
    const dup = await db.queryOne<any>(
      'SELECT id FROM update_modules WHERE software_id = ? AND name = ?',
      [body.software_id, body.name]
    );
    if (dup) {
    ...
    const result = await db.insert(
      'INSERT INTO update_modules (software_id, name, description) VALUES (?, ?, ?)',
      [body.software_id, body.name, body.description || null]
    );

    const mod = await db.queryOne<UpdModule>('SELECT * FROM update_modules WHERE id = ?', [Number(result)]);
...
updModulesRouter.put('/', requireAuth, requireAdmin, async (req, res, next) => {
  try {
    ...
    const existing = await db.queryOne<UpdModule>('SELECT * FROM update_modules WHERE id = ?', [id]);
    if (!existing) throw notFound('Module not found');

    // Check duplicate name if renaming
    if (body.name && body.name !== existing.name) {
      const dup = await db.queryOne<any>(
        'SELECT id FROM update_modules WHERE software_id = ? AND name = ? AND id != ?',
        [existing.software_id, body.name, id]
      );
      if (dup) {
        return res.status(409).json({ success: false, error: 'Module name already exists for this software' });
      }
...
updModulesRouter.delete('/', requireAuth, requireAdmin, async (req, res, next) => {
  try {
    ...
    if (!affected) throw notFound('Module not found');
...
updModulesRouter.get('/:moduleId/developers', requireAuth, async (req, res, next) => {
  try {
    ...
    const mod = await db.queryOne<any>('SELECT id FROM update_modules WHERE id = ?', [moduleId]);
    if (!mod) throw notFound('Module not found');

    const developers = await db.query<any>(
      `SELECT um.id AS assignment_id, um.user_id, um.module_id,
...
updModulesRouter.post('/:moduleId/developers', requireAuth, requireAdmin, async (req, res, next) => {
  try {
    ...
    const user = await db.queryOne<any>('SELECT id FROM users WHERE id = ?', [body.user_id]);
    if (!user) throw notFound('User not found');

    // Check for duplicate assignment
    const dup = await db.queryOne<any>(
      'SELECT id FROM update_user_modules WHERE user_id = ? AND module_id = ?',
      [body.user_id, moduleId]
    );
    if (dup) {
    ...
    const result = await db.insert(
      'INSERT INTO update_user_modules (user_id, module_id) VALUES (?, ?)',
      [body.user_id, moduleId]
    );
...
updModulesRouter.delete('/:moduleId/developers', requireAuth, requireAdmin, async (req, res, next) => {
  try {
    ...
    const affected = await db.execute(
      'DELETE FROM update_user_modules WHERE user_id = ? AND module_id = ?',
      [userId, moduleId]
    );
    if (!affected) throw notFound('Assignment not found');
