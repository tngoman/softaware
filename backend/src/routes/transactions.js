...
transactionsRouter.get('/:id', requireAuth, async (req, res, next) => {
    try {
        ...
        const transaction = await db.queryOne('SELECT * FROM transactions_vat WHERE transaction_id = ?', [id]);
        if (!transaction) {
...
transactionsRouter.post('/', requireAuth, async (req, res, next) => {
    try {
        const { transaction_date, transaction_type, party_name, invoice_number, vat_type, vat_amount, exclusive_amount, total_amount, description, payment_method, reference_number, } = req.body;
        // Validation
 ...
 */
transactionsRouter.put('/:id', requireAuth, async (req, res, next) => {
    try {
        const id = parseInt(req.params.id);
        if (isNaN(id)) {
            throw badRequest('Invalid transaction ID');
        }
        const existing = await db.queryOne('SELECT transaction_id FROM transactions_vat WHERE transaction_id = ?', [id]);
        if (!existing) {
            throw notFound('Transaction not found');
        }
        const { transaction_date, transaction_type, party_name, invoice_number, vat_type, vat_amount, exclusive_amount, total_amount, description, payment_method, reference_number, } = req.body;
        ...
        if (vat_type !== undefined)
            updates.vat_type = vat_type;
        if (vat_amount !== undefined)
            updates.vat_amount = vat_amount;
        if (exclusive_amount !== undefined)
            updates.exclusive_amount = exclusive_amount;
        ...
        await db.execute(`UPDATE transactions_vat SET ${updateFields} WHERE transaction_id = ?`, [...updateValues, id]);
...
transactionsRouter.delete('/:id', requireAuth, async (req, res, next) => {
    try {
        ...
        const affectedRows = await db.execute('DELETE FROM transactions_vat WHERE transaction_id = ?', [id]);
        if (affectedRows === 0) {
            throw notFound('Transaction not found');
        }
        res.json({ success: true });
 ...
 */
transactionsRouter.post('/clear-income', requireAuth, async (req, res, next) => {
    try {
        const affectedRows = await db.execute('DELETE FROM transactions_vat WHERE transaction_type = ?', ['income']);
        res.json({
            success: true,
