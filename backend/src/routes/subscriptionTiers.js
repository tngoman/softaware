...
router.get('/subscriptions/current', requireAuth, async (req, res) => {
    try {
       ...
       WHERE wc.user_id = ?
       ORDER BY wc.created_at DESC`, [userId]);
        res.json({
            success: true,
            subscriptions: rows || []
        });
...
router.get('/subscriptions/tiers', async (req, res) => {
    try {
        const [rows] = await db.execute(`SELECT * FROM subscription_tier_limits ORDER BY max_messages_per_month ASC`);
        res.json({
            success: true,
            tiers: rows || []
        });
 ...
 */
router.post('/subscriptions/:clientId/upgrade', requireAuth, async (req, res) => {
    try {
        const { clientId } = req.params;
        const { tier, monthlyPrice } = req.body;
        const userId = req.userId;
        // Verify ownership
        const [clientRows] = await db.execute(`SELECT * FROM widget_clients WHERE id = ? AND user_id = ?`, [clientId, userId]);
        ...
        // Update subscription
        await db.execute(`UPDATE widget_clients 
 ...
 */
router.put('/subscriptions/:clientId/config', requireAuth, async (req, res) => {
    try {
        const { clientId } = req.params;
        const userId = req.userId;
        const { tonePreset, customToneInstructions, leadCaptureEnabled, leadNotificationEmail, preferredModel, externalApiProvider, externalApiKey } = req.body;
        // Verify ownership and tier
        const [clientRows] = await db.execute(`SELECT wc.*, stl.tone_control, stl.lead_capture
       FROM widget_clients wc
       LEFT JOIN subscription_tier_limits stl ON wc.subscription_tier = stl.tier
       WHERE wc.id = ? AND wc.user_id = ?`, [clientId, userId]);
        if (!clientRows || clientRows.length === 0) {
            return res.status(404).json({ error: 'Widget client not found or access denied' });
        }
 ...
 */
router.get('/subscriptions/:clientId/usage', requireAuth, async (req, res) => {
    try {
        const { clientId } = req.params;
        const userId = req.userId;
        // Verify ownership
        ...
        // Get usage stats
        const [usageRows] = await db.execute(`SELECT 
         cycle_start,
         cycle_end,
       ...
       LIMIT 6`, [clientId]);
        // Get lead stats
        const [leadRows] = await db.execute(`SELECT 
       ...
       WHERE client_id = ?
         AND captured_at >= DATE_SUB(NOW(), INTERVAL 30 DAY)`, [clientId]);
        res.json({
            success: true,
            usage: usageRows || [],
 ...
 */
router.get('/subscriptions/:clientId/leads', requireAuth, async (req, res) => {
    try {
        const { clientId } = req.params;
        const userId = req.userId;
       ...
       LIMIT ? OFFSET ?`, [clientId, Number(limit), Number(offset)]);
        // Get total count
        const [countRows] = await db.execute(`SELECT COUNT(*) as total FROM widget_leads_captured WHERE client_id = ?`, [clientId]);
        res.json({
            success: true,
            leads: leadRows || [],
