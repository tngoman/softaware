...
systemUsersRouter.get('/', requireAuth, async (req, res, next) => {
    try {
        const users = await db.query('SELECT id, email, name, phone, avatarUrl, contact_id, is_admin, is_staff, isActive, createdAt, updatedAt FROM users ORDER BY createdAt DESC');
        const result = [];
        for (const u of users) {
            const roles = await db.query(`SELECT r.id, r.name, r.slug FROM roles r
         JOIN user_roles ur ON ur.role_id = r.id
         WHERE ur.user_id COLLATE utf8mb4_unicode_ci = ? COLLATE utf8mb4_unicode_ci`, [u.id]);
 ...
 */
systemUsersRouter.get('/:id', requireAuth, async (req, res, next) => {
    try {
        const { id } = req.params;
        const u = await db.queryOne('SELECT id, email, name, phone, avatarUrl, contact_id, is_admin, is_staff, isActive, createdAt, updatedAt FROM users WHERE id = ?', [id]);
        if (!u)
...
systemUsersRouter.post('/', requireAuth, async (req, res, next) => {
    try {
        ...
        if (!contact_id)
            throw badRequest('A contact must be selected when creating a user');
        const existing = await db.queryOne('SELECT id FROM users WHERE email = ?', [email]);
        if (existing)
            throw badRequest('Email already registered');
        // Verify contact exists
        const contactExists = await db.queryOne('SELECT id FROM contacts WHERE id = ? AND active = 1', [contact_id]);
        if (!contactExists)
            throw badRequest('Selected contact does not exist');
        const passwordHash = await bcrypt.hash(password, 12);
        const { generateId, toMySQLDate } = await import('../db/mysql.js');
        const userId = generateId();
        const now = toMySQLDate(new Date());
        await db.execute('INSERT INTO users (id, email, name, phone, passwordHash, contact_id, is_admin, is_staff, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [userId, email, name || null, phone || null, passwordHash, contact_id, is_admin ? 1 : 0, is_staff ? 1 : 0, now, now]);
        ...
        if (role) {
            await db.execute('INSERT INTO user_roles (user_id, role_id) VALUES (?, ?)', [userId, role.id]);
        }
        const u = await db.queryOne('SELECT id, email, name, phone, avatarUrl, contact_id, is_admin, is_staff, isActive, createdAt, updatedAt FROM users WHERE id = ?', [userId]);
        const roles = await db.query(`SELECT r.id, r.name, r.slug FROM roles r
       JOIN user_roles ur ON ur.role_id = r.id
 ...
 */
systemUsersRouter.put('/:id', requireAuth, async (req, res, next) => {
    try {
        const { id } = req.params;
        ...
        const updated = await db.queryOne('SELECT id, email, name, phone, avatarUrl, contact_id, is_admin, is_staff, isActive, createdAt, updatedAt FROM users WHERE id = ?', [id]);
        const roles = await db.query(`SELECT r.id, r.name, r.slug FROM roles r
       JOIN user_roles ur ON ur.role_id = r.id
       WHERE ur.user_id COLLATE utf8mb4_unicode_ci = ? COLLATE utf8mb4_unicode_ci`, [id]);
 ...
 */
systemUsersRouter.delete('/:id', requireAuth, async (req, res, next) => {
    try {
        const { id } = req.params;
        ...
        try {
            await db.execute(`UPDATE teams SET createdByUserId = ? WHERE createdByUserId = ?`, [userId, id]);
        }
        catch { /* skip */ }
