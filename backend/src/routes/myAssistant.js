...
// Helpers
// ============================================================================
async function requireStaffRole(userId) {
...
router.get('/:id', requireAuth, async (req, res) => {
    try {
        ...
        if (!userId)
            throw unauthorized('Authentication required.');
        const assistant = await db.queryOne(`SELECT id, name, description, personality, personality_flare,
              primary_goal, custom_greeting, voice_style, preferred_model,
              status, tier, pages_indexed, is_staff_agent, is_primary,
              business_type, website, knowledge_categories,
              created_at, updated_at
       FROM assistants
       WHERE id = ? AND userId = ?`, [req.params.id, userId]);
        if (!assistant)
            throw notFound('Assistant not found.');
...
router.post('/', requireAuth, async (req, res) => {
    try {
        ...
        // Staff: enforce max 1
        if (isStaff) {
            const existing = await db.queryOne('SELECT id FROM assistants WHERE userId = ? AND is_staff_agent = 1', [userId]);
            if (existing) {
                throw badRequest('You already have a staff assistant. Update it instead of creating a new one.');
            }
        }
        const { name, description, personality, personality_flare, primary_goal, custom_greeting, voice_style, preferred_model, business_type, website, } = req.body;
        if (!name || typeof name !== 'string' || name.trim().length === 0) {
            throw badRequest('Assistant name is required.');
        }
        // Determine if this should be the primary assistant
        const existingCount = await db.queryOne('SELECT COUNT(*) as cnt FROM assistants WHERE userId = ?', [userId]);
        const isFirstAssistant = (existingCount?.cnt ?? 0) === 0;
        const idPrefix = isStaff ? 'staff-assistant' : 'client-assistant';
        const id = `${idPrefix}-${Date.now()}`;
        const now = toMySQLDate(new Date());
        await db.execute(`INSERT INTO assistants
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'paid', 'active', 0, ?, ?)`, [
        ...
        const created = await db.queryOne(`SELECT id, name, description, personality, personality_flare,
              primary_goal, custom_greeting, voice_style, preferred_model,
...
router.put('/:id', requireAuth, async (req, res) => {
    try {
        ...
        if (!userId)
            throw unauthorized('Authentication required.');
        const assistant = await db.queryOne('SELECT id FROM assistants WHERE id = ? AND userId = ?', [req.params.id, userId]);
        if (!assistant)
            throw notFound('Assistant not found.');
        ...
        await db.execute(`UPDATE assistants SET ${sets.join(', ')} WHERE id = ?`, vals);
        const updated = await db.queryOne(`SELECT id, name, description, personality, personality_flare,
              primary_goal, custom_greeting, voice_style, preferred_model,
              status, tier, is_staff_agent, is_primary,
              business_type, website, created_at, updated_at
       FROM assistants WHERE id = ?`, [assistant.id]);
...
// ============================================================================
router.put('/:id/set-primary', requireAuth, async (req, res) => {
    try {
        const userId = req.userId;
        if (!userId)
            throw unauthorized('Authentication required.');
        const assistant = await db.queryOne("SELECT id FROM assistants WHERE id = ? AND userId = ? AND status = 'active'", [req.params.id, userId]);
        if (!assistant)
            throw notFound('Assistant not found.');
        const now = toMySQLDate(new Date());
        // Transaction: unset all → set the chosen one
        await db.transaction(async (conn) => {
            await conn.execute('UPDATE assistants SET is_primary = 0, updated_at = ? WHERE userId = ?', [now, userId]);
            await conn.execute('UPDATE assistants SET is_primary = 1, updated_at = ? WHERE id = ?', [now, assistant.id]);
        });
...
router.delete('/:id', requireAuth, async (req, res) => {
    try {
        ...
        if (!userId)
            throw unauthorized('Authentication required.');
        const assistant = await db.queryOne('SELECT id, is_primary FROM assistants WHERE id = ? AND userId = ?', [req.params.id, userId]);
        if (!assistant)
            throw notFound('Assistant not found.');
        ...
        if (assistant.is_primary) {
            const next = await db.queryOne("SELECT id FROM assistants WHERE userId = ? AND status = 'active' ORDER BY created_at DESC LIMIT 1", [userId]);
            if (next) {
                await db.execute('UPDATE assistants SET is_primary = 1, updated_at = ? WHERE id = ?', [toMySQLDate(new Date()), next.id]);
            }
        }
...
// ============================================================================
router.post('/core-instructions', requireAuth, async (req, res) => {
    try {
        const userId = req.userId;
        if (!userId)
            throw unauthorized('Authentication required.');
        // Only admins can set core instructions
        const adminCheck = await db.queryOne('SELECT is_admin FROM users WHERE id = ?', [userId]);
        if (!adminCheck || !adminCheck.is_admin)
            throw forbidden('Only administrators can set core instructions.');
        const { assistantId, core_instructions } = req.body;
        if (!assistantId)
            throw badRequest('assistantId is required.');
        await db.execute('UPDATE assistants SET core_instructions = ?, updated_at = ? WHERE id = ?', [core_instructions || null, toMySQLDate(new Date()), assistantId]);
...
router.get('/software-tokens', requireAuth, async (req, res) => {
    try {
        const userId = req.userId;
        if (!userId)
            throw unauthorized('Authentication required.');
        await requireStaffRole(userId);
        const tokens = await db.query(`SELECT id, software_id, software_name, api_url, created_at, updated_at
       FROM staff_software_tokens
       WHERE user_id = ?
       ORDER BY software_name ASC`, [userId]);
...
router.post('/software-tokens', requireAuth, async (req, res) => {
    try {
        const userId = req.userId;
        if (!userId)
            throw unauthorized('Authentication required.');
        ...
        const now = toMySQLDate(new Date());
        const existing = await db.queryOne('SELECT id FROM staff_software_tokens WHERE user_id = ? AND software_id = ?', [userId, software_id]);
        if (existing) {
            await db.execute(`UPDATE staff_software_tokens
         SET token = ?, api_url = ?, software_name = ?, updated_at = ?
         WHERE id = ?`, [token, api_url, software_name || null, now, existing.id]);
            res.json({ success: true, message: 'Token updated.', id: existing.id });
        }
        else {
            const id = randomUUID();
            await db.execute(`INSERT INTO staff_software_tokens (id, user_id, software_id, software_name, api_url, token, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [id, userId, software_id, software_name || null, api_url, token, now, now]);
...
router.delete('/software-tokens/:id', requireAuth, async (req, res) => {
    try {
        const userId = req.userId;
        if (!userId)
            throw unauthorized('Authentication required.');
        const { id } = req.params;
        const token = await db.queryOne('SELECT id FROM staff_software_tokens WHERE id = ? AND user_id = ?', [id, userId]);
        if (!token)
