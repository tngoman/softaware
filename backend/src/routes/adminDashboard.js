...
adminDashboardRouter.get('/', requireAuth, async (req, res, next) => {
    try {
        ...
        const wsNew = await db.queryOne('SELECT COUNT(*) AS cnt FROM teams WHERE createdAt >= DATE_SUB(NOW(), INTERVAL 30 DAY)');
        ...
        // ── Subscriptions ───────────────────────────────────────
        const subStats = await db.queryOne(`
      SELECT
        COUNT(*)                                                  AS total,
    ...
    `);
        // ── Software Products ───────────────────────────────────
        const swTotal = await db.queryOne('SELECT COUNT(*) AS cnt FROM update_software');
        const swIntegrated = await db.queryOne('SELECT COUNT(*) AS cnt FROM update_software WHERE has_external_integration = 1');
        const moduleTotal = await db.queryOne('SELECT COUNT(*) AS cnt FROM update_modules');
        const releaseTotal = await db.queryOne('SELECT COUNT(*) AS cnt FROM update_releases');
        // ── Connected Clients (Desktops) ────────────────────────
        const clientTotal = await db.queryOne('SELECT COUNT(*) AS cnt FROM update_clients');
        const clientOnline = await db.queryOne('SELECT COUNT(*) AS cnt FROM update_clients WHERE last_heartbeat >= DATE_SUB(NOW(), INTERVAL 5 MINUTE)');
        const clientBlocked = await db.queryOne('SELECT COUNT(*) AS cnt FROM update_clients WHERE is_blocked = 1');
        // ── AI & Assistants ─────────────────────────────────────
        const assistantTotal = await db.queryOne('SELECT COUNT(*) AS cnt FROM assistants');
        const aiConfigRow = await db.queryOne('SELECT COUNT(*) AS cnt FROM ai_model_config');
        const apiKeyTotal = await db.queryOne('SELECT COUNT(*) AS cnt FROM api_keys');
        // Credit usage (AI consumption)
        const creditStats = await db.queryOne(`
      SELECT
    ...
    `);
        const creditBalanceRow = await db.queryOne('SELECT COALESCE(SUM(balance), 0) AS totalBalance FROM credit_balances');
        // AI usage by request type
        const aiUsageByType = await db.query(`
    ...
    `);
        ...
        const siteTotal = await db.queryOne('SELECT COUNT(*) AS cnt FROM generated_sites');
        const siteDeployed = await db.queryOne("SELECT COUNT(*) AS cnt FROM generated_sites WHERE status = 'deployed'");
        const siteDraft = await db.queryOne("SELECT COUNT(*) AS cnt FROM generated_sites WHERE status = 'draft'");
        // Widget clients (chat widgets deployed on third-party sites)
        const widgetTotal = await db.queryOne('SELECT COUNT(*) AS cnt FROM widget_clients');
        const widgetActive = await db.queryOne("SELECT COUNT(*) AS cnt FROM widget_clients WHERE status = 'active'");
        // ── Leads ───────────────────────────────────────────────
        const leadTotal = await db.queryOne('SELECT COUNT(*) AS cnt FROM lead_captures');
        const leadNew = await db.queryOne("SELECT COUNT(*) AS cnt FROM lead_captures WHERE status = 'NEW'");
        const leadThisMonth = await db.queryOne('SELECT COUNT(*) AS cnt FROM lead_captures WHERE createdAt >= DATE_SUB(NOW(), INTERVAL 30 DAY)');
        // ── Activation Keys ─────────────────────────────────────
        const keyStats = await db.queryOne(`
      SELECT
        COUNT(*)                                          AS total,
    ...
    `);
        ...
        const activity = [];
        // Recent teams
        const recentTeams = await db.query('SELECT id, name, createdAt FROM teams ORDER BY createdAt DESC LIMIT 3');
        ...
        // Recent users
        const recentUsers = await db.query('SELECT id, email, name, createdAt FROM users ORDER BY createdAt DESC LIMIT 3');
        ...
        const recentClients = await db.query(`SELECT id, hostname, machine_name, ip_address, last_heartbeat
       FROM update_clients
       ORDER BY last_heartbeat DESC LIMIT 3`);
        ...
        // Recent leads
        const recentLeads = await db.query('SELECT id, contactName, companyName, email, createdAt FROM lead_captures ORDER BY createdAt DESC LIMIT 2');
        ...
        res.json({
            ...
            data: {
                ...
                ai: {
                    ...
                    usageByType: aiUsageByType.map((r) => ({
                        type: r.requestType,
                        count: Number(r.cnt),
