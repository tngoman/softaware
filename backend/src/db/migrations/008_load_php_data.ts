...
async function loadTaxRates(): Promise<void> {
  ...
  for (const rate of rates) {
    const exists = await db.queryOne(
      'SELECT id FROM tax_rates WHERE tax_name = ?',
      [rate.tax_name]
    );
 ...
 */
async function loadQuoteItems(): Promise<void> {
 ...
 */
async function loadInvoiceItems(): Promise<void> {
  // Placeholder - actual data from PHP dump
  const items: any[] = [];
...
async function loadPayments(): Promise<void> {
  ...
  for (const payment of payments) {
    ...
    const totalPaid = await db.queryOne<{ total: number }>(
    ...
    if (totalPaid && totalPaid.total >= invoice.invoice_amount) {
      await db.execute(
        ...
        [new Date().toISOString(), payment.invoice_id]
      );
    }
  }
}
