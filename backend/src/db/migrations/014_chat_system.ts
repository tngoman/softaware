...
export async function up() {
  ...
  if (await tableExists('team_chats')) {
    ...
    if ((existing?.cnt ?? 0) === 0) {
      ...
      // Migrate groups
      const teams = await db.query<any>('SELECT * FROM team_chats');
      for (const team of teams) {
        const convId = await db.insert(
          `INSERT INTO conversations (type, name, description, created_by, created_at, updated_at)
           VALUES ('group', ?, ?, ?, ?, ?)`,
          [
            team.name,
        ...
        const members = await db.query<any>(
          'SELECT * FROM team_chat_members WHERE team_id = ?',
          [team.id],
        );
        for (const m of members) {
          await db.execute(
            `INSERT INTO conversation_members
             ...
             VALUES (?, ?, ?, ?, ?)`,
            [convId, m.user_id, m.role, m.joined_at, m.removed_at],
          );
        }

        // Migrate messages (map old IDs → new IDs for reply_to)
        const oldMessages = await db.query<any>(
          'SELECT * FROM team_chat_messages WHERE team_id = ? ORDER BY id ASC',
          [team.id],
        );
        const idMap = new Map<number, string>(); // old ID → new ID
