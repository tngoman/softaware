...
export async function up(pool: Pool) {
  ...
  try {
    ...
    const roles: Array<{ name: string; slug: string; description: string }> = [
    ...
    for (const r of roles) {
      ...
      if (existing.length === 0) {
        ...
        await conn.execute(
          'UPDATE roles SET name = ?, description = ? WHERE slug = ?',
    ...
    await conn.execute(
      `UPDATE roles SET name = 'Viewer', slug = 'viewer', description = 'Read-only access to dashboards and reports'
       WHERE slug = 'user' AND NOT EXISTS (SELECT 1 FROM (SELECT id FROM roles WHERE slug = 'viewer') AS t)`,
    );

    // ── 2. Seed permissions ─────────────────────────────────────────
    const permissions: Array<{ name: string; slug: string; group: string }> = [
      // Dashboard
    ...
    for (const p of permissions) {
      const [existing] = await conn.query<any[]>(
        'SELECT id FROM permissions WHERE slug = ?', [p.slug]
      );
      if (existing.length === 0) {
        await conn.execute(
          'INSERT INTO permissions (name, slug, description, permission_group, created_at, updated_at) VALUES (?, ?, NULL, ?, NOW(), NOW())',
    ...
    async function roleId(slug: string): Promise<number> {
      const [rows] = await conn.query<any[]>('SELECT id FROM roles WHERE slug = ?', [slug]);
      return rows[0]?.id;
    }
    // Helper: get permission id
    async function permId(slug: string): Promise<number> {
      const [rows] = await conn.query<any[]>('SELECT id FROM permissions WHERE slug = ?', [slug]);
      return rows[0]?.id;
    }
    // Helper: link
    async function link(rSlug: string, pSlug: string) {
      const rId = await roleId(rSlug);
      const pId = await permId(pSlug);
      if (!rId || !pId) return;
      const [existing] = await conn.query<any[]>(
        'SELECT role_id FROM role_permissions WHERE role_id = ? AND permission_id = ?', [rId, pId]
      );
      if (existing.length === 0) {
        await conn.execute(
    ...
    for (const p of devPerms) await link('developer', p);
    ...
    for (const p of cmPerms) await link('client_manager', p);
    ...
    for (const p of qaPerms) await link('qa_specialist', p);
    ...
    for (const p of deployPerms) await link('deployer', p);

    // Viewer — read-only dashboard
    const viewerPerms = ['dashboard.view'];
    for (const p of viewerPerms) await link('viewer', p);

    // ── 4. Assign admin user to Admin role (via user_roles) ─────────
    const adminRoleId = await roleId('admin');
    if (adminRoleId) {
      // Get admin user
      const [adminUsers] = await conn.query<any[]>(
      ...
      for (const u of adminUsers) {
        const [existing] = await conn.query<any[]>(
          'SELECT id FROM user_roles WHERE user_id COLLATE utf8mb4_unicode_ci = ? COLLATE utf8mb4_unicode_ci AND role_id = ?',
          [u.id, adminRoleId]
        );
        if (existing.length === 0) {
          await conn.execute(
