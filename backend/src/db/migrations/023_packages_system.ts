...
export async function up(): Promise<void> {
  ...
  const [existing] = await db.query('SELECT id FROM contacts WHERE id = 1') as any[];
  ...
  for (const pkg of packageSeeds) {
    const [exists] = await db.query('SELECT id FROM packages WHERE slug = ?', [pkg.slug]) as any[];
  ...
  const [staffPkg] = await db.query('SELECT id, credits_included FROM packages WHERE slug = ?', ['staff']) as any[];
  if (staffPkg && staffPkg.length > 0) {
    const staffPkgId = staffPkg[0].id;
    const credits = staffPkg[0].credits_included;
    const [existingAssignment] = await db.query(
      'SELECT id FROM contact_packages WHERE contact_id = 1 AND package_id = ?',
      [staffPkgId]
    ) as any[];
