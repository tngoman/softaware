...
export function sanitizeText(text) {
    ...
    clean = clean.replace(/(\+27|0)[6-8]\d[\s\-]?\d{3}[\s\-]?\d{4}/g, '[PHONE REMOVED]');
    // 3. Generic international phone numbers (10+ digits with optional separators)
    clean = clean.replace(/\b\d{3}[\-.\s]?\d{3}[\-.\s]?\d{4}\b/g, '[PHONE REMOVED]');
    ...
    clean = clean.replace(/\b(?:\d[\s\-]*?){13,19}\b/g, '[CARD REMOVED]');
    ...
    clean = clean.replace(/\b(?:account|acc|ref(?:erence)?)\s*(?:no\.?|number|#)?\s*:?\s*(\d{7,11})\b/gi, '[ACCOUNT REMOVED]');
    // 7. Physical addresses (street numbers followed by road types)
    // Light touch — only obvious patterns to avoid over-stripping
    clean = clean.replace(/\b\d{1,5}\s+[A-Z][a-z]+\s+(?:Street|St|Road|Rd|Avenue|Ave|Drive|Dr|Lane|Ln|Boulevard|Blvd|Crescent|Cres)\b/gi, '[ADDRESS REMOVED]');
    return clean;
}
/**
 ...
 */
export function logAnonymizedChat(clientId, rawPrompt, rawResponse, options = {}) {
    try {
        const db = getDb();
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `);
        stmt.run(clientId, options.source || 'assistant', cleanPrompt, cleanResponse, options.model || null, options.provider || null, options.durationMs || null);
    }
    catch (error) {
        // Non-fatal — analytics must never crash the chat
        console.error('[Analytics] Non-fatal: Failed to log telemetry:', error.message);
    }
}
