 ...
 */
export async function validateTeamMembership(req, res, next) {
    try {
        const userId = req.userId;
        ...
        const membership = await db.queryOne('SELECT * FROM team_members WHERE teamId = ? AND userId = ?', [teamId, userId]);
        if (!membership) {
