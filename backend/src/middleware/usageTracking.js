...
async function getTierLimits(tier) {
    try {
        const [rows] = await db.execute(`SELECT max_pages, max_messages_per_month 
 ...
 */
async function ensureBillingCycle(clientId) {
 ...
 */
export async function trackMessageUsage(clientId) {
    try {
       ...
       WHERE id = ?`, [clientId]);
        // Log usage
        const cycle = getCurrentBillingCycle();
        await db.execute(`INSERT INTO widget_usage_logs (id, client_id, message_count, cycle_start, cycle_end)
       VALUES (?, ?, 1, ?, ?)
 ...
 */
export async function enforceMessageLimit(req, res, next) {
    try {
        ...
        await trackMessageUsage(clientId);
 ...
 */
export async function getUsageStats(clientId, months = 3) {
    try {
        const [rows] = await db.execute(`SELECT 
         cycle_start,
         cycle_end,
       ...
       ORDER BY cycle_start DESC`, [clientId, months]);
        return rows || [];
    }
    catch (error) {
 ...
 */
export async function resetExpiredBillingCycles() {
    try {
        const result = await db.execute(`UPDATE widget_clients
 ...
 */
export async function getClientsNearLimit(threshold = 0.9) {
    try {
        const [rows] = await db.execute(`SELECT 
         wc.id,
         wc.user_id,
       ...
       ORDER BY usage_ratio DESC`, [threshold]);
        return rows || [];
    }
    catch (error) {
