...
import { REQUEST_PRICING } from '../config/credits.js';
/**
 ...
 */
export function requirePackage(req, res, next) {
    (async () => {
        try {
 ...
 * Must be placed after requirePackage.
 */
export function deductCreditsAfterResponse(req, res, next) {
    ...
    res.end = function (...args) {
        ...
        if (res.statusCode >= 200 && res.statusCode < 300 && req.contactId) {
            ...
            if (cost > 0) {
                packageService.deductCredits(req.contactId, cost, req.user?.id || null, requestType, { path: req.path, method: req.method }, `${requestType} request`).then(result => {
 ...
 */
export function packageCreditMiddleware(req, res, next) {
...
function detectRequestType(req) {
    ...
    if (path.includes('/chat') || path.includes('/conversation'))
        return 'TEXT_CHAT';
    if (path.includes('/embed') || path.includes('/ingest'))
        return 'TEXT_SIMPLE';
    if (path.includes('/image'))
        return 'TEXT_CHAT'; // image gen uses chat-level pricing
    if (path.includes('/tts') || path.includes('/speech'))
        return 'TEXT_SIMPLE';
    if (path.includes('/stt') || path.includes('/transcri'))
        return 'TEXT_SIMPLE';
    if (path.includes('/code') || path.includes('/execute'))
        return 'CODE_AGENT_EXECUTE';
...
function calculateCost(requestType, responseBody) {
    const costs = REQUEST_PRICING;
    const config = costs[requestType];
    if (!config)
        return 1; // minimum 1 credit for any API call
