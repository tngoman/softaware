...
export interface ChecklistItem {
  key: string;           // Unique slug, e.g. "pricing_info"
  label: string;         // Human-readable, e.g. "Pricing Information"
  satisfied: boolean;    // Has relevant content been ingested?
  type: 'url' | 'file'; // Suggested upload method
  custom?: boolean;      // true = user-added (paid only)
}
...
export interface PersonaTemplate {
  ...
  name: string;
  description: string;
  checklist: Omit<ChecklistItem, 'satisfied'>[];
}
...
export function getDefaultChecklist(businessType: string): ChecklistItem[] {
...
export function getTemplate(businessType: string): PersonaTemplate {
