...
async function main() {
  try {
    ...
    const sources = await db.query<any>('SELECT * FROM task_sources WHERE sync_enabled = 1');
    ...
    // 3. Show stats
    const [count] = await db.query<any>('SELECT COUNT(*) as total FROM local_tasks WHERE task_deleted = 0');
    console.log(`\n✅ Total active local tasks: ${count.total}`);

    // 4. Show a sample
    const sample = await db.query<any>('SELECT id, external_id, title, status, type, source_id FROM local_tasks WHERE task_deleted = 0 ORDER BY id DESC LIMIT 5');
    if (sample.length > 0) {
      console.log('\n📝 Latest tasks:');
    ...
    // 5. Show sync log
    const logs = await db.query<any>('SELECT * FROM task_sync_log ORDER BY id DESC LIMIT 3');
    console.log('\n📜 Recent sync logs:');
    for (const l of logs) {
